...
function getBaseRef(): string {
  if (process.env.GITHUB_BASE_REF) {
    try {
      execSync(`git rev-parse --verify origin/${process.env.GITHUB_BASE_REF}`, { stdio: 'ignore' })
...
function runScopeCheck() {
  ...
  if (blockFiles.length > 0 && primitiveFiles.length > 0) {
    ...
    console.error(
      'This PR touches blocks but ALSO modifies existing primitives or design tokens.',
    )
    console.error(
      'Block contributions must compose existing primitives raw and must NEVER mutate existing primitives.',
    )
  ...
  for (const blockName of uniqueNewBlocks) {
    ...
    if (!/^[a-z0-9]+(-[a-z0-9]+)*$/.test(blockName)) {
      console.error(`❌ [PR Scope Guard] Block folder name "${blockName}" must be lowercase kebab-case (e.g. cloud-backup-schedule).`)
      process.exit(1)
    }
    ...
    if (fs.existsSync(vueCompPath) || fs.existsSync(reactCompPath)) {
      console.error(`❌ [PR Scope Guard] Block name "${blockName}" collides with an existing primitive in components/${blockName}! Blocks must have unique, descriptive names.`)
      process.exit(1)
    }
  }

  if (primitiveFiles.length > 0 && blockFiles.length === 0) {
    console.log(`ℹ️ [Primitive PR] PR modifies ${primitiveFiles.length} primitive file(s). Automated test suites will verify compatibility.`)
  } else if (blockFiles.length > 0) {
    console.log(`✅ [Block PR] Scope is clean! PR touches ${blockFiles.length} block file(s) with zero regressions to existing primitives.`)
  } else {
