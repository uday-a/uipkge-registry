...
function runScopeCheck() {
  ...
  if (blockFiles.length > 0 && primitiveFiles.length > 0) {
    ...
    if (process.env.ALLOW_CROSS_BOUNDARY_PR === "true") {
      console.warn(
        "⚠️ [PR Scope Guard Warning] Cross-boundary changes permitted via ALLOW_CROSS_BOUNDARY_PR=true.",
      );
      return;
    }
    ...
    console.error("\n" + "=".repeat(70));
    console.error(
      "❌ [PR SCOPE VIOLATION] Unwanted Modification to Existing Primitives!",
    );
    ...
    console.error("\nHow to fix this:");
    console.error(
      "1. If these edits were accidental (e.g. formatter, auto-import), revert them:",
    );
    console.error(
      "   git checkout origin/main -- packages/vue/components/ packages/react/components/ packages/shared/",
    );
    console.error("2. If you genuinely intended to modify or fix a primitive:");
    console.error(
      "   Submit the primitive change in a dedicated PR titled `feat(ui): ...` or `fix(ui): ...`",
    );
    console.error(
      "   so that all 1,400+ primitive regression tests run independently.",
    );
    console.error("=".repeat(70) + "\n");
    process.exit(1);
  }
  ...
  for (const blockName of uniqueNewBlocks) {
    ...
    const vueCompPath = path.join(
      process.cwd(),
      "packages/vue/components",
      blockName,
    );
    const reactCompPath = path.join(
      process.cwd(),
