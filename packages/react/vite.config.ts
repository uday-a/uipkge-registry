...
export default defineConfig({
  ...
  resolve: {
    alias: [
      { find: '@/lib/utils', replacement: path.join(root, 'bootstrap/utils/utils.ts') },
      { find: '@/lib/use-kanban', replacement: path.join(root, 'bootstrap/use-kanban/useKanban.ts') },
      { find: '@/lib/use-month-grid', replacement: path.join(root, 'bootstrap/use-month-grid/useMonthGrid.ts') },
      { find: '@/lib/use-theme', replacement: path.join(root, 'bootstrap/use-theme/use-theme.ts') },
      { find: /^@\/components\/ui\/(.*)$/, replacement: path.join(root, 'components/$1') },
      { find: /^@\/lib\/(.*)$/, replacement: path.join(root, 'bootstrap/$1') },
      { find: /^@\/bootstrap\/(.*)$/, replacement: path.join(root, 'bootstrap/$1') },
      // Demo compatibility aliases
      { find: /.*\/components\/story\/Story(\.tsx?)?$/, replacement: path.join(root, 'playground/src/Story.tsx') },
      { find: /^@react-registry\/(.*)$/, replacement: path.join(root, 'components/$1') },
      { find: /^@react-registry-blocks\/(.*)$/, replacement: path.join(root, 'blocks/$1') },
    ],
  },
})
