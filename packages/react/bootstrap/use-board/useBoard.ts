 ...
 */
export function useBoard<TItem extends { id: string }>(opts: UseBoardOptions<TItem>): UseBoardReturn {
  ...
  const [selectedIds, setSelectedIds] = React.useState<ReadonlySet<string>>(new Set<string>())
  ...
  const moveItem = React.useCallback((itemId: string | string[], toLaneId: string, toIndex?: number) => {
  ...
  const toggleSelection = React.useCallback((itemId: string, additive = false) => {
    setSelectedIds((prev) => {
  ...
  const handlers = React.useMemo<UseBoardReturn['handlers']>(
    () => ({
      onDragStart(e, itemId, laneId) {
        ...
        const ids = inSelection && selected.size > 1 ? Array.from(selected) : [itemId]
      ...
      onLaneDragOver(e, laneId) {
        ...
        const allOk = draggingIdsRef.current.every((id) => isAllowed(id, fromLane, laneId))
      ...
      onLaneDrop(e, laneId) {
        e.preventDefault()
        const fallbackId = e.dataTransfer?.getData('text/plain') || draggingIdRef.current
        const ids = draggingIdsRef.current.length > 0 ? draggingIdsRef.current : fallbackId ? [fallbackId] : []
        if (ids.length === 0) return
  ...
  const registerAllowedLanes = React.useCallback((cardId: string, lanes: readonly string[] | undefined) => {
    if (lanes && lanes.length > 0) allowedLanesByCard.current.set(cardId, lanes)
  ...
  const registerLaneDisabled = React.useCallback((laneId: string, disabled: boolean) => {
  ...
  const isLaneAcceptingFor = React.useCallback((laneId: string) => {
    ...
    const fromLane = sourceLaneByDragId.current ?? laneId
    return draggingIdsRef.current.every((id) => isAllowed(id, fromLane, laneId))
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [])
