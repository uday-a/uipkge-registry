...
export function renderTable(props: Partial<DataTableProps<Employee, unknown>> = {}) {
  const ref = createRef<DataTableHandle<Employee>>()
  const result = render(<DataTable ref={ref} columns={plainColumns} data={employees} {...props} />)
  return { ...result, ref }
}

export function renderSortable(props: Partial<DataTableProps<Employee, unknown>> = {}) {
  return renderTable({
...
export function rowCount(container: HTMLElement = document.body): number {
  const tbody = container.querySelector('[data-slot="data-table"] tbody') ?? container.querySelector('tbody')
  if (!tbody) return 0
...
export function tableApi(ref: React.RefObject<DataTableHandle<Employee> | null>) {
