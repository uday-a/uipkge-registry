...
import { employees, plainColumns, sortableColumns, filters, manyEmployees, type Employee } from './fixtures'
...
function renderSortable(props: Partial<DataTableProps<Employee, unknown>> = {}) {
  return renderTable({
...
function rowCount(): number {
  const tbody = document.querySelector('[data-slot="data-table"] tbody') ?? document.querySelector('tbody')
  if (!tbody) return 0
...
function bodyTexts(): string[] {
  const tbody = document.querySelector('[data-slot="data-table"] tbody') ?? document.querySelector('tbody')
  if (!tbody) return []
  return Array.from(tbody.querySelectorAll('tr')).map((r) => r.textContent ?? '')
}
...
async function clickByName(name: string) {
  const btn = screen.getAllByRole('button').find((b) => (b.textContent ?? '').includes(name))
...
describe('DataTable React catalog (200)', () => {
  ...
  it('DT-REACT-003 custom empty slot', () => {
    renderTable({ data: [], emptyState: createElement('div', null, 'Nothing here') })
  ...
  it('DT-REACT-007 loading true', () => {
    ...
    expect(
      document.querySelector('[data-slot="data-table"]') || document.querySelector('table') || document.body,
    ).toBeTruthy()
  })
  ...
  it('DT-REACT-041 select single row', async () => {
    ...
    await waitFor(() => expect(tableApi(ref).getFilteredSelectedRowModel().rows.length).toBe(1))
  })
  ...
  it('DT-REACT-053 search visible with filterColumn', () => {
    renderSortable()
    expect(screen.getByPlaceholderText(/email/i) || document.querySelector('table')).toBeTruthy()
  })
  ...
  it('DT-REACT-056 type search filters', async () => {
    ...
    await waitFor(() => expect(bodyTexts().some((t) => t.toLowerCase().includes('ankita'))).toBe(true))
  })
  ...
  it('DT-REACT-065 default filterMode inline chips', () => {
    renderSortable()
    expect(screen.getAllByRole('button').some((b) => b.textContent?.includes('Department'))).toBe(true)
    expect(screen.getAllByRole('button').some((b) => b.textContent?.includes('Status'))).toBe(true)
  })
  ...
  it('DT-REACT-067 multiselect filter via API', async () => {
    ...
    tableApi(ref).setPageSize(50)
    tableApi(ref).getColumn('department')?.setFilterValue(['Engineering'])
    await waitFor(() => expect(bodyTexts().every((t) => t.includes('Engineering'))).toBe(true))
  })
  ...
  it('DT-REACT-082 AND two filters', async () => {
    ...
    await waitFor(() => expect(bodyTexts().every((t) => t.includes('Engineering') && t.includes('active'))).toBe(true))
  })
  ...
  it('DT-REACT-085 popover mode Filters button', () => {
    ...
    expect(screen.getAllByRole('button').some((b) => /Filter/i.test(b.textContent || ''))).toBe(true)
  })
  ...
  it('DT-REACT-095 modal mode Filters button', () => {
    ...
    expect(screen.getAllByRole('button').some((b) => b.textContent?.includes('Filters'))).toBe(true)
  })
  ...
  it('DT-REACT-118 next page', () => {
    const { ref } = renderTable({ data: manyEmployees(25), enablePagination: true })
    expect(rowCount()).toBe(10)
    tableApi(ref).nextPage()
    expect(tableApi(ref).getState().pagination.pageIndex).toBeGreaterThanOrEqual(0)
  })
  ...
  it('DT-REACT-134 hide column via API', async () => {
    const { ref } = renderSortable({ enableColumnVisibility: true, enablePagination: false })
    tableApi(ref).getColumn('email')?.toggleVisibility(false)
    await waitFor(() =>
      expect(Array.from(document.querySelectorAll('th')).some((th) => th.textContent?.includes('Email'))).toBe(false),
    )
  })
  ...
  it('DT-REACT-154 toolbar inside default', () => {
    renderSortable({ toolbarPosition: 'above', maxHeight: '200px', stickyHeader: true })
    expect(document.querySelector('table')).toBeTruthy()
  })
  ...
  it('DT-REACT-160 enableResize mounts', () => {
    renderSortable({ enableResize: true, enableReorder: true, defaultColumnPinning: { left: ['select'] } })
    expect(document.querySelector('table')).toBeTruthy()
  })
  ...
  it('DT-REACT-176 totalRows server mode', () => {
    const { ref } = renderTable({ totalRows: 50, data: employees.slice(0, 10) })
  ...
  it('DT-REACT-198 unicode emoji cell', () => {
    renderTable({ data: [{ ...employees[0], name: 'Ada 🚀' }], enablePagination: false })
  ...
  it('DT-REACT-200 html-like string escaped as text', () => {
    renderTable({ data: [{ ...employees[0], name: '<img src=x onerror=alert(1)>' }], enablePagination: false })
    expect(document.querySelector('tbody')?.innerHTML).not.toMatch(/<img src=x/)
  ...
  it('DT-REACT-203 keyboard navigation triggers focus and selection', () => {
    const { ref } = renderSortable()
    const container = document.querySelector('[data-slot="data-table"]') as HTMLElement
  ...
  it('DT-REACT-204 bulkActionPosition inline mode renders inline bar', () => {
    renderTable({
      ...
      renderBulkActions: () => createElement('button', { className: 'custom-del' }, 'Delete'),
    })
