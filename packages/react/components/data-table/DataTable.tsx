...
import { Search, Copy, Download, X, ChevronDown, ChevronRight } from 'lucide-react'
...
function DataTableInner<TData, TValue>(
  ...
  const [densityOverride, setDensityOverride] = React.useState<'compact' | 'cozy' | 'comfortable' | null>(null)
  ...
  const table = useReactTable({
    ...
    getPaginationRowModel: isServerSide || !enablePagination ? undefined : getPaginationRowModel(),
  ...
  const [lastSelectedRowIndex, setLastSelectedRowIndex] = React.useState<number>(-1)
  ...
  return (
    ...
    >
      ...
      >
        ...
        {bulkActionPosition === 'inline' && renderBulkActions && selectedRows.length > 0 && (
        ...
        >
          <Table>
            ...
            <TableBody
              className={loading && table.getRowModel().rows?.length ? 'pointer-events-none opacity-60' : undefined}
              aria-busy={loading || undefined}
            >
              {loading && !table.getRowModel().rows?.length ? (
                Array.from({ length: 5 }).map((_, i) => (
                  <TableRow key={`sk-${i}`} className="hover:bg-transparent">
                    {Array.from({ length: Math.max(columns.length, 1) }).map((__, j) => (
                    ...
                    {row.getIsGrouped() ? (
                      ...
                      <>
                        <TableRow
                          ...
                          style={virtual ? { contentVisibility: 'auto', containIntrinsicSize: 'auto 48px' } : undefined}
                          onClick={(e) => onRowClicked(row, idx, e)}
                        >
                          {row.getVisibleCells().map((cell) => (
                            <TableCell
                              key={cell.id}
                              className={row.getIsSelected() ? 'bg-muted' : 'bg-card'}
      ...
      {bulkActionPosition === 'floating' && selectedRows.length > 0 && (
        <div
          ...
          className="border-border/80 bg-background/95 text-foreground fixed bottom-4 left-1/2 z-30 flex -translate-x-1/2 items-center gap-2.5 rounded-full border px-3.5 py-1.5 shadow-xl backdrop-blur-md transition-all select-none sm:absolute"
        >
