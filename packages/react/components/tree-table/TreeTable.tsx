...
const TreeTable = React.forwardRef<HTMLDivElement, TreeTableProps>(
  (
    ...
    return (
      <div ref={ref} data-uipkge data-slot="tree-table" role="treegrid" className={cn('relative w-full', className)}>
        <Table>
          <TableHeader>
            <TableRow>
              {selectable && (
                ...
                {columns.map((col, ci) => (
                  <TableCell key={col.key} className={cn(ci === 0 && 'font-medium', col.cellClass)}>
                    ...
                    >
                      ...
                      {renderCell
                        ? (renderCell(col, fr.row, fr.depth) ?? (col.render ? col.render(fr.row) : fr.row[col.key]))
                        : col.render
                          ? col.render(fr.row)
                          : fr.row[col.key]}
                    </div>
