    ...
    if (task.isGroup) {
      return (
        ...
        >
          ...
          {task.progress != null ? <span className="font-mono text-xs opacity-80">{task.progress}%</span> : null}
        </div>
      )
    }
      ...
      >
        {task.progress != null && task.progress > 0 ? (
          <div
            ...
            className={cn('absolute inset-y-0 left-0 transition-[left]', progressColors[task.status ?? 'in-progress'])}
          />
        ) : null}
        ...
        <div className="relative z-10 flex w-full min-w-0 items-center justify-between px-2">
          ...
          {task.progress != null ? (
            <span className="ml-1 shrink-0 font-mono text-xs opacity-80">{task.progress}%</span>
          ) : null}
        </div>
