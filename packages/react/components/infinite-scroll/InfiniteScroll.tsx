...
function InfiniteScrollInner<T>(props: InfiniteScrollProps<T>, ref: React.Ref<HTMLDivElement>) {
  ...
  function check() {
    ...
    if (state.reverse) {
      ...
      if (sentinelRect.bottom >= edgeTop - state.distance && sentinelRect.top <= edgeBottom) {
        state.onLoadMore?.()
      }
    } else {
      // Forward: fire when the sentinel approaches the bottom edge.
      if (sentinelRect.top <= edgeBottom + state.distance && sentinelRect.bottom >= edgeTop - state.distance) {
        state.onLoadMore?.()
      }
    }
  }
