...
describe("Phone", () => {
  it("renders the iPhone geometry and preserves screen content by default", () => {
    ...
    expect(
      container.querySelector(
        '[data-slot="phone-camera-control"][data-side="right"]',
      ),
    ).toBeTruthy();
    expect(
      container.querySelectorAll(
        '[data-slot="phone-volume-button"][data-side="left"]',
      ),
    ).toHaveLength(2);
  ...
  it("uses the official iPhone chassis aspect ratio", () => {
    ...
    const chassis = container.querySelector(
  ...
  it("renders Galaxy controls on the right only", () => {
    ...
    expect(
      container.querySelector(
        '[data-slot="phone-volume-button"][data-side="right"]',
      ),
    ).toBeTruthy();
    expect(
      container.querySelector(
        '[data-slot="phone-side-button"][data-side="right"]',
      ),
    ).toBeTruthy();
    expect(container.querySelector('[data-side="left"]')).toBeNull();
    expect(
      container.querySelector('[data-slot="phone-camera-control"]'),
    ).toBeNull();
  });
  ...
  it("keeps the deprecated android variant working", () => {
    ...
    expect(
      container
        .querySelector('[data-slot="phone"]')
        ?.getAttribute("data-model"),
    ).toBe("galaxy-s26-ultra");
  });
