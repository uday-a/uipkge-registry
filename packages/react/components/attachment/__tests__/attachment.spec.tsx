...
describe("Attachment", () => {
  ...
  it("shows title and description", () => {
    const { container } = render(
      <Attachment title="notes.pdf" description="PDF · 2.4 MB" />,
    );
  ...
  it("sets size and orientation from props", () => {
    const { container } = render(
      <Attachment title="a" size="xs" orientation="vertical" />,
    );
  ...
  it("calls onRemove when removable", () => {
    ...
    const { container } = render(
      <Attachment title="notes.pdf" removable onRemove={onRemove} />,
    );
    fireEvent.click(
