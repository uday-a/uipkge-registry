  ...
  (
    ...
    const moveRight = React.useCallback(
      (keys?: string[]) => {
        ...
        const next = [...s.resolvedTargetKeys, ...moved.filter((k) => !s.resolvedTargetKeys.includes(k))]
...
function TransferList({ side, title, items, selected, onSelectedChange, onSearch, footer }: TransferListProps) {
  ...
  const visibleEnabledKeys = React.useMemo(() => visible.filter((i) => !i.disabled).map((i) => i.key), [visible])
  ...
  function onRowKeydown(e: React.KeyboardEvent, item: TransferItem) {
    ...
    onSelectedChange(selectedSet.has(item.key) && selected.length === 1 ? [] : [item.key])
    setLastAnchor(item.key)
  }
  ...
  return (
    ...
    >
      ...
      >
        <ul role="listbox" aria-label={title} aria-multiselectable="true" className="py-1">
          {visible.map((item) => (
            <li
              key={item.key}
              role="option"
