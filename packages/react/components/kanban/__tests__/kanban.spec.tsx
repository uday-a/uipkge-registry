...
function renderBoard(
  ...
  onCardMove?: (event: { cardId: string; fromColumnId: string; toColumnId: string; toIndex?: number }) => void,
) {
  return render(
    <Kanban onCardMove={onCardMove}>
      <KanbanBoard>
        {Object.entries(cards).map(([columnId, ids]) => (
          <KanbanColumn key={columnId} id={columnId} label={columnId === 'todo' ? 'To do' : 'Done'}>
            <KanbanColumnHeader>
...
describe('Kanban Primitive (React)', () => {
  ...
  it('exposes each draggable card to assistive tech as a focusable grab target', () => {
    ...
    expect(container.querySelector('[data-slot="kanban-live-region"]')!.getAttribute('aria-live')).toBe('polite')
  })
  ...
  it('picks a card up on Space and announces how to move it', () => {
    ...
    expect(container.querySelector('[data-slot="kanban-live-region"]')!.textContent).toContain('Picked up card')
  })
  ...
  it('moves a grabbed card to the next column on ArrowRight', () => {
    ...
    expect(handleMove).toHaveBeenCalledWith({ cardId: 'card-1', fromColumnId: 'todo', toColumnId: 'done' })
    expect(container.querySelector('[data-slot="kanban-live-region"]')!.textContent).toContain('Moved to Done')
  })

  it('reorders within a column on ArrowDown, reporting the new position', () => {
    const handleMove = vi.fn()
    const { container } = renderBoard({ todo: ['card-1', 'card-2'], done: [] }, {}, handleMove)
    const card = container.querySelector('[data-card-id="card-1"]')!
    ...
    expect(container.querySelector('[data-slot="kanban-live-region"]')!.textContent).toContain('Position 2 of 2')
  })
  ...
  it('ignores arrow keys until the card is grabbed', () => {
    ...
    fireEvent.keyDown(container.querySelector('[data-card-id="card-1"]')!, { key: 'ArrowRight' })
  ...
  it('releases the card on Escape and on blur so it cannot get stuck held', () => {
    ...
    expect(card.getAttribute('aria-pressed')).toBe('false')
    expect(container.querySelector('[data-slot="kanban-live-region"]')!.textContent).toContain('Move cancelled')
  ...
  it('drops the card on a second Space', () => {
    ...
    expect(card.getAttribute('aria-pressed')).toBe('false')
    expect(container.querySelector('[data-slot="kanban-live-region"]')!.textContent).toContain('Card dropped')
  })

  it('keeps disabled cards out of the tab order and unmovable', () => {
    const handleMove = vi.fn()
    const { container } = renderBoard({ todo: ['card-1'], done: [] }, { disabled: true }, handleMove)
    const card = container.querySelector('[data-card-id="card-1"]')!
  ...
  it('leaves pointer dragging intact when keyboardDraggable is off', () => {
    const { container } = renderBoard({ todo: ['card-1'], done: [] }, { keyboardDraggable: false })
    const card = container.querySelector('[data-card-id="card-1"]')!
