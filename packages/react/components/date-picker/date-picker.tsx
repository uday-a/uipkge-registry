...
function getLastTime(
  ...
  if (type === "single" && internal instanceof Date) {
    if (internal.getHours() || internal.getMinutes() || internal.getSeconds()) {
      return {
        h: internal.getHours(),
        m: internal.getMinutes(),
        s: internal.getSeconds(),
      };
    }
  }
  if (type === "range") {
    ...
    if (
      ...
      return {
        h: r.start.getHours(),
        m: r.start.getMinutes(),
        s: r.start.getSeconds(),
      };
    }
  }
...
const DatePicker = React.forwardRef<HTMLButtonElement, DatePickerProps>(
  (
    ...
    const [previewValue, setPreviewValue] =
      React.useState<InternalValue>(undefined);
    ...
    const minDate = React.useMemo(
      () => coerceDate(minValue) ?? undefined,
      [minValue],
    );
    const maxDate = React.useMemo(
      () => coerceDate(maxValue) ?? undefined,
      [maxValue],
    );
    ...
    function emitOut(v: InternalValue) {
      ...
      if (type === "range") {
        ...
        onValueChange?.({
          start: serializeDate(r.start),
          end: serializeDate(r.end),
        });
        return;
      }
    ...
    const display = React.useMemo(() => {
      ...
      if (type === "multiple") {
        ...
        if (arr.length <= 3)
          return arr.map((d) => fmtDate(d, locale, effectiveFormat)).join(", ");
      ...
      if (type === "range") {
        ...
        const fmt = showTime
          ? (d: Date) =>
              fmtDateTime(d, locale, effectiveFormat, showSeconds, use24Hour)
          : (d: Date) => fmtDate(d, locale, effectiveFormat);
        if (r.start && r.end)
          return `${fmt(r.start)} ${separator} ${fmt(r.end)}`;
    ...
    const lastTime = React.useMemo(
      () => getLastTime(activeValue, type, defaultTime),
      [activeValue, type, defaultTime],
    );
    ...
    function handleCalendarUpdate(v: unknown) {
      ...
      if (needConfirm) {
        if (type === "multiple") {
          const arr = (v as Date[]) ?? [];
          setPreviewValue(
            arr.map((d) => (showTime ? withTime(d, lastTime) : d)),
          );
        } else if (type === "range") {
          ...
          setPreviewValue(
            d ? (showTime ? withTime(d, lastTime) : d) : undefined,
          );
        }
        return;
      }
      if (!showTime) {
        if (type === "range")
          handleUpdate(rangeFromCalendar(v as DateRange | undefined));
    ...
    const calendarValue = React.useMemo(() => {
      ...
      const v = activeValue;
      if (type === "multiple")
        return (v as InternalMultiple | undefined)?.map(stripTime);
      if (type === "range")
        return rangeToCalendar(v as InternalRange | undefined);
            ...
            <div className="flex">
              ...
              {type === "range" ? (
                ...
                <Calendar
                  ...
                  startMonth={
                    captionLayout && minDate ? stripTime(minDate) : undefined
                  }
                  endMonth={
                    captionLayout && maxDate ? stripTime(maxDate) : undefined
                  }
