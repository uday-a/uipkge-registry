...
describe('Link', () => {
  ...
  it('renders as an anchor element', () => {
    const { container } = render(<Link href="/about" />)
    expect(container.querySelector('[data-slot="link"]')?.tagName.toLowerCase()).toBe('a')
  })

  it('renders the href attribute', () => {
    const { container } = render(<Link href="/about" />)
    expect(container.querySelector('[data-slot="link"]')?.getAttribute('href')).toBe('/about')
  })
  ...
  it('applies data-color attribute', () => {
    ...
    expect(container.querySelector('[data-slot="link"]')?.getAttribute('data-color')).toBe('muted')
  })

  it('applies data-underline attribute', () => {
    const { container } = render(<Link href="/about" underline="always" />)
    expect(container.querySelector('[data-slot="link"]')?.getAttribute('data-underline')).toBe('always')
  })
