...
describe("Slider", () => {
  ...
  it("sets aria-valuenow", () => {
    ...
    expect(
      container.querySelector('[role="slider"]')?.getAttribute("aria-valuenow"),
    ).toBe("50");
  });

  it("sets aria-valuemin", () => {
    const { container } = render(
      <Slider value={[50]} min={10} onValueChange={() => {}} />,
    );
    expect(
      container.querySelector('[role="slider"]')?.getAttribute("aria-valuemin"),
    ).toBe("10");
  });

  it("sets aria-valuemax", () => {
    const { container } = render(
      <Slider value={[50]} max={200} onValueChange={() => {}} />,
    );
    expect(
  ...
  it("renders two thumbs in range mode", () => {
    const { container } = render(
      <Slider value={[20, 80]} range onValueChange={() => {}} />,
    );
    expect(
