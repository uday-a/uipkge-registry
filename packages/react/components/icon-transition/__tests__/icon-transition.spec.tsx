...
const DefaultIcon = (props: { className?: string }) => (
  <svg className={`default-icon ${props.className ?? ''}`} data-testid="default-icon" />
)
const ActiveIcon = (props: { className?: string }) => (
  <svg className={`active-icon ${props.className ?? ''}`} data-testid="active-icon" />
)
...
describe('IconTransition', () => {
  ...
  it('renders with data-slot="icon-transition"', () => {
    const { container } = render(<IconTransition defaultIcon={DefaultIcon} activeIcon={ActiveIcon} />)
    expect(container.querySelector('[data-slot="icon-transition"]')).toBeTruthy()
  })
  ...
  it('renders as a button by default', () => {
    const { container } = render(<IconTransition defaultIcon={DefaultIcon} activeIcon={ActiveIcon} />)
    expect(container.querySelector('[data-slot="icon-transition"]')?.tagName.toLowerCase()).toBe('button')
  })
  ...
  it('renders the active icon when active prop is true', () => {
    const { container } = render(<IconTransition defaultIcon={DefaultIcon} activeIcon={ActiveIcon} active />)
    expect(container.querySelector('svg.active-icon')).toBeTruthy()
  })

  it('applies activeClass when active', () => {
    const { container } = render(
      <IconTransition defaultIcon={DefaultIcon} activeIcon={ActiveIcon} active activeClass="text-success" />,
    )
    expect(container.querySelector('[data-slot="icon-transition"]')?.className).toContain('text-success')
  })

  it('transitions to active icon on click (resetAfter=0 keeps active)', () => {
    const { container } = render(<IconTransition defaultIcon={DefaultIcon} activeIcon={ActiveIcon} resetAfter={0} />)
