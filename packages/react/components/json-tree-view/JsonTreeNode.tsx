...
function JsonTreeNode(props: JsonTreeNodeProps): React.ReactElement {
  ...
  const entries: [string | number, JsonValue][] = React.useMemo(() => {
    if (Array.isArray(data))
      return data.map((v, i) => [i, v] as [number, JsonValue]);
    if (data !== null && typeof data === "object")
      return Object.entries(data) as [string, JsonValue][];
    return [];
  }, [data]);
    ...
    >
      ...
      {isContainer && (
        ...
        >
          ...
          <span className={`${keyColor} select-none`}>
            {isRoot ? label : `"${label}"`}
          </span>
      ...
      {isContainer && open && (
        <div role="group">
          ...
          <div
            className="text-muted-foreground py-0.5 select-none"
            style={{ paddingLeft: `${indent}px` }}
          >
            {type === "array" ? "]" : "}"}
          </div>
      ...
      {!isContainer && (
        ...
        >
          ...
          <span className="text-muted-foreground">:</span>
          <span
            className={`${typeColor[type] ?? "text-foreground"} rounded text-left font-mono`}
          >
            {formatValue(data)}
          </span>
          <button
