...
const LeafletMapComponent = React.forwardRef<LeafletMapRef, LeafletMapProps>(
  (
    ...
    const applyTiles = React.useCallback((tiles: LeafletTilePreset) => {
      ...
      if (tiles.overlayUrl) {
        overlayLayerRef.current = L.tileLayer(tiles.overlayUrl, {
          maxZoom: tiles.maxZoom ?? 19,
        });
      ...
      >
        ...
        {cornerOrder.map((corner) => {
          ...
          const showFullscreen =
            mapReady && fullscreen && fsPosition === corner;
        ...
        {mapReady && (
          <LeafletMapContext.Provider value={mapRef.current}>
            {children}
...
function useLeafletLayer<T extends L.Layer>(
  build: (L: LeafletModule) => T,
): T | null {
...
const LeafletMarkerComponent = ({
  ...
  const overlays = kids.filter(
    (k) =>
      ...
      (k.type === LeafletPopupComponent || k.type === LeafletTooltipComponent),
  );
  ...
  React.useEffect(() => {
    ...
    loadLeaflet().then((L) => {
      ...
      m = L.marker(
        ...
        defined({
          icon: el
            ? L.divIcon({ className: "uipkge-leaflet-div-icon", html: el })
            : undefined,
          interactive: true,
          draggable,
          opacity,
...
const LeafletPolylineComponent = ({
  lngLatPath,
  onClick,
  children,
  ...opts
}: LeafletPolylineProps) => {
  const onClickRef = useLatest(onClick);
  const optsRef = useLatest(opts);
  const layer = useLeafletLayer<L.Polyline>((L) => {
    const l = L.polyline(toLatLngs(lngLatPath) as L.LatLngExpression[], {
      ...optsRef.current,
      interactive: true,
    });
...
const LeafletPolygonComponent = ({
  ...
  const layer = useLeafletLayer<L.Polygon>((L) => {
    const l = L.polygon(toLatLngs(lngLatPath) as L.LatLngExpression[], {
      ...optsRef.current,
      interactive: true,
    });
    l.on("click", (ev) => onClickRef.current?.(ev));
...
const LeafletCircleComponent = ({
  ...
  const layer = useLeafletLayer<L.Circle>((L) => {
    const l = L.circle(toLatLng(center), {
      ...optsRef.current,
      interactive: true,
      radius,
    });
...
const LeafletCircleMarkerComponent = ({
  center,
  radius,
  onClick,
  children,
  ...opts
}: LeafletCircleMarkerProps) => {
  const onClickRef = useLatest(onClick);
  const optsRef = useLatest(opts);
  const layer = useLeafletLayer<L.CircleMarker>((L) => {
    const l = L.circleMarker(toLatLng(center), {
      ...optsRef.current,
      interactive: true,
      radius,
    });
...
const LeafletTileLayerComponent = ({
  ...
  const layer = useLeafletLayer<L.TileLayer>((L) =>
    L.tileLayer(url, defined(optsRef.current)),
  );
  React.useEffect(() => {
