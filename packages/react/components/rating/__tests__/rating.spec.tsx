...
describe('Rating', () => {
  ...
  it('renders correct number of star buttons', () => {
    const { container } = render(<Rating value={3} max={5} onValueChange={() => {}} />)
  ...
  it('calls onValueChange when star clicked', () => {
    ...
    const { container } = render(<Rating value={0} onValueChange={onValueChange} />)
  ...
  it('shows value when showValue', () => {
    const { container } = render(<Rating value={3} showValue onValueChange={() => {}} />)
    expect(container.textContent).toContain('3')
  })

  it('disables buttons when disabled', () => {
    const { container } = render(<Rating value={3} disabled onValueChange={() => {}} />)
    const buttons = container.querySelectorAll('button')
    buttons.forEach((btn) => {
  ...
  it('sets aria-valuenow', () => {
    const { container } = render(<Rating value={3} onValueChange={() => {}} />)
    expect(container.querySelector('[data-slot="rating"]')?.getAttribute('aria-valuenow')).toBe('3')
  })

  it('sets aria-valuemax', () => {
    const { container } = render(<Rating value={3} max={5} onValueChange={() => {}} />)
    expect(container.querySelector('[data-slot="rating"]')?.getAttribute('aria-valuemax')).toBe('5')
  })

  it('clears value when clearable and same star clicked', () => {
    const onValueChange = vi.fn()
    const { container } = render(<Rating value={3} clearable onValueChange={onValueChange} />)
    const buttons = container.querySelectorAll('button')
    fireEvent.click(buttons[2])
  ...
  it('uses defaultValue when uncontrolled', () => {
    ...
    expect(container.querySelector('[data-slot="rating"]')?.getAttribute('aria-valuenow')).toBe('4')
  })

  it('does not call onValueChange when disabled and clicked', () => {
    const onValueChange = vi.fn()
    const { container } = render(<Rating value={3} disabled onValueChange={onValueChange} />)
    const buttons = container.querySelectorAll('button')
    fireEvent.click(buttons[2])
