...
describe('Chip', () => {
  ...
  it('applies variant classes', () => {
    ...
    expect(container.querySelector('[data-slot="chip"]')?.className).toContain('bg-success/10')
  })

  it('does not show close button when closable is false', () => {
    const { container } = render(<Chip />)
    expect(container.querySelector('button[aria-label="Remove item"]')).toBeFalsy()
  })

  it('shows close button when closable is true', () => {
    const { container } = render(<Chip closable />)
    expect(container.querySelector('button[aria-label="Remove item"]')).toBeTruthy()
  })

  it('calls onClose when close button is clicked', () => {
    const onClose = vi.fn()
    const { container } = render(<Chip closable onClose={onClose} />)
    fireEvent.click(container.querySelector('button[aria-label="Remove item"]')!)
    // onClose is called via setTimeout; verify data-leaving is set immediately
    expect(container.querySelector('[data-slot="chip"]')?.getAttribute('data-leaving')).toBeDefined()
  })
})
