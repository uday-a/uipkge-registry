...
describe('Input', () => {
  ...
  it('renders placeholder', () => {
    const { container } = render(<Input value="" placeholder="Enter text" onChange={() => {}} />)
    expect(container.querySelector('input')?.getAttribute('placeholder')).toBe('Enter text')
  })

  it('sets aria-invalid when status is error', () => {
    const { container } = render(<Input value="" status="error" onChange={() => {}} />)
    expect(container.querySelector('[data-slot="input"]')?.getAttribute('aria-invalid')).toBe('true')
  })

  it('disables input when disabled prop is true', () => {
    const { container } = render(<Input value="" disabled onChange={() => {}} />)
    expect(container.querySelector('input')?.hasAttribute('disabled')).toBe(true)
  })

  it('renders prefix text', () => {
    const { getByText } = render(<Input value="" prefix="$" onChange={() => {}} />)
    expect(getByText('$')).toBeTruthy()
  })

  it('renders suffix text', () => {
    const { getByText } = render(<Input value="" suffix="kg" onChange={() => {}} />)
    expect(getByText('kg')).toBeTruthy()
  })

  it('renders addonBefore', () => {
    const { getByText } = render(<Input value="" addonBefore="https://" onChange={() => {}} />)
    expect(getByText('https://')).toBeTruthy()
  })

  it('renders addonAfter', () => {
    const { getByText } = render(<Input value="" addonAfter=".com" onChange={() => {}} />)
  ...
  it('sets type on the input element', () => {
    const { container } = render(<Input value="" type="email" onChange={() => {}} />)
  ...
  it('toggles password visibility when showPasswordToggle is set', () => {
    const { container } = render(<Input value="secret" type="password" showPasswordToggle onChange={() => {}} />)
    const input = container.querySelector('input')!
  ...
  it('clears value when allowClear button is clicked', () => {
    const onChange = vi.fn()
    const { container } = render(<Input value="text" allowClear onChange={onChange} />)
    const input = container.querySelector('input')!
  ...
  it('shows count when showCount and maxLength are set', () => {
    const { container } = render(<Input value="abc" showCount maxLength={10} onChange={() => {}} />)
