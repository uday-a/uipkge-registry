...
const Textarea = React.forwardRef<HTMLTextAreaElement, TextareaProps>(
  (props, forwardedRef) => {
    ...
    const [internal, setInternal] = React.useState<string | number>(
      defaultValue ?? "",
    );
    ...
    const computedSuccessMessages = React.useMemo(() => {
      if (successMessages) {
        return Array.isArray(successMessages)
          ? successMessages
          : [successMessages];
      }
      if (success) {
    ...
    const showCountConfig = React.useMemo<{
      ...
      <div className={cn("relative space-y-2", className)}>
        ...
        <div id={descriptionId} className="mt-1.5">
          ...
          {hint && (!hasError || persistentHint) && !focused && (
            <p className={cn("text-muted-foreground text-sm", hintClassName)}>
              {hint}
            </p>
          )}
