...
describe('Skeleton', () => {
  ...
  it('renders as a div', () => {
    const { container } = render(<Skeleton />)
    expect(container.querySelector('[data-slot="skeleton"]')?.tagName.toLowerCase()).toBe('div')
  })
  ...
  it('applies width style when width prop is set', () => {
    ...
    expect(container.querySelector('[data-slot="skeleton"]')?.getAttribute('style')).toContain('200px')
  })

  it('applies height style when height prop is set', () => {
    const { container } = render(<Skeleton height="100px" />)
    expect(container.querySelector('[data-slot="skeleton"]')?.getAttribute('style')).toContain('100px')
  })

  it('renders children when loading is false', () => {
    const { container } = render(<Skeleton loading={false}>Loaded content</Skeleton>)
  ...
  it('SkeletonGroup renders with data-slot="skeleton-group"', () => {
    ...
    expect(container.querySelector('[data-slot="skeleton-group"]')).toBeTruthy()
  })

  it('SkeletonText renders multiple lines', () => {
    const { container } = render(<SkeletonText lines={4} />)
    expect(container.querySelector('[data-slot="skeleton-text"]')).toBeTruthy()
    expect(container.querySelectorAll('[data-slot="skeleton-text"] > div').length).toBe(4)
  })
})
