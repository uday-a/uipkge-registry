    ...
    return (
      <div className="flex flex-col gap-2">
        ...
        <div className="flex items-center gap-4">
          {/* Min value display */}
          <div
            className="text-muted-foreground min-w-[3rem] text-sm tabular-nums"
            aria-hidden="true"
          >
            {currentValue?.[0] ?? min}
          </div>
          ...
          <SliderPrimitive.Root
            ...
            defaultValue={
              isControlled ? undefined : (defaultValue ?? [min, max])
            }
            ...
            {showTicks && ticks.length > 0 && (
              ...
              >
                {ticks.map((tick) => (
                  <div
                    key={tick}
                    className="bg-muted-foreground/30 h-2 w-0.5 rounded-full"
                  />
                ))}
              </div>
            )}
        ...
        {showTicks && ticks.length > 0 && (
          <div
            className="text-muted-foreground flex justify-between px-1 text-xs"
            aria-hidden="true"
          >
            <span>{min}</span>
            <span>{max}</span>
