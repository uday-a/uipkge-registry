...
describe('RichTextEditor (React)', () => {
  ...
  it('renders editor content area and applies classes', () => {
    const { container } = render(<RichTextEditor value="<p>Initial content</p>" className="custom-editor" />)
  ...
  it('renders the incoming value as editable content', () => {
    const { container } = render(<RichTextEditor value="<p>Initial text</p>" />)
  ...
  it('exposes a labelled formatting toolbar', () => {
    ...
    for (const label of ['Bold', 'Italic', 'Underline', 'Bullet list', 'Link', 'Undo', 'Redo']) {
      expect(control(container, label), `missing control: ${label}`).toBeTruthy()
    }
  })
  ...
  it('closes the popover once a URL is submitted', () => {
    ...
    fireEvent.submit(document.querySelector('[data-slot="popover-content"] form')!)
  ...
  it('renders content updates pushed in through the value prop', () => {
    const { container, rerender } = render(<RichTextEditor value="<p>First</p>" />)
  ...
  it('applies the configured minimum height to the editing surface', () => {
    const { container } = render(<RichTextEditor value="<p>Hi</p>" minHeight="240px" />)
