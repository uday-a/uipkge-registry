...
describe('QrCode', () => {
  ...
  it('renders an img element for canvas type', async () => {
    const { container } = render(<QrCode value="https://example.com" type="canvas" />)
    await waitFor(() => {
  ...
  it('renders bordered container by default', () => {
    ...
    expect(container.querySelector('[data-slot="qr-code"]')?.className).toContain('border')
  })

  it('does not apply border when bordered=false', () => {
    const { container } = render(<QrCode value="test" bordered={false} />)
    expect(container.querySelector('[data-slot="qr-code"]')?.className).not.toContain('border')
  })
})
