...
describe("Highlight", () => {
  ...
  it("renders as a span", () => {
    const { container } = render(<Highlight text="hello" query="ell" />);
    expect(
      container.querySelector('[data-slot="highlight"]')?.tagName.toLowerCase(),
    ).toBe("span");
  });
  ...
  it('uses span tag when highlightTag is "span"', () => {
    const { container } = render(
      <Highlight text="hello world" query="world" highlightTag="span" />,
    );
    const match = container.querySelector('[data-slot="highlight-match"]');
  ...
  it("caps highlights at maxHighlights", () => {
    const { container } = render(
      <Highlight text="a a a" query="a" maxHighlights={2} />,
    );
    expect(
      container.querySelectorAll('[data-slot="highlight-match"]').length,
    ).toBe(2);
  });
});
