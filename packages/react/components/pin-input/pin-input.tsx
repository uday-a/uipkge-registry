...
const statusClassMap: Record<PinInputStatus, string> = {
  error:
    "border-destructive focus-within:border-destructive focus-within:ring-destructive/40 text-destructive",
  warning:
    "border-warning focus-within:border-warning focus-within:ring-warning/40 text-warning",
  success:
    "border-success focus-within:border-success focus-within:ring-success/40 text-success",
  default: "",
};
      ...
      <div
        ...
        onAnimationEnd={(event) => {
          ...
          if (
            ...
            event.animationName === "pin-slot-pop"
          ) {
            setIsPopping(false);
          }
        }}
          ...
          (effectiveMask ? (
            <span
              ...
              className={cn(
                ...
                popGen > 0 && "pin-char-pop",
              )}
            />
          ) : (
            <span
              key={`char-${char}-${popGen}`}
              className={cn(popGen > 0 && "pin-char-pop")}
            >
              {char}
            </span>
          ))}
