...
describe("Table", () => {
  ...
  it('TableHeader has data-slot="table-header"', () => {
    const { container } = renderTable();
    expect(
      container.querySelector('thead[data-slot="table-header"]'),
    ).toBeTruthy();
  });
  ...
  it('TableBody has data-slot="table-body"', () => {
    const { container } = renderTable();
    expect(
      container.querySelector('tbody[data-slot="table-body"]'),
    ).toBeTruthy();
  });

  it('TableRow has data-slot="table-row"', () => {
    const { container } = renderTable();
    expect(container.querySelectorAll('tr[data-slot="table-row"]').length).toBe(
      3,
    );
  });
  ...
  it("all sub-components have data-uipkge", () => {
    ...
    expect(
      container
        .querySelector('[data-slot="table-head"]')
        ?.hasAttribute("data-uipkge"),
    ).toBe(true);
    expect(
      container
        .querySelector('[data-slot="table-cell"]')
        ?.hasAttribute("data-uipkge"),
    ).toBe(true);
  });
});
