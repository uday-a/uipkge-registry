...
vi.mock("react-map-gl/mapbox", () => ({
  default: ({ children }: { children?: React.ReactNode }) => (
    <div className="mock-map">{children}</div>
  ),
...
describe("Map", () => {
  ...
  it("has data-uipkge on container", () => {
    const { container } = render(<Map accessToken="test-token" />);
    expect(
      container.querySelector('[data-slot="map"]')?.hasAttribute("data-uipkge"),
    ).toBe(true);
  });
  ...
  it("applies custom class", () => {
    const { container } = render(
      <Map accessToken="test-token" className="h-96" />,
    );
    expect(container.querySelector('[data-slot="map"]')?.className).toContain(
      "h-96",
    );
  });
  ...
  it("renders as a div", () => {
    const { container } = render(<Map accessToken="test-token" />);
    expect(
      container.querySelector('[data-slot="map"]')?.tagName.toLowerCase(),
    ).toBe("div");
  });
  ...
  it("renders a token-required placeholder when accessToken is empty", () => {
    ...
    expect(container.querySelector('[data-slot="map"]')?.textContent).toContain(
