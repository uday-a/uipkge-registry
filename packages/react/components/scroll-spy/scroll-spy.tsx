...
export type ScrollSpyVariant =
  ...
  | "tabs"
  | "pills";
export type ScrollSpyIndicatorMode =
  "segment" | "fill" | "progress" | "pill" | "dot" | "line";
...
export type ScrollSpyColor =
  "primary" | "foreground" | "destructive" | "secondary" | (string & {});
...
export function resolveScrollSpyColor(
  color: ScrollSpyColor = "primary",
): HandleColorResolved {
  if (color === "primary")
    return {
      bgClass: "bg-primary",
...
export interface ScrollSpyProps extends Omit<
...
export const ScrollSpyRoot = React.forwardRef<HTMLElement, ScrollSpyProps>(
  (
    ...
    const resolvedRailPosition: ScrollSpyRailPosition =
      railPosition ?? (position === "left" ? "right" : "left");
    ...
    const registeredItemsMap = React.useRef<Map<string, RegisteredItem>>(
      new Map(
        items && items.length > 0
          ? flattenItems(items).map((it) => [it.value, it])
          : [],
      ),
    );
    ...
    const getItems = React.useCallback(
      () => Array.from(registeredItemsMap.current.values()),
      [],
    );
    ...
    const recomputeActive = React.useCallback(() => {
      ...
      const triggerThreshold =
        containerTop + offsetTopRef.current + boundsRef.current + 40;

      let currentTarget = "";

      for (const item of currentItems) {
        const selector = item.value.startsWith("#")
          ? item.value
          : `#${item.value}`;
        const target = isWin
    ...
    const isItemParentActive = React.useCallback(
      (val: string) => {
        ...
        const activeItem = itemsList.find(
          (i) =>
            i.value === activeValue ||
        ...
        while (parent) {
          if (parent === val || parent.replace(/^#/, "") === cleanVal)
            return true;
    ...
    const isItemScrolled = React.useCallback(
      (val: string) => {
        ...
        const idx = itemsList.findIndex(
          (i) => i.value === val || i.value.replace(/^#/, "") === cleanVal,
        );
        ...
        <ScrollSpyItemDepthContext.Provider value={0}>
          <nav
            ...
            className={cn(
              ...
              affix &&
                (resolvedPosition === "bottom" ? "sticky bottom-4" : "sticky"),
              className,
            )}
            style={
              affix && resolvedPosition !== "bottom"
                ? { top: `${offsetTop}px`, ...style }
                : style
            }
            {...props}
          >
                            ...
                            <React.Fragment key={item.href}>
                              ...
                              {(item.children ?? []).map((child, childIdx) => {
                                const prevChildHadChildren = Boolean(
                                  childIdx > 0 &&
                                  item.children?.[childIdx - 1]?.children
                                    ?.length,
                                );
                                return (
                                  <React.Fragment key={child.href}>
                                    <ScrollSpyItem
                                      ...
                                      depth={child.depth ?? 2}
                                      className={
                                        childIdx === 0 || prevChildHadChildren
                                          ? "mt-2"
                                          : undefined
                                      }
                                    >
...
export const ScrollSpyTitle = React.forwardRef<
...
export const ScrollSpyList = React.forwardRef<
  HTMLUListElement,
  ScrollSpyListProps
>(({ className, children, ...props }, ref) => {
  const ctx = React.useContext(ScrollSpyContext);
  ...
  return (
    <ul
      ...
      style={{
        borderLeftWidth:
          isStraight && !isRightRail
            ? `${ctx?.resolvedLineWidth ?? 2.5}px`
            : undefined,
        borderRightWidth:
          isStraight && isRightRail
            ? `${ctx?.resolvedLineWidth ?? 2.5}px`
      ...
      className={cn(
        ...
        isStraight &&
          (isRightRail ? "border-border border-r" : "border-border border-l"),
        className,
      )}
      {...props}
    >
...
export const ScrollSpyIndicator = React.forwardRef<
  HTMLDivElement,
  ScrollSpyIndicatorProps
>(({ className, color, ...props }, ref) => {
  ...
  const [straightHighlight, setStraightHighlight] = React.useState({
    top: 0,
    height: 0,
    visible: false,
  });
  ...
  const updateGeometry = React.useCallback(() => {
    ...
    const activeIdx = markers.findIndex(
      (m) =>
        m.value === ctx.activeValue ||
        m.value.replace(/^#/, "") === ctx.activeValue.replace(/^#/, ""),
    );
    ...
    const isFillMode = ctx.indicator === "fill" || ctx.keepScrolled;
    const startLen = isFillMode
      ? 0
      : measureLength(buildCircuitPath(markers, activeIdx, rounded, "top"));
    const endLen = measureLength(
      buildCircuitPath(markers, activeIdx, rounded, "bottom"),
    );
  ...
  const isLeftWithRightRail =
    ctx.position === "left" && ctx.railPosition === "right";
  ...
  return (
    ...
    >
      ...
      {ctx.turn === "straight" &&
        ...
        (ctx.indicator !== "segment" || ctx.keepScrolled) && (
          <div
            ...
            style={{
              ...
              left: isLeftWithRightRail
                ? undefined
                : `-${ctx.resolvedLineWidth}px`,
              right: isLeftWithRightRail
                ? `-${ctx.resolvedLineWidth}px`
                : undefined,
              transition:
                canAnimate && ctx.indicator !== "progress"
...
export const ScrollSpyItem = React.forwardRef<
  HTMLLIElement,
  ScrollSpyItemProps
...
export const ScrollSpyStepper = React.forwardRef<
  HTMLDivElement,
  ScrollSpyStepperProps
>(({ className, ...props }, ref) => {
  const ctx = React.useContext(ScrollSpyContext);
  if (!ctx)
    throw new Error("ScrollSpyStepper must be used inside <ScrollSpy>.");
  ...
  const activeIndex = Math.max(
    ...
    items.findIndex(
      (i) =>
        i.value === activeValue ||
        i.value.replace(/^#/, "") === activeValue.replace(/^#/, ""),
    ),
  );
