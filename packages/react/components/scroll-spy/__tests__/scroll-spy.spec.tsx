...
describe('ScrollSpy (React)', () => {
  ...
  it('has data-uipkge on nav', () => {
    const { container } = render(<ScrollSpy />)
    expect(container.querySelector('[data-slot="scroll-spy"][data-uipkge]')).toBeTruthy()
  })
  ...
  it('renders href on scroll-spy links', () => {
    const { container } = render(<ScrollSpy items={[{ href: '#section-1', title: 'Section 1' }]} />)
    const link = container.querySelector('a[data-slot="scroll-spy-link"]') as HTMLAnchorElement
    expect(link.getAttribute('href')).toBe('#section-1')
  })

  it('renders title text on scroll-spy links', () => {
    const { container } = render(<ScrollSpy items={[{ href: '#sec', title: 'My Section' }]} />)
    expect(container.textContent).toContain('My Section')
  })

  it('renders title when title prop is provided', () => {
    const { container } = render(<ScrollSpy title="On this page" items={[{ href: '#sec', title: 'My Section' }]} />)
  ...
  it('renders angle circuit variant with SVG track', () => {
    ...
    const nav = container.querySelector('nav[data-slot="scroll-spy"]') as HTMLElement
  ...
  it('supports left side position with right-side rail', () => {
    ...
    const list = container.querySelector('ul[data-slot="scroll-spy-list"]') as HTMLElement
  ...
  it('renders top sticky stepper mode', () => {
    ...
    expect(container.querySelector('[data-slot="scroll-spy-stepper-top"]')).toBeTruthy()
  })
  ...
  it('renders top sticky scroll spy mode highlighting only active section', () => {
    ...
    expect(container.querySelector('[data-slot="scroll-spy-top"]')).toBeTruthy()
    const buttons = container.querySelectorAll('[data-slot="scroll-spy-top"] button')
  ...
  it('renders bottom floating stepper mode', () => {
    ...
    expect(container.querySelector('[data-slot="scroll-spy-stepper-bottom"]')).toBeTruthy()
  })
  ...
  it('supports compound component composition', () => {
    ...
    expect(container.querySelector('[data-slot="scroll-spy-title"]')?.textContent).toBe('On this page')
    expect(container.querySelectorAll('[data-slot="scroll-spy-link"]').length).toBe(2)
  })
  ...
  it('supports keepScrolled prop and sets data-keep-scrolled', () => {
    ...
    expect(container.querySelector('[data-slot="scroll-spy"]')?.getAttribute('data-keep-scrolled')).toBe('true')
  })
  ...
  it('supports highlightParent and marks parent link with data-parent-active', () => {
    ...
    const links = Array.from(container.querySelectorAll('[data-slot="scroll-spy-link"]'))
    const parentLink = links.find((l) => l.getAttribute('href') === '#parent-1')
  ...
  it('supports lineWidth prop and sets data-line-width', () => {
    ...
    expect(container.querySelector('[data-slot="scroll-spy"]')?.getAttribute('data-line-width')).toBe('3')
  })
  ...
  it('supports color prop, sets data-color, applies color only to handle and keeps link text neutral', () => {
    ...
    const activeLink = container.querySelector('[data-slot="scroll-spy-link"][data-active="true"]')
