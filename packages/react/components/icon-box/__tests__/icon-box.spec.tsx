...
const StubIcon = (props: { className?: string }) => <svg data-testid="stub-icon" className={props.className} />
...
describe('IconBox', () => {
  ...
  it('renders as a div container', () => {
    const { container } = render(<IconBox icon={StubIcon} />)
    expect(container.querySelector('[data-slot="icon-box"]')?.tagName.toLowerCase()).toBe('div')
  })

  it('renders the icon component inside', () => {
    const { container } = render(<IconBox icon={StubIcon} />)
    expect(container.querySelector('svg[data-testid="stub-icon"]')).toBeTruthy()
  })
  ...
  it('applies size classes for sm', () => {
    ...
    expect(container.querySelector('[data-slot="icon-box"]')?.className).toContain('p-1.5')
  })

  it('applies circle shape classes', () => {
    const { container } = render(<IconBox icon={StubIcon} shape="circle" />)
    expect(container.querySelector('[data-slot="icon-box"]')?.className).toContain('rounded-full')
  })
})
