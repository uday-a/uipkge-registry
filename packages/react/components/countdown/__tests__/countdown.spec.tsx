...
describe('Countdown (React)', () => {
  ...
  it('renders custom label and unit separator', () => {
    const { container } = render(<Countdown target={NOW + MINUTE} label="Sale Ends In" separator=":" />)
  ...
  it('splits the remaining time into days, hours, minutes and seconds', () => {
    const { container } = render(<Countdown target={NOW + 2 * DAY + 3 * HOUR + 4 * MINUTE + 5 * SECOND} />)
  ...
  it('rolls days into hours for HH:MM:SS and into minutes for MM:SS', () => {
    const hhmmss = render(<Countdown target={NOW + DAY + 2 * HOUR} format="HH:MM:SS" />)
    // 1 day + 2 hours reported as 26 hours, not "01" days and "02" hours.
    expect(hhmmss.container.textContent).toContain('26')
    cleanup()

    const mmss = render(<Countdown target={NOW + 2 * HOUR + MINUTE} format="MM:SS" />)
  ...
  it('honours pad=false so single digits are not zero-padded', () => {
    ...
    const bare = render(<Countdown target={NOW + 5 * SECOND} format="SS" pad={false} />)
  ...
  it('ticks down once per second and reports the remaining milliseconds', () => {
    const onTick = vi.fn()
    const { container } = render(<Countdown target={NOW + 10 * SECOND} format="SS" onTick={onTick} />)
  ...
  it('calls onFinish once when it reaches zero and then stops ticking', () => {
    ...
    const { container } = render(
      <Countdown target={NOW + 2 * SECOND} format="SS" onFinish={onFinish} onTick={onTick} />,
    )
  ...
  it('calls onFinish immediately for a target already in the past', () => {
    const onFinish = vi.fn()
    const { container } = render(<Countdown target={NOW - MINUTE} format="SS" onFinish={onFinish} />)
    expect(onFinish).toHaveBeenCalledTimes(1)
    expect(container.textContent).toContain('00')
  })
  ...
  it('stops while paused and resumes from the current time', () => {
    const onTick = vi.fn()
    const { container, rerender } = render(<Countdown target={NOW + 60 * SECOND} format="SS" paused onTick={onTick} />)
    ...
    rerender(<Countdown target={NOW + 60 * SECOND} format="SS" paused={false} onTick={onTick} />)
    act(() => {
  ...
  it('accepts a Date and an ISO string as well as epoch milliseconds', () => {
    const fromDate = render(<Countdown target={new Date(NOW + 30 * SECOND)} format="SS" />)
    expect(fromDate.container.textContent).toContain('30')
    cleanup()

    const fromIso = render(<Countdown target={new Date(NOW + 45 * SECOND).toISOString()} format="SS" />)
    expect(fromIso.container.textContent).toContain('45')
  })

  it('renders only the units named by the format', () => {
    const { container } = render(<Countdown target={NOW + DAY + HOUR} format="HH:MM" />)
    expect(container.querySelector('[data-slot="countdown-hours"]')).toBeTruthy()
    expect(container.querySelector('[data-slot="countdown-minutes"]')).toBeTruthy()
    expect(container.querySelector('[data-slot="countdown-days"]')).toBeNull()
    expect(container.querySelector('[data-slot="countdown-seconds"]')).toBeNull()
  })
