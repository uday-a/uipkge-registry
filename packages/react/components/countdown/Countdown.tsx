...
> {
  ...
  children?:
    ((props: CountdownRenderProps) => React.ReactNode) | React.ReactNode;
...
const Countdown = React.forwardRef<HTMLDivElement, CountdownProps>(
  (
    ...
    const pad2 = React.useCallback(
      (n: number) => (pad ? String(n).padStart(2, "0") : String(n)),
      [pad],
    );
    ...
    const display = React.useMemo(() => {
      ...
      if (f === "HH:MM:SS")
        return `${pad2(hours)}${sep}${pad2(minutes)}${sep}${pad2(seconds)}`;
          ...
          {f.includes("DD") &&
            (renderDays ? (
              ...
              <span
                ...
                className="text-foreground inline-flex text-2xl font-semibold"
              >
                <CountdownDigits value={displayParts.days} pad={pad} />
              </span>
            ))}
          {f.includes("DD") && f.includes("HH") && (
            <span className="text-muted-foreground text-2xl">{separator}</span>
          )}
          {f.includes("HH") &&
            (renderHours ? (
