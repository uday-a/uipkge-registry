...
describe('Fab', () => {
  ...
  it('applies data-position attribute', () => {
    ...
    expect(container.querySelector('[data-slot="fab"]')?.getAttribute('data-position')).toBe('top-left')
  })

  it('applies data-variant attribute', () => {
    const { container } = render(<Fab variant="destructive" />)
    expect(container.querySelector('[data-slot="fab"]')?.getAttribute('data-variant')).toBe('destructive')
  })
