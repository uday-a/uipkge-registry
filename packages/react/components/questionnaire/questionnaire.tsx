...
export interface QuestionnaireProps extends Omit<
...
function Questionnaire({
  ...
  const toggleChoice = (name: string, choice: string, multiple?: boolean) => {
    if (multiple) {
      const cur = Array.isArray(answers[name])
        ? [...(answers[name] as string[])]
        : [];
      setAnswer(
        name,
        cur.includes(choice)
          ? cur.filter((v) => v !== choice)
          : [...cur, choice],
      );
      return;
    }
  ...
  return (
    <form
      ...
      onKeyDown={(e) => {
        ...
        if (!shortcuts || e.metaKey || e.ctrlKey || e.altKey || !current)
          return;
              ...
              {current.choices.map((choice, index) => (
                ...
                >
                  <input
                    ...
                    onChange={() =>
                      toggleChoice(current.name, choice.value, current.multiple)
                    }
                  />
