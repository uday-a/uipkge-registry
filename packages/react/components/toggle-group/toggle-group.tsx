...
const ToggleGroup = React.forwardRef<React.ElementRef<typeof ToggleGroupPrimitive.Root>, ToggleGroupProps>(
  ({ className, variant, size, spacing = 0, animated = true, children, type, ...props }, ref) => {
    ...
    return (
      <ToggleGroupPrimitive.Root
        ref={setRefs}
        {...(type === 'multiple'
          ? ({ type: 'multiple', ...props } as ToggleGroupPrimitive.ToggleGroupMultipleProps)
          : ({ type: 'single', ...props } as ToggleGroupPrimitive.ToggleGroupSingleProps))}
        data-uipkge=""
