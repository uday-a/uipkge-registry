...
export const BarRaceChart = React.forwardRef<HTMLDivElement, BarRaceChartProps>(
  ({ frames, topN = 8, autoPlay = true, interval = 1400, height = 380, option, className, ariaLabel }, ref) => {
    const theme = useChartTheme()
    const [cursor, setCursor] = React.useState(0)

    React.useEffect(() => {
      if (!autoPlay || frames.length < 2) return
      const timer = setInterval(() => setCursor((c) => (c + 1) % frames.length), interval)
      return () => clearInterval(timer)
    }, [autoPlay, frames.length, interval])

    const mergedOption = React.useMemo(() => {
      const frame = frames[Math.min(cursor, frames.length - 1)] ?? { label: '', values: [] }
