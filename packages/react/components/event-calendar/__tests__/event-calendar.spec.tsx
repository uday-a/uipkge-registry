...
describe("EventCalendar (React)", () => {
  it('renders root container with data-slot="event-calendar"', () => {
    const { container } = render(
      <EventCalendar value="2026-05-18" events={sampleEvents} />,
    );
    expect(
      container.querySelector('[data-slot="event-calendar"]'),
    ).toBeTruthy();
  });

  it("renders month view by default with day cells and events", () => {
    render(
      <EventCalendar value="2026-05-18" events={sampleEvents} view="month" />,
    );
  ...
  it("switches to week view and renders all-day header and events", () => {
    render(
      <EventCalendar value="2026-05-18" events={sampleEvents} view="week" />,
    );
  ...
  it("switches to day view and renders hourly schedule", () => {
    render(
      <EventCalendar value="2026-05-18" events={sampleEvents} view="day" />,
    );
    expect(screen.getByText("Sprint Planning")).toBeTruthy();
  ...
  it("calculates overlapping timed event positions into split columns", () => {
    ...
    const positions = calculateTimedEventPositions(
      sampleEvents,
      date,
      0,
      24,
