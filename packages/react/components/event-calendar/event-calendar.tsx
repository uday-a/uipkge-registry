...
export function EventCalendar({
  ...
  const [internalDate, setInternalDate] = React.useState<Date>(() =>
    parseDate(value ?? defaultValue ?? new Date()),
  );
  ...
  React.useEffect(() => {
    ...
    const update = () => {
      setNowPosition(
        getCurrentTimePosition(firstInterval, intervalCount, intervalMinutes),
      );
    };
    update();
  ...
  const monthDays = React.useMemo(
    () => getMonthDays(activeDate, weekStartsOn),
  ...
  const intervals = React.useMemo(() => {
    ...
    for (let i = 0; i < intervalCount; i++) {
      const hour =
        (firstInterval + Math.floor((i * intervalMinutes) / 60)) % 24;
  ...
  const weekDays = React.useMemo(
    () => getWeekDays(activeDate, weekStartsOn),
    [activeDate, weekStartsOn],
  );
  const workWeekDays = React.useMemo(
    () => getWorkWeekDays(activeDate),
    [activeDate],
  );
        ...
        <div className="flex flex-1 flex-col">
          ...
          <div className="border-border bg-muted/20 text-muted-foreground grid grid-cols-7 border-b text-center text-xs font-medium">
            {weekdayHeaderLabels.map((dayName, idx) => (
              <div
                key={idx}
                className="border-border/40 border-r py-2 last:border-r-0"
              >
                {dayName}
              </div>
            ))}
                    ...
                    <>
                      <span className="text-muted-foreground text-[11px] font-medium tracking-wider uppercase">
                        {d.toLocaleDateString("en-US", {
                          weekday: activeView === "day" ? "long" : "short",
                        })}
                      </span>
                      <span
                        className={cn(
            ...
            >
              {(activeView === "week"
                ...
                return (
                  <div
                    key={formatDateKey(d)}
                    className="flex min-h-[32px] flex-col gap-1 p-1"
                  >
                    {renderAllDay
                      ? renderAllDay({
                          date: d,
                        ...
                        >
                          {renderEvent ? (
                            ...
                            <div
                              ...
                              onClick={(e) =>
                                handleEventCardClick(item.event, e)
                              }
                            >
