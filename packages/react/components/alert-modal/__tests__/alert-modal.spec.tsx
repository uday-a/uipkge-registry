...
describe('AlertModal', () => {
  it('renders trigger when provided', () => {
    const { container } = render(
      <AlertModal trigger={<button data-testid="trigger-btn">Open</button>} onOpenChange={() => {}} />,
    )
  ...
  it('renders description text', () => {
    render(<AlertModal open description="This is permanent." onOpenChange={() => {}} />)
  ...
  it('hides cancel button when cancelLabel is null', () => {
    const { container } = render(<AlertModal open cancelLabel={null} onOpenChange={() => {}} />)
    const buttons = document.querySelectorAll('button')
    const texts = Array.from(buttons).map((b) => b.textContent?.trim())
  ...
  it('calls onAction when action button is clicked', () => {
    ...
    const buttons = document.querySelectorAll('button')
    const actionBtn = Array.from(buttons).find((b) => b.textContent?.trim() === 'Continue')
    if (actionBtn) {
