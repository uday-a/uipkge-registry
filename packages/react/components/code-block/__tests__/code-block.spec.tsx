...
describe('CodeBlock', () => {
  ...
  it('keeps code visible when the hidden header removes the disclosure control', () => {
    const { container } = render(<CodeBlock code="x = 1" showHeader={false} defaultExpanded={false} />)
    expect(container.querySelector('code')).toBeVisible()
  })

  it('exposes disclosure state and the controlled code region', () => {
    const { getByRole } = render(<CodeBlock code="x = 1" language="typescript" />)
    ...
    expect(getByRole('button', { name: 'Show code' })).toHaveAttribute('aria-expanded', 'false')
  ...
  it('syntax-highlights recognized languages', async () => {
    ...
    await vi.waitFor(() => expect(container.querySelectorAll('[data-syntax-token]').length).toBeGreaterThan(1))

    expect(container.querySelector('[data-syntax-token]')?.getAttribute('style')).toContain('--shiki-light')
  })

  it('keeps block elements outside the pre element', () => {
    const { container } = render(<CodeBlock code="x = 1" />)
    const childTags = Array.from(container.querySelector('pre')!.children).map((child) => child.tagName)
  ...
  it('shows an announced error when clipboard access fails', async () => {
    ...
    const failedButton = await vi.waitFor(() => getByRole('button', { name: 'Copy failed' }))
