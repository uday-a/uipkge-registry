...
describe('Spinner', () => {
  ...
  it('has aria-label="Loading"', () => {
    const { container } = render(<Spinner />)
    expect(container.querySelector('[data-slot="spinner"]')?.getAttribute('aria-label')).toBe('Loading')
  })

  it('applies size classes', () => {
    const { container } = render(<Spinner size="lg" />)
    // SVG elements expose className as SVGAnimatedString; use getAttribute instead.
    expect(container.querySelector('[data-slot="spinner"]')?.getAttribute('class')).toContain('size-8')
  })
})
