...
describe('Anchor', () => {
  ...
  it('has data-uipkge on nav', () => {
    const { container } = render(<Anchor />)
    expect(container.querySelector('[data-slot="anchor"][data-uipkge]')).toBeTruthy()
  })
  ...
  it('renders href on anchor links', () => {
    const { container } = render(<Anchor items={[{ href: '#section-1', title: 'Section 1' }]} />)
    const link = container.querySelector('[data-slot="anchor-link"] a') as HTMLAnchorElement
    expect(link.getAttribute('href')).toBe('#section-1')
  })

  it('renders title text on anchor links', () => {
    const { container } = render(<Anchor items={[{ href: '#sec', title: 'My Section' }]} />)
