...
export function extractCvaVariants(source: string): PropMeta[] {
  ...
  if (dIdx >= 0) {
    ...
    for (const m of dBlock.matchAll(/(\w+)\s*:\s*['"]([^'"]+)['"]/g)) defaults[m[1]] = m[2]
  }
  ...
  while ((km = keyRe.exec(block)) !== null) {
    ...
    const values = [...inner.matchAll(/(?:^|,)\s*(?:['"]?([\w-]+)['"]?)\s*:/g)].map((m) => m[1]).filter(Boolean)
    out.push({
      name: key,
...
function parseInterfaceBody(body: string): PropMeta[] {
  ...
  for (let raw of lines) {
    const line = raw.trim()
    if (!line) continue
    const docM = line.match(/^\/\*\*?\s*(.*?)\s*\*?\*\/$/) || line.match(/^\/\/\s*(.*)$/)
    if (docM) {
      doc = docM[1]
      continue
    }
    if (line.startsWith('*')) {
      doc = (doc ? doc + ' ' : '') + line.replace(/^\*\/?\s?/, '').replace(/\s*\*\/$/, '')
      continue
    }
    const m = line.match(/^(?:readonly\s+)?([A-Za-z_$][\w$]*)(\?)?\s*:\s*(.+?);?$/)
    if (m) {
      out.push({ name: m[1], type: m[3].replace(/;$/, '').trim(), required: !m[2], default: '', doc: doc.trim() })
      doc = ''
    } else {
      doc = ''
    }
...
export function extractReactProps(files: File[]): PropMeta[] {
  const main = files.find((f) => /\.tsx$/.test(f.path) && !/\.variants\./.test(f.path))
  ...
  if (main) {
    const im = main.content.match(/interface\s+\w*Props\b[^{]*\{([\s\S]*?)\n\}/)
    if (im) {
    ...
    const dm = main.content.match(/\(\s*\{([\s\S]*?)\}\s*(?:,\s*ref)?\s*\)\s*=>/)
    if (dm) {
      for (const d of dm[1].matchAll(/(\w+)\s*=\s*('[^']*'|"[^"]*"|true|false|\d+)/g)) {
 ...
 */
export function mergeVueCvaProps(fields: PropMeta[], cva: PropMeta[]): PropMeta[] {
  ...
  const merged = fields.map((f) => {
    ...
    const boolToggle = (v?.values ?? []).every((x) => x === 'true' || x === 'false')
    if (!v || f.values?.length || /^boolean\b/.test(f.type) || boolToggle) return f
    return { ...f, type: v.type, values: v.values, default: f.default || v.default }
  })
