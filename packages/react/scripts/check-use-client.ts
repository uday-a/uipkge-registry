...
export function findClientOnlyMarkers(source: string): string[] {
  ...
  for (const hook of CLIENT_HOOKS) {
    ...
    if (new RegExp(`(^|[^\\w$])(React\\.)?${hook}\\s*[(<]`).test(cleaned)) reasons.push(`${hook}()`)
  }
  if (/(^|[^\w$])(React\.)?createContext\s*[(<]/.test(cleaned)) reasons.push('createContext()')
  return reasons
}
...
export async function checkUseClient(root: string = DEFAULT_ROOT): Promise<UseClientViolation[]> {
  const files = await fg(['components/**/*.{ts,tsx}', 'blocks/**/*.{ts,tsx}', 'bootstrap/**/*.{ts,tsx}'], {
    cwd: root,
    absolute: true,
  ...
  for (const file of files) {
    ...
    if (reasons.length) violations.push({ file: relative(root, file), reasons })
  }
  return violations
}

export function reportUseClient(violations: UseClientViolation[], checked: number = lastScanned) {
  if (!violations.length) {
    console.log(`[check-use-client] ${checked} source file(s) checked -- every client-only file declares 'use client'.`)
    return
  }
  ...
  console.error(`  import these modules from Server Components and their build fails without it.\n`)
}
