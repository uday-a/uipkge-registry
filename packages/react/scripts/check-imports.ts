...
const VUE_LEAKS = ['vue', 'reka-ui', 'lucide-vue-next', 'vue-sonner', '@vueuse/']
...
function findUnimportedHelpers(source: string): string[] {
  ...
  const covered = new Set([...extractNamedImports(source), ...extractLocalDefs(source)])
...
function findVueLeaks(source: string): string[] {
  ...
  while ((m = re.exec(source)) !== null) {
    ...
    else if (VUE_LEAKS.some((v) => spec === v || spec.startsWith(v))) leaks.push(`'${spec}'`)
  }
  return leaks
}

async function main() {
  const files = await fg(['components/**/*.{ts,tsx}', 'blocks/**/*.{ts,tsx}', 'bootstrap/**/*.ts'], {
    cwd: ROOT,
    absolute: true,
  ...
  console.log(`[check-imports] ${files.length} source file(s) checked -- imports are React-clean.`)
}
