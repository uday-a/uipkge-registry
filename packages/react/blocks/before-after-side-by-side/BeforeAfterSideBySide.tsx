                ...
                <ul className="flex-1 space-y-2 p-4">
                  {panel.rows.map((row) => (
                    <li key={row.text} className="flex items-start gap-2">
                      {row.note ? (
                        <span className="border-border text-muted-foreground mt-px flex size-4 shrink-0 items-center justify-center rounded-full border font-mono text-[10px]">
                          {row.note}
                        </span>
                      ) : (
        ...
        <ol className="space-y-3">
          {annotations.map((annotation) => (
            <li key={annotation.id} className="flex items-start gap-3 text-sm">
              <span className="border-border text-muted-foreground mt-0.5 flex size-5 shrink-0 items-center justify-center rounded-full border font-mono text-[10px]">
                {annotation.id}
              </span>
