...
describe('CloudBackupSchedule React features', () => {
  it('renders quota statistics and AES-256 badge', () => {
    ...
    expect(screen.getByText(/45 GB consumed/i)).toBeDefined()
    expect(screen.getByText(/100 GB allocated/i)).toBeDefined()
    expect(screen.getByText(/60 days \(rolling\)/i)).toBeDefined()
    expect(screen.getByText(/AES-256/i)).toBeDefined()
    expect(screen.getAllByText(/Daily Scheduled Snapshot/i).length).toBeGreaterThanOrEqual(1)
  })
