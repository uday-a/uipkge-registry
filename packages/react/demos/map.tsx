...
import { Map, MapMarker, MapPopup, MapSource, MapLayer, type MapVariant } from '@react-registry/map'
import {
  Navigation,
  ChevronDown,
  ChevronUp,
...
export default function MapDemos() {
  ...
  const [zoomNavPosition, setZoomNavPosition] = useState<'top-left' | 'top-right' | 'bottom-left' | 'bottom-right'>(
    'top-right',
  )
  ...
  const activeScopePreset = scopeOptions.find((s) => s.id === activeScope) ?? scopeOptions[0]
  ...
  return (
    <>
      ...
      >
        <Map accessToken={token} center={[-74.006, 40.713]} zoom={11} className="h-80 w-full rounded-lg border">
      ...
      >
        <Map accessToken={token} center={[-0.128, 51.507]} zoom={12} className="h-80 w-full rounded-lg border">
          <MapMarker longitude={-0.128} latitude={51.507} anchor="bottom" onClick={() => setPopupOpen((open) => !open)}>
            <button
              type="button"
          ...
          {popupOpen && (
            ...
            >
              <div className="space-y-0.5 px-1 py-0.5">
                ...
                <p className="text-muted-foreground text-xs">King&apos;s Cross · open 08:00–18:00</p>
              </div>
            </MapPopup>
          )}
        </Map>
      ...
      >
        ...
        >
          ...
          <MapMarker longitude={deliveryOrigin[0]} latitude={deliveryOrigin[1]} anchor="bottom">
            <span className="bg-primary ring-background size-2.5 rounded-full ring-2" />
          </MapMarker>
          <MapMarker longitude={deliveryDest[0]} latitude={deliveryDest[1]} anchor="bottom">
        ...
        <div className="space-y-3">
          ...
          <div className="bg-muted/60 border-border/80 flex flex-col gap-2.5 rounded-lg border p-2.5">
            ...
            <div className="border-border/60 flex flex-wrap items-center justify-between gap-2 border-b pb-2">
              <span className="text-muted-foreground text-xs font-semibold tracking-wider uppercase">View Scope:</span>
          ...
          <div className="relative overflow-hidden rounded-lg border shadow-xs">
            ...
            >
              ...
              {activeScope !== 'metro' ? (
                worldHubs.map((hub) => (
                  <MapMarker key={hub.name} longitude={hub.coords[0]} latitude={hub.coords[1]} anchor="center">
                    <div className="group relative flex cursor-pointer flex-col items-center">
                      ...
                      <div className="bg-background/95 border-border pointer-events-none z-20 mt-1.5 hidden flex-col items-center rounded border px-2 py-0.5 font-mono text-xs shadow-md backdrop-blur-sm group-hover:flex">
                        <span className="text-foreground font-bold">{hub.name}</span>
      ...
      >
        <div className="relative overflow-hidden rounded-lg border shadow-xs">
          ...
          <div className="bg-background/90 border-border/80 pointer-events-auto absolute top-3 left-3 z-10 flex max-w-sm flex-col gap-2 rounded-lg border p-3 shadow-md backdrop-blur-md sm:max-w-md">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-xs font-semibold">Global Cloud Topology</span>
              </div>
            ...
            <p className="text-muted-foreground text-xs leading-relaxed">
              Transoceanic subsea fiber networks with live region ping telemetry. Click any region pill below to
              navigate.
            </p>
          ...
          >
            {cloudRegions.map((r) => (
              <MapMarker key={r.id} longitude={r.coords[0]} latitude={r.coords[1]} anchor="center">
                <div
                  ...
                  <div className="relative flex items-center justify-center">
                    ...
                    <span
                      className={`size-2.5 rounded-full border border-white shadow-sm ${
                        selectedRegionId === r.id ? 'bg-sky-400 ring-4 ring-sky-400/50' : 'bg-emerald-500'
                      }`}
                    />
                  </div>
        ...
        <div className="border-border relative overflow-hidden rounded-lg border shadow-xs">
          ...
          <div className="border-border bg-muted/40 space-y-2 border-b p-3 text-xs">
            <div className="flex flex-wrap items-center justify-between gap-2">
              ...
              <div className="flex items-center gap-1.5">
                <span className="text-muted-foreground font-medium">Zoom Nav Pos:</span>
              ...
              <div className="flex items-center gap-1.5">
                <span className="text-muted-foreground font-medium">Fullscreen Pos:</span>
                <div className="border-border bg-background inline-flex rounded-md border p-0.5">
                  {(['top-left', 'top-right', 'bottom-left', 'bottom-right'] as const).map((pos) => (
                    <button
                      key={pos}
              ...
              <div className="flex items-center gap-3">
                ...
                <label className="flex cursor-pointer items-center gap-1.5 select-none">
                  <input
                    ...
                    className="border-border accent-primary size-3.5 rounded"
                  />
                  <span className="text-muted-foreground">Show +/- Buttons</span>
                </label>
                <label className="flex cursor-pointer items-center gap-1.5 select-none">
                  <input
                    type="checkbox"
      ...
      >
        <div className="relative overflow-hidden rounded-lg border shadow-xs">
          <div className="bg-background/90 border-border/80 absolute top-3 left-3 z-10 flex items-center gap-2 rounded-md border px-2.5 py-1.5 font-mono text-xs shadow-xs backdrop-blur-md">
            ...
            <span className="text-foreground font-bold">3D Extruded Urban Footprints</span>
      ...
      >
        <div className="relative overflow-hidden rounded-lg border shadow-xs">
          <div className="bg-background/90 border-border/80 absolute top-3 left-3 z-10 flex items-center gap-2 rounded-md border px-2.5 py-1.5 font-mono text-xs shadow-xs backdrop-blur-md">
            ...
            <span className="text-foreground font-bold">Satellite Streets (Hybrid)</span>
      ...
      >
        <div className="relative overflow-hidden rounded-lg border shadow-xs">
          <div className="bg-background/90 border-border/80 absolute top-3 left-3 z-10 flex items-center gap-2 rounded-md border px-2.5 py-1.5 font-mono text-xs shadow-xs backdrop-blur-md">
            ...
            <span className="text-foreground font-bold">Topographic Outdoors</span>
      ...
      >
        <div className="relative overflow-hidden rounded-lg border shadow-xs">
          <div className="bg-background/90 border-border/80 absolute top-3 left-3 z-10 flex items-center gap-2 rounded-md border px-2.5 py-1.5 font-mono text-xs shadow-xs backdrop-blur-md">
            ...
            <span className="text-foreground font-bold">Navigation Night HUD</span>
      ...
      >
        <div className="relative overflow-hidden rounded-lg border shadow-xs">
          ...
          <div className="bg-background/90 border-border/80 pointer-events-auto absolute top-3 left-3 z-10 w-72 rounded-lg border p-3 shadow-md backdrop-blur-md transition-all duration-200">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-xs font-semibold">Global Air Corridors</span>
              </div>
            ...
            <div className="mt-2 flex items-center justify-between font-mono text-xs">
              <span className="text-muted-foreground">
                Active Routes: <strong className="text-foreground font-semibold">4 Arcs</strong>
              </span>
              <span className="text-muted-foreground">
                Hubs: <strong className="text-foreground font-semibold">6 Global</strong>
              </span>
            </div>
            ...
            {expandGlobeInfo && (
              <div className="border-border/50 mt-3 space-y-1.5 border-t pt-2.5 font-mono text-xs">
                <div className="flex justify-between">
                  <span className="text-muted-foreground font-sans">Projection:</span>
                  <span className="font-semibold">Spherical Earth</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground font-sans">Atmosphere:</span>
                  <span className="font-semibold text-emerald-500">Active Fog Glow</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground font-sans">Primary Hubs:</span>
            ...
            >
              ...
              {expandGlobeInfo ? <ChevronUp className="size-3" /> : <ChevronDown className="size-3" />}
            </button>
          </div>
          ...
          >
            {globeNodes.map((node) => (
              <MapMarker key={node.name} longitude={node.coords[0]} latitude={node.coords[1]} anchor="center">
          ...
          <div className="bg-background/90 border-border/80 pointer-events-auto absolute top-3 left-3 z-10 w-72 rounded-lg border p-3 shadow-md backdrop-blur-md transition-all duration-200">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-xs font-semibold">Facility Clustering</span>
              </div>
            ...
            <p className="text-muted-foreground mt-1 text-xs">Click a cluster circle to expand regional depots</p>

            {expandClusterInfo && (
              <div className="border-border/50 mt-3 space-y-1.5 border-t pt-2.5 font-mono text-xs">
                <div className="flex justify-between">
                  <span className="text-muted-foreground font-sans">West Coast:</span>
                  <span className="font-semibold">13 Facilities</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground font-sans">Midwest / Central:</span>
                  <span className="font-semibold">16 Facilities</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground font-sans">East Coast:</span>
                  <span className="font-semibold">16 Facilities</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground font-sans">Cluster Radius:</span>
            ...
            >
              ...
              {expandClusterInfo ? <ChevronUp className="size-3" /> : <ChevronDown className="size-3" />}
            </button>
          </div>
