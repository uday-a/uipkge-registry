...
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@react-registry/select'
import { Textarea } from '@react-registry/textarea'
import { PinInput, PinInputGroup, PinInputSlot } from '@react-registry/pin-input'
import { Loader2, CheckCircle2, KeyRound, Smartphone, Mail, AlertCircle } from 'lucide-react'
...
function LoginForm() {
  ...
  async function onSubmit(value: LoginValues) {
    ...
    if (value.email === 'error@demo.com') setLoginError('Invalid email or password')
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="max-w-sm space-y-4">
        ...
        {loginSuccess && <p className="text-sm text-emerald-600">Login successful! Redirecting...</p>}
  ...
  if (success) {
    return (
      <div className="flex max-w-sm flex-col items-center gap-3 rounded-lg border p-6 text-center">
        ...
        <p className="text-muted-foreground text-sm">Check your email to verify your account.</p>
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>Password</FormLabel>
              <FormControl>
                <Input type="password" placeholder="Min 8 characters" showPasswordToggle {...field} />
              </FormControl>
              <FormDescription>Must contain at least 8 characters, one number and one symbol.</FormDescription>
              <FormMessage />
            </FormItem>
          )}
        />
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Input type="password" placeholder="Repeat password" showPasswordToggle {...field} />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
...
function Signup2Form() {
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(() => {})} className="max-w-2xl space-y-4">
...
function ProfileForm() {
  ...
  const form = useForm<ProfileValues>({
    ...
    defaultValues: { displayName: '', email: '', phone: '', bio: '', country: '' },
  })
...
const resetRequestSchema = z.object({ email: z.string().email('Enter a valid email') })
    ...
    return (
      <div className="max-w-sm space-y-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1 mb-4 text-sm">Enter your email and we'll send you a reset link.</p>
        </div>
        <Form {...requestForm}>
          <form onSubmit={requestForm.handleSubmit(onRequest)} className="space-y-3">
            ...
            <Button type="submit" className="w-full" disabled={requestForm.formState.isSubmitting}>
              {requestForm.formState.isSubmitting && <Loader2 className="mr-2 size-4 animate-spin" />}
  ...
  return (
    <div className="max-w-sm space-y-4">
      <div>
        ...
        <p className="text-muted-foreground mt-1 mb-4 text-sm">Enter your new password below.</p>
      </div>
      <Form {...confirmForm}>
        <form onSubmit={confirmForm.handleSubmit(onConfirm)} className="space-y-3">
          ...
          <Button type="submit" className="w-full" disabled={confirmForm.formState.isSubmitting}>
            {confirmForm.formState.isSubmitting && <Loader2 className="mr-2 size-4 animate-spin" />}
  ...
  return (
    <div className="max-w-sm space-y-4">
      <div className="text-center">
        ...
        <p className="text-muted-foreground mt-1 text-sm">Enter the 6-digit code sent to you@example.com</p>
      </div>
      ...
      {error && <p className="text-destructive text-sm">Invalid code. Try again.</p>}
      {success && <p className="text-sm text-emerald-600">Verified successfully!</p>}
      ...
      {submitting && <p className="text-muted-foreground text-center text-xs">Verifying…</p>}
    </div>
  )
}
...
function Signup3Form() {
  const form = useForm<{ fn: string; ln: string; role: string; email: string; phone: string; dept: string }>({
    defaultValues: { fn: '', ln: '', role: '', email: '', phone: '', dept: '' },
  })
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(() => {})} className="max-w-3xl space-y-4">
    ...
    <div className="max-w-sm space-y-4">
      ...
      {!success ? (
        <div className="flex items-center gap-3">
          <PinInput value={code} onChange={setCode} maxLength={6} onComplete={onSubmit}>
            <PinInputGroup>
              {Array.from({ length: 6 }, (_, i) => (
                <PinInputSlot key={i} index={i} />
              ))}
          ...
          <Button size="sm" disabled={submitting || code.length < 6} onClick={onSubmit}>
...
function Profile2Form() {
  const form = useForm<{
    ...
    defaultValues: { firstName: '', lastName: '', email: '', phone: '', jobTitle: '', dept: '', bio: '' },
  })
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(() => {})} className="max-w-2xl space-y-4">
...
function Profile3Form() {
  const form = useForm<{
    ...
    defaultValues: { fn: '', ln: '', username: '', email: '', phone: '', website: '', city: '', country: '', zip: '' },
  })
  const text = (name: keyof ReturnType<typeof form.getValues> & string, label: string, type = 'text') => (
    <FormField
      control={form.control}
      name={name}
      render={({ field }) => (
...
function FormHelpersDemo() {
  const [validationStatus, setValidationStatus] = useState<'error' | 'warning' | 'success' | undefined>()
  ...
  return (
    <Form {...form}>
      <form className="max-w-lg space-y-6">
        <FormSection title="Account Details" description="Enter your account information below." divider>
          <FormField
            control={form.control}
            name="helperEmail"
            render={({ field }) => (
          ...
          <FormField
            ...
            name="helperUsername"
            render={({ field }) => (
              <FormItem label="Username" required status={validationStatus} help={validationMessage}>
                <FormControl>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Button type="button" size="sm" variant="outline" onClick={() => setValidationDemo('success')}>
              <CheckCircle2 className="mr-1 size-3.5 text-emerald-500" /> Success
            </Button>
          </div>
        </FormSection>
...
function HorizontalLayoutDemo() {
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(() => {})} className="max-w-lg space-y-4">
        <FormField
          control={form.control}
          name="hName"
          render={({ field }) => (
