...
import { Input, InputGroup, InputGroupAddon, InputGroupButton } from '@react-registry/input'
...
export default function InputDemo() {
  ...
  return (
    <>
      <Story title="Default" description="Two-way bound with value/onChange. Plain text input with label.">
      ...
      <Story title="Status" description="Error and warning visual states via the status prop.">
        <div className="grid max-w-sm gap-3">
          ...
          <Input placeholder="Error with value" status="error" defaultValue="invalid" />
        </div>
      </Story>

      <Story title="Prefix & Suffix" description="String or node content rendered inside the input wrapper.">
        <div className="grid max-w-sm gap-3">
          <Input placeholder="Username" prefix="@" />
          <Input placeholder="0.00" suffix="USD" />
          <Input placeholder="Search..." prefix={<Search className="size-4" />} />
          <Input placeholder="you@example.com" suffix={<Mail className="size-4" />} />
        </div>
      </Story>

      <Story title="Addon before & after" description="Input group styling with addon segments.">
        <div className="grid max-w-sm gap-3">
          <Input placeholder="website" addonBefore="https://" addonAfter=".com" />
          <Input
            placeholder="0.00"
      ...
      <Story title="Allow clear" description="Shows an X icon when the input has value and is focused or hovered.">
        <div className="grid max-w-sm gap-3">
          <Input placeholder="Type something..." defaultValue="Clear me" allowClear />
        </div>
      </Story>
      ...
      <Story title="Password toggle" description="Eye icon to toggle password visibility.">
        <div className="grid max-w-sm gap-3">
          <Input type="password" placeholder="Password" defaultValue="secret123" showPasswordToggle />
          <Input
            type="password"
