    ...
    <div className="space-y-8">
      ...
      >
        <div className="flex flex-wrap gap-2">
          <Button onClick={simulatePageLoad}>
            <Loader2 className="mr-2 size-4 animate-spin" style={{ opacity: pageBar.loading ? 1 : 0 }} />
            Load dashboard
          </Button>
      ...
      >
        <Card className="max-w-md">
          <CardHeader>
            ...
            <CardDescription>Updates are saved to your account instantly.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <Button className="w-full" disabled={formStatus === 'saving'} onClick={submitForm}>
              <Save className="mr-2 size-4" />
              {formStatus === 'saving' ? 'Saving…' : formStatus === 'done' ? 'Saved!' : 'Save changes'}
            </Button>
            {formStatus === 'done' && <p className="text-xs text-emerald-600">Your billing details were updated.</p>}
          </CardContent>
        </Card>
      </Story>
      ...
      >
        <div className="max-w-md space-y-3">
          <LoadingBar value={manualValue} height={4} onValueChange={setManualValue} />
          <input
            value={manualValue}
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <span className="text-muted-foreground text-xs whitespace-nowrap">Fetching results…</span>
        </div>
      </Story>
      ...
      >
        <Button variant="outline" onClick={runBottomBar}>
          <Loader2 className="mr-2 size-4 animate-spin" style={{ opacity: bottomBar.loading ? 1 : 0 }} />
          Sync in background
        </Button>
        <p className="text-muted-foreground mt-3 text-xs">Watch the bottom of the viewport after clicking.</p>
      </Story>
      ...
      >
        <div className="grid max-w-md gap-4">
          <div className="space-y-1.5">
            <span className="text-muted-foreground text-xs">Custom color + height</span>
          ...
          <div className="space-y-1.5">
            <span className="text-muted-foreground text-xs">Indeterminate + spinner</span>
