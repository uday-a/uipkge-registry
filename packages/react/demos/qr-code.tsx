...
import { QrCode, type QRCodeStatus, type QRCodeErrorLevel } from '@react-registry/qr-code'
...
const longValue = 'https://uipkge.dev/components/qr-code?demo=true&source=github&ref=main'
    ...
    <>
      ...
      <Story title="Status" description="Active, expired, loading, and scanned states.">
        <div className="space-y-4">
          ...
          <QrCode value={customValue} status={currentStatus} onRefresh={onRefresh} />
        </div>
      </Story>
      ...
      <Story title="Long URL" description="Dense QR code from a long URL. Use larger size or higher error level.">
        <div className="flex flex-col items-start gap-2">
          ...
          <p className="text-muted-foreground max-w-md truncate text-xs">{longValue}</p>
        </div>
      </Story>
          ...
          extra={
            <div className="flex items-center gap-2">
              <Button
                size="sm"
                variant="outline"
                onClick={() => setCustomContentValue('https://uipkge.dev/components/advance-select')}
              >
                Change URL
              </Button>
              <Button size="sm" variant="outline" onClick={() => setCustomContentValue('https://uipkge.dev')}>
                Reset
              </Button>
            </div>
          }
