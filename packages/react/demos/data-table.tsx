...
function SelectAllCheckbox({ table }: { table: any }) {
  ...
  return (
    <input
      ref={ref}
      type="checkbox"
      className="accent-foreground size-4 cursor-pointer rounded"
      checked={isAll}
  ...
  return (
    <div className="space-y-4">
      ...
      <Story
        ...
        description="Anything React can render works in `column.cell` / `column.header`. Avatar+name composite, progress bar with side label, custom header markup — all just functions returning nodes."
      >
        <DataTable columns={richColumns} data={data} enablePagination={false} />
      </Story>
      ...
      <Story
        ...
        description="Opt-in: `filterMode=modal` opens filters in a right Sheet. Use when you have many filters or dense inputs (date ranges, long multiselects) that would crowd the toolbar."
      >
        <DataTable
          columns={sortableColumns}
          data={data}
      ...
      >
        <DataTable columns={headerFilterColumns} data={data} hideToolbar />
      </Story>
      ...
      <Story
        ...
        description="`infinite` hides pagination and calls `onFetchMore` when the sentinel row enters the viewport. Append rows on the consumer side."
      >
        <DataTable
          columns={sortableColumns}
          data={infiniteRows}
      ...
      <Story
        ...
        description="`enableDensityToggle` adds a Compact / Cozy / Comfortable control in the toolbar. The table density updates immediately."
      >
        <DataTable
          columns={sortableColumns}
          data={data}
      ...
      <Story
        ...
        description="Pass `totalRows` from the API and listen to `onStateChange` to fetch the next page. The table does not filter or paginate locally."
      >
        <DataTable
          columns={sortableColumns}
          data={serverRows}
      ...
      <Story
        ...
        description="`bulkActionPosition='inline'` renders selected-row actions as a banner above the table instead of the floating HUD."
      >
        <DataTable
          columns={sortableColumns}
          data={data}
