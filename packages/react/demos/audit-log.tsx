...
import { AuditLog, type AuditEntry } from '@react-registry-blocks/audit-log/AuditLog'

const severityEntries: AuditEntry[] = [
  { id: 's1', actor: 'Amara Osei', action: 'sign-in', target: 'admin.acme.com', ip: '77.12.44.9', date: new Date() },
  {
    id: 's2',
    actor: 'Priya Nair',
    action: 'user.updated',
  ...
  { id: 's3', actor: 'System', action: 'role.changed', target: 'contractors → Viewer', ip: '—', date: new Date() },
  { id: 's4', actor: 'Marcus Lee', action: 'export', target: 'ledger-q3.csv', ip: '45.62.110.204', date: new Date() },
  {
    id: 's5',
    actor: 'Jonas Weber',
    action: 'delete',
...
const manyEntries: AuditEntry[] = Array.from({ length: 16 }, (_, i) => ({
  id: `m${i}`,
...
export default function AuditLogDemo() {
  return (
    <>
      ...
      <Story title="Search filtered" description="Type in the search box to filter rows live by actor or target.">
        <AuditLog />
      </Story>
      ...
      >
        <AuditLog entries={severityEntries} />
      </Story>
