...
import { ChoroplethMapChart } from '@react-registry/charts'
...
const worldTooltip = {
  formatter: (p: any) => (p.value == null ? p.name : `${p.name}: ${p.value}% internet penetration`),
}
...
const CONUS_GEOJSON = {
  ...US_GEOJSON,
  features: (US_GEOJSON as any).features.filter((f: any) => !EXCLUDED.has(f.id)),
}
const unemployed = Object.entries(US_DEMOGRAPHICS).map(([id, d]) => ({ id, value: d.unemployment }))
const stateName = Object.fromEntries(Object.entries(US_DEMOGRAPHICS).map(([id, d]) => [id, d.name]))
const usTooltip = {
  formatter: (p: any) => `${stateName[p.name] ?? p.name}: ${p.value == null ? '—' : `${p.value}%`}`,
}
const usBins = [
...
const euTooltip = {
  formatter: (p: any) => (p.value == null ? (euName[p.name] ?? p.name) : `${euName[p.name] ?? p.name}: ${p.value}M`),
}
...
const inTooltip = {
  formatter: (p: any) => (p.value == null ? (inName[p.name] ?? p.name) : `${inName[p.name] ?? p.name}: ${p.value}M`),
}

// ── 6. Flight corridors & freight hubs ──
const logisticsPins = [
  { name: 'Frankfurt Air Cargo', coord: [8.6821, 50.1109] as [number, number], color: '#3b82f6' },
  { name: 'Heathrow Logistics', coord: [-0.4543, 51.47] as [number, number], color: '#10b981' },
  { name: 'Paris CDG Terminal', coord: [2.55, 49.0097] as [number, number], color: '#10b981' },
  { name: 'Milan Malpensa', coord: [8.723, 45.63] as [number, number], color: '#f59e0b' },
]
const logisticsLinks = [
  { from: [8.6821, 50.1109] as [number, number], to: [-0.4543, 51.47] as [number, number], label: 'FRA → LHR' },
  { from: [8.6821, 50.1109] as [number, number], to: [2.55, 49.0097] as [number, number], label: 'FRA → CDG' },
  { from: [2.55, 49.0097] as [number, number], to: [8.723, 45.63] as [number, number], label: 'CDG → MXP' },
]

export default function ChoroplethMapChartDemo() {
  return (
    <>
      <Story
      ...
      >
        <ChoroplethMapChart
          ...
          data={euData}
          showScale
          option={{ visualMap: { inRange: { color: ['#eff6ff', '#1d4ed8'] } }, tooltip: euTooltip }}
          height={440}
        />
      </Story>
      ...
      >
        <ChoroplethMapChart
          ...
          data={inData}
          showScale
          option={{ visualMap: { inRange: { color: ['#f0fdfa', '#0f766e'] } }, tooltip: inTooltip }}
          height={440}
        />
      </Story>
