...
import { KanbanBoard } from '@react-registry-blocks/kanban-task-board/KanbanBoard'
import { SimpleKanban, type SimpleKanbanColumn } from '@react-registry-blocks/kanban-task-board/SimpleKanban'
import { createInitialColumns } from '../../../../../packages/registry-react/bootstrap/kanban-data/kanbanData'

const initialSimpleColumns: SimpleKanbanColumn[] = [
  {
    id: 'backlog',
    title: 'Backlog',
...
export default function KanbanBoardDemo() {
  const [simpleColumns, setSimpleColumns] = useState<SimpleKanbanColumn[]>(initialSimpleColumns)
  ...
  return (
    <>
      <Story
        title="Simple Kanban"
        description="Clean, lightweight drag-and-drop board with column status indicators, item counts, date ranges, tags, assignee avatars, and inline task creation — inspired by Kibo UI."
      >
        <div className="bg-background/50 overflow-hidden rounded-xl border p-4">
          <SimpleKanban columns={simpleColumns} onColumnsChange={setSimpleColumns} />
        </div>
      </Story>

      <Story
        title="Full-Featured Enterprise Board"
        description="Complete kanban surface with priority badges, assignees, tags, due dates, subtask progress, comment/file counts, list/board view switch, task detail sheet, and full add-task dialog."
      >
        <div className="h-[600px] overflow-hidden rounded-lg border p-4">
          <KanbanBoard columns={columns} onColumnsChange={setColumns} lockParentScroll={false} />
        </div>
      </Story>
    </>
  )
