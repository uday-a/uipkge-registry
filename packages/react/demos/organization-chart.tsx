...
import { OrganizationChart, type OrgNode } from '@react-registry/organization-chart'
...
export default function OrganizationChartDemo() {
  ...
  return (
    <>
      ...
      >
        <OrganizationChart data={acmeOrg} onNodeClick={(node) => setSelectedNode(node)} className="w-full" />
        {selectedNode && (
          <p className="text-muted-foreground mt-3 text-xs">
            Selected: <span className="text-foreground font-medium">{selectedNode.name}</span> · {selectedNode.title}
          </p>
        )}
      </Story>
      ...
      >
        <OrganizationChart data={acmeOrg} direction="left-right" className="w-full" />
      </Story>
      ...
      >
        <OrganizationChart data={acmeOrg} defaultExpanded={false} className="w-full" />
      </Story>
        ...
        <Card className="max-w-3xl">
          ...
          <CardContent>
            <OrganizationChart
              ...
              onToggle={(node, expanded) => setLastToggle(`${node.name}: ${expanded ? 'expanded' : 'collapsed'}`)}
              className="w-full"
            />
            {lastToggle && <p className="text-muted-foreground mt-2 text-xs">{lastToggle}</p>}
          </CardContent>
        </Card>
      </Story>
