    ...
    <>
      ...
      >
        <div className="w-full max-w-md">
          {items.map((item) => (
            <div key={item.id} className="border-border/60 flex items-start justify-between gap-3 border-b px-4 py-3">
          ...
          <InfiniteScroll hasMore={hasMore} loading={loading} distance={200} onLoadMore={load} />
          {!hasMore ? (
      ...
      >
        <div className="border-border/60 max-h-80 w-full max-w-md overflow-y-auto rounded-md border">
          ...
          >
            {messages.map((msg) => (
              <div key={`r-${msg.id}`} className="border-border/60 flex gap-2 border-b px-4 py-2.5 text-sm">
                <span className="text-muted-foreground w-12 shrink-0 text-xs">{msg.author}</span>
                <span>{msg.text}</span>
              </div>
            ))}
          </InfiniteScroll>
      ...
      >
        <Card className="max-w-md">
          ...
          <CardContent>
            ...
            <InfiniteScroll hasMore={hasMore} loading={loading} distance={100} onLoadMore={load} />
          </CardContent>
        </Card>
      </Story>
    </>
