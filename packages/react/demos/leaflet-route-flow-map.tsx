...
export default function LeafletRouteFlowMapDemo() {
  return (
    <Story
      ...
      >
        {arcs.map((r) => (
          <React.Fragment key={r.id}>
            <LeafletPolyline lngLatPath={r.parts} color="#e2e8f0" weight={5} opacity={0.18} lineCap="round" />
            <LeafletPolyline
              lngLatPath={r.parts}
              color="#38bdf8"
              weight={2}
            ...
            >
              <LeafletTooltip sticky>
                <div className="font-mono text-xs">
                  <div className="text-foreground font-bold">{r.label}</div>
                  <div className="text-muted-foreground">{r.volume}</div>
                </div>
              </LeafletTooltip>
            </LeafletPolyline>
          </React.Fragment>
        ...
        <div className="border-border bg-card/95 absolute top-3 left-3 z-[1000] rounded-lg border px-3 py-2 shadow-sm backdrop-blur-sm">
          <p className="text-foreground text-xs font-semibold">Active corridors</p>
          <p className="text-muted-foreground mt-0.5 font-mono text-[10px]">6 routes · 8 hubs · live schedule</p>
        </div>
      </LeafletMap>
    </Story>
  )
