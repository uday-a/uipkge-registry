...
export default function BubbleMapDemo() {
  const [selectedNode, setSelectedNode] = React.useState<string | undefined>('us-east-iad')
  const [selectedThreat, setSelectedThreat] = React.useState<string | undefined>(undefined)
          ...
          {selectedNode && (
            <div className="border-border bg-card/60 text-muted-foreground flex items-center justify-between rounded-lg border px-4 py-2.5 text-xs">
              <span className="text-foreground font-medium">
                Selected Cluster: <span className="text-primary font-mono">{selectedNode}</span>
              </span>
              <span>
                Click any circle or press{' '}
                <kbd className="border-border rounded border px-1 py-0.5 font-mono">Enter</kbd> to inspect
              </span>
            </div>
          )}
        </div>
