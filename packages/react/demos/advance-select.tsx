...
export default function AdvanceSelectDemo() {
  ...
  const [maxTagValue, setMaxTagValue] = useState<unknown[]>(['apple', 'banana', 'cherry', 'durian'])
  const [hideSelectedValue, setHideSelectedValue] = useState<unknown[]>([])
  const [customOptValue, setCustomOptValue] = useState<unknown>()
  const [customTagValue, setCustomTagValue] = useState<unknown[]>(['apple', 'banana'])
  ...
  const [remoteOptions, setRemoteOptions] = useState<{ value: string; label: string }[]>([])
  ...
  function handleRemoteSearch(q: string) {
    ...
    remoteTimeout.current = setTimeout(() => {
      if (!q.trim()) {
        ...
        setRemoteOptions(Array.from({ length: 5 }, (_, i) => ({ value: `${q}-${i}`, label: `${q} result ${i + 1}` })))
      }
  ...
  const [labelValue, setLabelValue] = useState<{ value: unknown; label: string } | undefined>()
        ...
        <div className="w-80 space-y-2">
          <AdvanceSelect
            ...
            onValueChange={(v, option) =>
              setLabelValue(v == null ? undefined : { value: v, label: (option as { label: string }).label })
            }
            options={basicOptions}
            placeholder="Pick a fruit"
            className="w-64"
          />
          <p className="text-muted-foreground text-xs">Selected: {labelValue ? JSON.stringify(labelValue) : 'none'}</p>
        </div>
      </Story>
