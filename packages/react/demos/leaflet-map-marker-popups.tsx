...
export default function LeafletMapMarkerPopupsDemo() {
  return (
    <Story
      title="Marker Popups"
    ...
    >
      <LeafletMap variant="streets" center={[-73.985, 40.748]} zoom={13.5} className="h-96 w-full rounded-lg border">
        {sites.map((site) => (
          <LeafletMarker key={site.id} lngLat={site.lngLat} anchor="bottom">
            ...
            <LeafletPopup offset={[0, -38]} className="space-y-1 text-xs">
              <div className="text-foreground font-bold">{site.name}</div>
              <div className="text-muted-foreground">{site.detail}</div>
              <div className="font-mono text-xs font-medium text-emerald-500">{site.status}</div>
            </LeafletPopup>
          </LeafletMarker>
        ))}
      </LeafletMap>
