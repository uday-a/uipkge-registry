...
import {
  ...
  WaterfallChart,
  WordCloudChart,
} from '@react-registry/charts'

// In React, the charts module `use()`-registers every chart type at module
// load (sankey included), so RawChart can render the sankey demo below
// without any consumer-side registration — unlike the Vue vue-echarts build,
// which needs an explicit `use([...])` call.
...
// straight to ECharts. Same pattern works for sunburst, graph,
  ...
  return (
    <>
      <Story title="Area chart" description="Smooth area chart with multiple series.">
        <AreaChart data={monthlyData} xField="month" yField={['revenue', 'profit']} height="280" />
      </Story>

      <Story title="Bar chart" description="Vertical bars with rounded tops.">
        <BarChart data={categoryData} xField="category" yField="value" height="280" />
      </Story>

      <Story title="Line chart" description="Line chart with data point markers.">
        <LineChart data={monthlyData} xField="month" yField="revenue" height="280" />
      </Story>

      <Story title="Pie chart" description="Donut chart showing percentage breakdown.">
        <PieChart data={pieData} nameField="name" valueField="value" donut={true} height="320" />
      </Story>
      ...
      <Story title="Scatter chart" description="XY scatter with category grouping.">
        <ScatterChart data={scatterData} xField="x" yField="y" categoryField="category" height="280" />
      </Story>

      <Story title="Heatmap" description="Color-coded matrix with visual map legend.">
        <Heatmap data={heatmapData} xLabels={['Mon', 'Tue', 'Wed', 'Thu']} yLabels={['A', 'B', 'C']} height="300" />
      </Story>
      ...
      <Story title="Combo bar + line" description="Bars on the left axis, smooth line on the right.">
        <ComboChart data={comboData} xField="m" barField="orders" lineField="conv" height={300} />
      </Story>
