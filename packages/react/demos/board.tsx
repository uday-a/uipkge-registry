...
function useBoardState(
  ...
  const allowedLanesRef = useRef<Map<string, readonly string[] | undefined>>(
    new Map(),
  );
  ...
  const laneOf = useCallback(
    (id: string, l: Record<string, Task[]>): string | null => {
      for (const k of Object.keys(l))
        if (l[k]!.some((t) => t.id === id)) return k;
      return null;
    },
    [],
  );
  ...
  const moveItem = useCallback(
    (itemId: string | string[], toLaneId: string, toIndex?: number) => {
      ...
      setLanes((prev) => {
        ...
        if (
          optsRef.current.accepts &&
          from &&
          !optsRef.current.accepts(moving[0]!.id, from, toLaneId)
        )
          return prev;
        for (const id of ids) {
        ...
        const at =
          typeof toIndex === "number" ? toIndex : next[toLaneId]!.length;
  ...
  const registerAllowedLanes = useCallback(
    (id: string, l: readonly string[] | undefined) => {
  ...
  const isLaneAcceptingFor = useCallback(
    (laneId: string) => {
      ...
      if (
        optsRef.current.accepts &&
        from &&
        !optsRef.current.accepts(draggingId, from, laneId)
      )
        return false;
      return true;
    },
  ...
  const handlers = useMemo(
    () => ({
      ...
      onLaneDrop: (e: React.DragEvent, laneId: string) => {
        e.preventDefault();
        const dragged = draggingIds.length
          ? draggingIds
          : draggingId
            ? [draggingId]
            : [];
        if (dragged.length)
          moveItem(dragged.length === 1 ? dragged[0]! : dragged, laneId);
        setDragOverLaneId(null);
      },
    }),
    ...
    <>
        ...
        >
          {stages.map((s) => (
            ...
            >
              ...
              <BoardLaneEmpty when={countOf(b1.lanes, s) === 0}>
                Drag a card here.
              </BoardLaneEmpty>
            </BoardLane>
          ))}
              ...
              <BoardLaneBody>
                {b6.lanes[s]!.map((t) => (
                  ...
                  >
                    <p className="truncate text-sm font-medium">{t.title}</p>
                    {t.id === "t1" && (
                      <p className="text-info mt-0.5 text-xs">
        ...
        >
          {["inbox", "triaged"].map((k) => (
            ...
            >
              ...
              <BoardLaneEmpty when={countOf(b9.lanes, k) === 0}>
                Empty.
              </BoardLaneEmpty>
            </BoardLane>
          ))}
      ...
      >
        <div className="text-muted-foreground bg-muted/30 rounded-md border p-3 text-xs">
          ...
          <kbd className="border-border bg-background rounded border px-1 py-0.5 font-mono text-xs">
            Space
          </kbd>{" "}
          +{" "}
          <kbd className="border-border bg-background rounded border px-1 py-0.5 font-mono text-xs">
