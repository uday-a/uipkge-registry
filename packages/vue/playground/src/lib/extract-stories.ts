...
function parseDeclarations(code: string, isTopLevel = false, baseOffset = 0): DeclarationChunk[] {
  ...
  while ((match = declPattern.exec(code)) !== null) {
    ...
    for (let i = startIndex; i < code.length; i++) {
      ...
      if (!fnName && parenDepth === 0 && bracketDepth === 0 && braceDepth === 0 && ch === '\n') {
    ...
    const declSource = code.slice(startIndex, endIndex).trim().replace(/;$/, '')
    chunks.push({
      names,
      source: declSource,
...
function deriveComponentName(componentName: string | undefined, storyTitle: string): string {
...
function renderImports(imports: ImportStatement[], referencedIdents: Set<string>, isReact = false): string[] {
  const lines: string[] = []
  for (const imp of imports) {
    if (imp.sourcePath.includes('components/story/Story')) continue
    const rewrittenPath = isReact ? rewriteImportPath(imp.sourcePath) : imp.sourcePath

    const usedSpecifiers: string[] = []
    for (const spec of imp.specifiers) {
      if (referencedIdents.has(spec.local)) {
        usedSpecifiers.push(spec.imported === spec.local ? spec.local : `${spec.imported} as ${spec.local}`)
      }
    }

    const hasDefault = Boolean(imp.isDefault && imp.defaultName && referencedIdents.has(imp.defaultName))
    const hasNamespace = Boolean(imp.isNamespace && imp.namespaceName && referencedIdents.has(imp.namespaceName))
    ...
    if (hasDefault && !usedSpecifiers.length && !hasNamespace) {
      ...
      lines.push(`import ${imp.defaultName}, { ${usedSpecifiers.join(', ')} } from '${rewrittenPath}'`)
 ...
 */
export function extractStories(rawSource: string, _componentName?: string): Record<string, string> {
  const re = /<Story\s+([^>]*?)>([\s\S]*?)<\/Story>/g
  const out: Record<string, string> = {}
  const setupBlock = rawSource.match(/<script[^>]*setup[^>]*>([\s\S]*?)<\/script>/)?.[1] ?? ''
  ...
  while ((m = re.exec(rawSource)) !== null) {
    ...
    if (selectedDecls.length) scriptParts.push(selectedDecls.map((d) => d.source).join('\n\n'))
 ...
 */
export function extractStoriesJsx(rawSource: string, componentName?: string): Record<string, string> {
  const re = /<Story\s+([^>]*?)>([\s\S]*?)<\/Story>/g
  const out: Record<string, string> = {}

  const allImports = parseImports(rawSource)

  const fnMatch = rawSource.match(/export\s+default\s+function\s+([A-Za-z_$][\w$]*)\s*\([^)]*\)\s*\{([\s\S]*)\n\}/)
  ...
  const localDeclarationsCode = returnIdx >= 0 ? insideFnCode.slice(0, returnIdx) : insideFnCode

  const allDecls: DeclarationChunk[] = [
    ...parseDeclarations(topLevelCode, true, 0),
    ...parseDeclarations(localDeclarationsCode, false, fnMatch ? fnMatch.index! : 0),
  ]
