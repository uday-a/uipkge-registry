...
export function extractProps(source: string): PropsExtractResult {
  ...
  for (const id of identifierTokens) {
    ...
    if (localBody) {
      ...
      forwards.push({ name: bareName, source: findImportSource(block, bareName) })
    }
  }
  ...
  for (const f of fields) {
    if (!f.values || f.values.length === 0) {
      if (!/^[A-Za-z_$][\w$]*$/.test(f.type)) continue
      try {
        const aliasMatch = block.match(new RegExp(`(?:type|interface)\\s+${f.type}\\s*=\\s*([^;\\n]+)`))
        ...
        // Ignore regex syntax errors from complex type strings
      }
    }
  }
...
function stripComments(src: string): string {
  return src.replace(/\/\*[\s\S]*?\*\//g, (m) => ' '.repeat(m.length)).replace(/\/\/.*$/gm, (m) => ' '.repeat(m.length))
}
...
function findLocalTypeBody(block: string, name: string): string | null {
  const re = new RegExp(`(?:interface|type)\\s+${name}(?:\\s+extends\\s+[^{]+)?\\s*=?\\s*\\{`, 'm')
...
function findImportSource(block: string, name: string): string | null {
  ...
  while ((m = re.exec(block)) !== null) {
    const namedImports = m[1].split(',').map((s) => s.trim().replace(/\s+as\s+\w+$/, ''))
