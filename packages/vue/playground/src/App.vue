...
import { ref, computed, shallowRef, watch, provide, onMounted, onUnmounted } from 'vue'
import WorkbenchHeader from './components/WorkbenchHeader.vue'
import WorkbenchSidebar, { type SidebarItem } from './components/WorkbenchSidebar.vue'
import TestBenchDrawer, { type LoggedEvent } from './components/TestBenchDrawer.vue'
...
const loadComponent = async (id: string) => {
  const demoPath = Object.keys(demoModules).find((p) => p.endsWith(`/${id}.vue`))
  if (!demoPath) return
  ...
  try {
    ...
    if (localMeta) {
      ...
      const mainVue = localMeta.files?.find((f: any) => f.path?.endsWith('.vue'))
    ...
    try {
      ...
      if (res.ok) {
        ...
        const mainVueFile = json.files?.find((f: any) => f.path.endsWith('.vue'))
...
<template>
  <div class="bg-background text-foreground flex h-screen w-screen overflow-hidden font-sans antialiased">
        ...
        >
          ...
          >
            <div class="flex items-center gap-2">
              ...
              <span class="text-foreground text-[11px] font-semibold tracking-wider uppercase"
          ...
          <!-- Loading state -->
          <div v-if="loading" class="text-muted-foreground flex items-center justify-center py-32 font-mono text-xs">
            <div class="flex flex-col items-center gap-3">
              <div class="border-primary size-6 animate-spin rounded-full border-2 border-t-transparent" />
          ...
          <div v-else :key="remountKey">
            ...
            <div v-else class="text-muted-foreground py-24 text-center text-xs">Select a component to preview</div>
          </div>
        </div>
      </main>
