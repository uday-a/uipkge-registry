...
<template>
  <Teleport to="body" :disabled="!isFullscreen">
    ...
    <div
      v-if="isFullscreen"
      class="fixed inset-0 z-40 bg-black/60 backdrop-blur-xs animate-in fade-in duration-150"
      @click="isFullscreen = false"
    />

    <section
      :id="title.toLowerCase().replace(/[^a-z0-9]+/g, '-')"
      class="group overflow-hidden rounded-xl border border-border bg-card text-card-foreground shadow-xs transition-all duration-200"
      :class="[
        isFullscreen
          ? 'flex flex-col shadow-2xl bg-card border-border/80'
          : 'relative mb-8',
      ]"
      :style="
        isFullscreen
      ...
      >
        ...
        <div class="flex items-center gap-1.5 shrink-0">
          ...
          >
            <button
              type="button"
              class="flex items-center gap-1.5 rounded-md px-2.5 py-1 font-medium transition-colors cursor-pointer"
              :class="
                activeTab === 'preview'
          ...
          <button
            v-if="sourceCode"
            type="button"
            class="flex size-7 items-center justify-center rounded-md border border-border bg-background text-muted-foreground hover:bg-muted hover:text-foreground transition shadow-xs cursor-pointer"
