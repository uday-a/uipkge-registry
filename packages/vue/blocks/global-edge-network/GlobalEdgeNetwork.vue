...
<template>
  ...
  >
    ...
    <div class="pointer-events-none absolute bottom-3 left-3 z-10 sm:bottom-4 sm:left-4">
      <div
        class="border-border/60 bg-background/80 pointer-events-auto inline-flex items-center gap-2 rounded-full border px-3 py-1.5 text-xs shadow-xs backdrop-blur-md transition-all duration-150"
      >
        <template v-if="activeNode">
          <span
            class="h-1.5 w-1.5 shrink-0 rounded-full"
            :class="activeNode.status === 'optimal' ? 'bg-emerald-500' : 'bg-amber-500'"
          />
