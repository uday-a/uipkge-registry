...
const selectedDomainFilter = ref(props.initialDomainFilter)
const selectedPortTypeFilter = ref('all')
const selectedProductId = ref(props.initialSelectedProductId)
const activePortId = ref<'snowflake' | 'kafka' | 'graphql' | 's3'>('snowflake')
...
function resetFilters() {
  ...
  selectedPortTypeFilter.value = 'all'
  searchQuery.value = ''
}
...
<template>
  ...
  >
      ...
      <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div class="space-y-2">
          ...
          <div class="flex flex-wrap items-center gap-2 text-xs">
            ...
            <div
              class="inline-flex items-center gap-1.5 rounded-full border border-emerald-500/30 bg-emerald-500/10 px-2.5 py-0.5 font-medium text-emerald-700 dark:text-emerald-300"
            >
    ...
    <section class="space-y-3">
      ...
      <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        >
          ...
          <div class="flex items-start justify-between gap-2">
            <div class="flex items-center gap-2.5">
              <div class="bg-muted border-border/80 flex size-8 shrink-0 items-center justify-center rounded-lg border">
                <CreditCard v-if="domain.iconName === 'CreditCard'" class="size-4 text-indigo-500" />
                <Users v-else-if="domain.iconName === 'Users'" class="size-4 text-sky-500" />
                <Truck v-else-if="domain.iconName === 'Truck'" class="size-4 text-amber-500" />
                <Activity v-else class="size-4 text-emerald-500" />
              </div>
              <div>
          ...
          <!-- Domain Meta & Lead -->
          <div class="border-border/60 space-y-1.5 border-t pt-2.5">
            ...
            <div class="text-muted-foreground flex items-center justify-between gap-x-2">
              ...
              <span class="text-foreground font-mono font-medium tabular-nums"
                >{{ domain.activeConsumers }} Services</span
              >
            </div>
            <div class="text-muted-foreground flex items-center justify-between gap-x-2">
              <span>Storage / Egress:</span>
              <span class="text-foreground font-mono font-medium"
                >{{ domain.storageVolume }} · {{ domain.throughput }}</span
              >
            </div>
          </div>
        </div>
    ...
    <section class="border-border bg-card/50 space-y-4 rounded-xl border p-4 sm:p-5">
      <div class="border-border/80 flex flex-col gap-3 border-b pb-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <div class="flex items-center gap-2">
        ...
        <div class="flex flex-wrap items-center gap-2">
          <div class="relative w-full sm:w-64">
            ...
            <input
              ...
              class="border-border bg-background text-foreground placeholder:text-muted-foreground focus-visible:ring-ring h-8.5 w-full rounded-md border pr-3 pl-8 text-xs outline-none focus-visible:ring-1"
            />
          </div>

          <Button
            v-if="selectedDomainFilter !== 'all' || selectedPortTypeFilter !== 'all' || searchQuery"
            variant="ghost"
            size="sm"
            class="text-muted-foreground hover:text-foreground h-8.5 text-xs"
            @click="resetFilters"
          >
            Reset Filters
          </Button>
      ...
      <!-- Table Container -->
      <div class="border-border bg-background overflow-hidden rounded-lg border shadow-xs">
        <Table>
          <TableHeader>
            <TableRow class="bg-muted/40 hover:bg-muted/40">
              <TableHead class="text-xs font-semibold">Data Product & Domain</TableHead>
              <TableHead class="text-xs font-semibold">Output Ports</TableHead>
              <TableHead class="text-xs font-semibold">Quality & SLA Score</TableHead>
              <TableHead class="text-xs font-semibold">Consumers</TableHead>
              <TableHead class="text-xs font-semibold">Product Owner & Team</TableHead>
          ...
          <TableBody>
            ...
            >
              ...
              <TableCell class="py-3">
                <div class="space-y-1">
                  <div class="flex items-center gap-2">
                    <span class="text-foreground font-mono font-bold">{{ product.name }}</span>
                    <Badge variant="outline" class="font-mono text-xs font-normal">
                      {{ product.version }}
                    </Badge>
                    <Badge
                      v-if="product.classification === 'PII-Confidential'"
                      class="border-rose-500/20 bg-rose-500/10 text-xs font-normal text-rose-700 dark:text-rose-300"
                    >
                      PII Masked
                    </Badge>
                    <Badge
                      v-else-if="product.classification === 'Financial-Audit'"
                      class="border-amber-500/20 bg-amber-500/10 text-xs font-normal text-amber-700 dark:text-amber-300"
                    >
                      SOX Certified
                    </Badge>
                  </div>
                  ...
                  <div class="flex items-center gap-2 pt-0.5">
                    ...
                    <span class="text-muted-foreground font-mono text-xs"
                      >Freshness: {{ product.freshnessLatency }}</span
                    >
                  </div>
                </div>
              </TableCell>
              ...
              <TableCell class="py-3">
                <div class="flex flex-wrap items-center gap-1.5">
                  ...
                  >
                    <Database v-if="port.id === 'snowflake'" class="size-3 text-sky-500" />
                    <Zap v-else-if="port.id === 'kafka'" class="size-3 text-amber-500" />
                    <Network v-else-if="port.id === 'graphql'" class="size-3 text-indigo-500" />
                    <HardDrive v-else class="size-3 text-emerald-500" />
              ...
              <TableCell class="py-3">
                <div class="w-40 space-y-1.5">
                  ...
                  <div class="text-muted-foreground text-xs">
                    Uptime: <span class="text-foreground font-mono font-medium">{{ product.uptime }}</span>
                  </div>
                </div>
              </TableCell>
              ...
              <TableCell class="py-3">
                <div class="space-y-1">
                  <div class="flex items-center gap-1.5">
                    <Badge variant="outline" class="font-mono text-xs font-semibold tabular-nums">
                      {{ product.activeConsumersCount }} Consumer Apps
                    </Badge>
                  </div>
                  <div class="text-muted-foreground line-clamp-1 max-w-[180px] text-xs">
              ...
              <TableCell class="py-3">
                <div class="flex items-center gap-2.5">
                  <Avatar class="size-7">
                    <AvatarImage v-if="product.owner.avatar" :src="product.owner.avatar" :alt="product.owner.name" />
                    <AvatarFallback class="text-xs font-medium">{{ product.owner.initials }}</AvatarFallback>
                  </Avatar>
                  <div class="min-w-0">
                    <span class="text-foreground block truncate font-medium">{{ product.owner.name }}</span>
                    <span class="text-muted-foreground block font-mono text-xs">{{ product.owner.team }}</span>
                  </div>
                </div>
              </TableCell>
              ...
              <TableCell class="py-3 text-right" @click.stop>
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end" class="w-56 text-xs">
                    ...
                    >
                      <FileCode2 class="size-3.5 text-purple-500" />
    ...
    <section class="border-border bg-card/60 space-y-6 rounded-xl border p-4 sm:p-6">
      ...
      <div class="border-border/80 flex flex-col gap-3 border-b pb-4 lg:flex-row lg:items-start lg:justify-between">
        ...
        <div class="flex flex-wrap items-center gap-2">
          ...
          <div
            class="border-border bg-background inline-flex items-center gap-1.5 rounded-md border px-2.5 py-1 text-xs"
          >
            <span class="text-muted-foreground font-mono">Contract:</span>
            <span class="text-foreground font-mono">{{ selectedProduct.contractHash.slice(0, 16) }}...</span>
            <Button
              variant="ghost"
              size="sm"
              class="h-5 px-1 text-xs"
              @click="copyToClipboard(selectedProduct.contractHash, 'hash')"
            >
              <Check v-if="copiedUri === 'hash'" class="size-3 text-emerald-500" />
          ...
          <Button
            ...
            @click="handleOpenAccessModal(selectedProduct)"
          >
            <KeyRound class="size-3.5 text-amber-500" />
          ...
          <Button
            ...
            size="sm"
            class="h-8.5 gap-1.5 text-xs"
            @click="handleOpenQueryRunner(selectedProduct)"
          >
      ...
      <div class="grid grid-cols-1 gap-2.5 sm:grid-cols-2 lg:grid-cols-4">
        <div class="border-border bg-background/80 space-y-1 rounded-lg border p-3 text-xs">
          ...
          <p class="text-muted-foreground text-xs leading-relaxed">
            ...
            <span class="text-foreground font-medium">{{ selectedProduct.owner.name }}</span> ({{
        ...
        <div class="border-border bg-background/80 space-y-1 rounded-lg border p-3 text-xs">
          ...
          <p class="text-muted-foreground text-xs leading-relaxed">
            ...
            <span class="text-foreground font-mono font-medium">{{ selectedProduct.slaScore }}% SLA</span> and automated
            test assertions.
          </p>
        </div>

        <div class="border-border bg-background/80 space-y-1 rounded-lg border p-3 text-xs">
          <div class="flex items-center gap-1.5 font-semibold text-sky-500">
      ...
      <div class="space-y-4">
        <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div class="border-border bg-muted/60 inline-flex items-center gap-1 rounded-lg border p-1">
            <button
              v-for="port in selectedProduct.ports"
              :key="port.id"
              :class="
                cn(
            ...
            >
              <Database v-if="port.id === 'snowflake'" class="size-3.5 text-sky-500" />
              <Zap v-else-if="port.id === 'kafka'" class="size-3.5 text-amber-500" />
              <Network v-else-if="port.id === 'graphql'" class="size-3.5 text-indigo-500" />
              <HardDrive v-else class="size-3.5 text-emerald-500" />
              <span>{{ port.name }}</span>
            </button>
          </div>
        </div>
        ...
        <div class="border-border bg-background space-y-4 rounded-xl border p-4 sm:p-5">
          <!-- Port Meta Bar -->
          <div
            class="border-border/80 flex flex-col gap-3 border-b pb-3 lg:flex-row lg:items-center lg:justify-between"
          >
            <div class="space-y-1">
              <div class="flex flex-wrap items-center gap-2">
                <span class="text-foreground font-mono text-sm font-bold">{{ activePort.name }}</span>
                <Badge variant="secondary" class="font-mono text-xs">{{ activePort.protocol }}</Badge>
                <Badge variant="outline" class="font-mono text-xs">{{ activePort.accessRole }}</Badge>
              </div>
              <div class="flex items-center gap-2">
                ...
                <Button
                  ...
                  @click="copyToClipboard(activePort.uri, activePort.uri)"
                >
                  <Check v-if="copiedUri === activePort.uri" class="size-3 text-emerald-500" />
                  <Copy v-else class="size-3" />
                  <span class="ml-1 font-mono text-xs">{{ copiedUri === activePort.uri ? 'Copied' : 'Copy' }}</span>
                </Button>
              </div>
            </div>
            ...
            <div class="flex items-center gap-2">
              <span
                v-if="activePort.throughput"
                class="border-border bg-muted/40 text-muted-foreground rounded-md border px-2.5 py-1 font-mono text-xs"
              >
                Throughput: <strong class="text-foreground">{{ activePort.throughput }}</strong>
              </span>
              <span
                v-if="activePort.rateLimit"
                class="border-border bg-muted/40 text-muted-foreground rounded-md border px-2.5 py-1 font-mono text-xs"
              >
                Limit: <strong class="text-foreground">{{ activePort.rateLimit }}</strong>
              </span>
            </div>
          </div>

          <!-- Port Schema Dictionary (If Columns Defined) -->
          <div v-if="activePort.columns && activePort.columns.length > 0" class="space-y-2">
            <div class="flex items-center justify-between gap-x-2">
              <span class="text-foreground text-xs font-semibold">Schema Column Definitions & Constraints</span>
              <span class="text-muted-foreground font-mono text-xs"
                >{{ activePort.columns.length }} columns registered</span
              >
            </div>
            ...
            <div class="border-border bg-card overflow-hidden rounded-lg border">
              <Table>
                ...
                <TableBody>
                  <TableRow v-for="col in activePort.columns" :key="col.name" class="font-mono text-xs">
                    <TableCell class="text-foreground font-bold">
                      <div class="flex items-center gap-1.5">
                        <KeyRound v-if="col.isKey" class="size-3.5 text-amber-500" />
                        <span>{{ col.name }}</span>
                      </div>
                    </TableCell>
                    <TableCell>
          ...
          <div v-if="activePort.sampleQuery || activePort.samplePayload" class="space-y-2">
            <div class="flex items-center justify-between gap-x-2">
              <div class="flex items-center gap-2">
              ...
              <Button
                ...
                @click="copyToClipboard(activePort.sampleQuery || activePort.samplePayload || '', 'code')"
              >
                <Check v-if="copiedUri === 'code'" class="size-3 text-emerald-500" />
                <Copy v-else class="size-3" />
            ...
            >
              <pre class="whitespace-pre"><code>{{ activePort.sampleQuery || activePort.samplePayload }}</code></pre>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="space-y-3 pt-2">
        ...
        <div class="grid grid-cols-1 gap-4 lg:grid-cols-3">
          ...
          <div class="border-border bg-background space-y-3 rounded-lg border p-4 text-xs">
            ...
            <div class="space-y-2">
              ...
              <div class="flex items-center justify-between gap-x-2">
                ...
                <span class="text-foreground font-mono font-medium">{{ selectedProduct.freshnessLatency }}</span>
              </div>
              <div class="flex items-center justify-between gap-x-2">
                <span class="text-muted-foreground">Quality Check Assertions:</span>
                <span class="text-foreground font-mono font-medium text-emerald-600 dark:text-emerald-400">
              ...
              <div class="flex items-center justify-between gap-x-2">
                ...
                <Badge variant="secondary" class="font-mono text-xs">{{ selectedProduct.classification }}</Badge>
              </div>
            </div>
          </div>
          ...
          <div class="border-border bg-background space-y-2.5 rounded-lg border p-4 text-xs lg:col-span-2">
            ...
            <div class="space-y-2">
              <div
                v-for="(policy, idx) in selectedProduct.governancePolicies"
                :key="idx"
                class="border-border/70 bg-muted/30 flex items-start gap-2.5 rounded-md border p-2.5"
              >
                <div
                  class="mt-0.5 flex size-4 shrink-0 items-center justify-center rounded-full bg-emerald-500/10 text-emerald-600 dark:text-emerald-400"
                >
    ...
    <div v-if="isQueryDrawerOpen" class="border-border bg-muted/30 overflow-hidden rounded-xl border transition-all">
      <div class="border-border bg-card flex items-center justify-between gap-x-2 border-b px-4 py-2.5">
        <div class="flex items-center gap-2">
          ...
          <Badge variant="outline" class="font-mono text-xs">{{ activePort.protocol }}</Badge>
        </div>
        <div class="flex items-center gap-2">
          <Button
            variant="default"
      ...
      <div class="space-y-3 p-4">
        ...
        >
          <pre
            class="leading-relaxed whitespace-pre"
          ><code>{{ activePort.sampleQuery || activePort.samplePayload }}</code></pre>
        </div>

        <div v-if="activePort.mockResults" class="border-border bg-card overflow-hidden rounded-lg border">
          <div
          ...
          <Table>
            <TableHeader>
              <TableRow class="bg-muted/30 hover:bg-muted/30">
                <TableHead
                  v-for="key in Object.keys(activePort.mockResults[0] || {})"
                  :key="key"
            ...
            <TableBody>
              <TableRow v-for="(row, rIdx) in activePort.mockResults" :key="rIdx" class="font-mono text-xs">
    ...
    <div
      v-if="isAccessModalOpen"
      class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-xs"
      @click.self="isAccessModalOpen = false"
    >
      <div class="border-border bg-card text-card-foreground w-full max-w-lg space-y-4 rounded-xl border p-5 shadow-lg">
        <div class="border-border/80 flex items-start justify-between border-b pb-3">
          <div class="space-y-1">
            ...
            <p class="text-muted-foreground text-xs">
              ...
              <code class="text-foreground font-mono font-semibold">{{ selectedProduct.name }}</code
              >.
            </p>
          </div>
          <Button variant="ghost" size="sm" class="size-7 p-0" @click="isAccessModalOpen = false">
            <X class="size-4" />
          </Button>
        </div>

        <div
          v-if="isAccessApproved"
          class="space-y-2 rounded-lg border border-emerald-500/30 bg-emerald-500/10 p-4 text-center"
        >
          <div
            class="inline-flex size-8 items-center justify-center rounded-full bg-emerald-500/20 text-emerald-600 dark:text-emerald-400"
          >
            <CheckCircle2 class="size-5" />
          </div>
          <h4 class="text-xs font-semibold text-emerald-700 dark:text-emerald-300">Access Grant Provisioned</h4>
          <p class="text-muted-foreground text-xs">
        ...
        <form v-else class="space-y-3 text-xs" @submit.prevent="submitAccessRequest">
          ...
          <div class="space-y-1">
            ...
            >
              <option v-for="port in selectedProduct.ports" :key="port.id" :value="port.id">
          ...
          <div class="border-border/80 flex items-center justify-end gap-2 border-t pt-3">
            <Button type="button" variant="outline" size="sm" class="h-8 text-xs" @click="isAccessModalOpen = false">
              Cancel
            </Button>
    ...
    <div
      ...
      class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-xs"
      @click.self="isRegisterModalOpen = false"
    >
      <div class="border-border bg-card text-card-foreground w-full max-w-lg space-y-4 rounded-xl border p-5 shadow-lg">
        <div class="border-border/80 flex items-start justify-between border-b pb-3">
          <div class="space-y-1">
          ...
          <Button variant="ghost" size="sm" class="size-7 p-0" @click="isRegisterModalOpen = false">
            <X class="size-4" />
          </Button>
        </div>
        ...
        >
          ...
          <h4 class="text-xs font-semibold text-emerald-700 dark:text-emerald-300">
        ...
        <form v-else class="space-y-3 text-xs" @submit.prevent="submitRegisterProduct">
          ...
          <div class="border-border/80 flex items-center justify-end gap-2 border-t pt-3">
            <Button type="button" variant="outline" size="sm" class="h-8 text-xs" @click="isRegisterModalOpen = false">
              Cancel
            </Button>
            <Button type="submit" variant="default" size="sm" class="h-8 text-xs font-medium">
