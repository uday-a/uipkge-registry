...
const currentRoom = computed(() => {
  return rooms.find((r) => r.id === currentRoomId.value) || rooms[0]
})
...
function getHotspotStyle(hotspot: RoomHotspot) {
  // Translate coordinate relative to current camera yaw
...
<template>
  <div
    ...
    :class="[
      'bg-background text-foreground w-full space-y-4 font-sans transition-all duration-300',
    ...
    <header class="bg-card rounded-xl border p-3.5 shadow-xs sm:px-5 sm:py-3">
      <div class="flex flex-wrap items-center justify-between gap-3">
        ...
        <div class="flex items-center gap-2">
          ...
          >
            <Volume2 v-if="isAudioActive" class="size-3.5 text-sky-500" />
          ...
          >
            <Check v-if="copiedToast" class="size-3.5 text-emerald-500" />
    ...
    >
      ...
      >
        ...
        <div v-if="isMeasurementMode" class="pointer-events-none absolute inset-0 size-full">
          ...
          >
            <div
              class="flex items-center gap-1.5 rounded-md border border-rose-500/80 bg-zinc-950/90 px-2 py-0.5 font-mono text-xs font-bold text-rose-400 shadow-xl backdrop-blur-md"
            >
              <Ruler class="size-3" />
              <span>{{ m.title }}:</span>
      ...
      <div class="pointer-events-auto absolute inset-0 size-full">
        ...
        >
          ...
          <div v-if="hotspot.type === 'portal'" class="group relative flex flex-col items-center">
            <!-- Pulsing Radar Ping Rings -->
            <span class="absolute -inset-2.5 rounded-full bg-emerald-400 opacity-60 duration-1000" />
            <span class="absolute -inset-1 rounded-full bg-emerald-500/40 opacity-80 blur-xs" />

            <!-- Interactive Portal Button Pin -->
            <button
              type="button"
              class="relative flex size-9 items-center justify-center rounded-full border-2 border-emerald-400 bg-emerald-600/90 text-white shadow-xl backdrop-blur-md transition-transform hover:scale-115 focus-visible:ring-2 focus-visible:ring-emerald-400 focus-visible:outline-none"
            ...
            <div
              :class="[
                'pointer-events-none absolute top-11 z-30 flex w-52 flex-col items-center rounded-lg border border-emerald-500/40 bg-zinc-950/95 p-2 text-center shadow-sm backdrop-blur-md transition-all duration-200',
            ...
            >
              ...
              <span class="mt-1 inline-flex items-center gap-1 font-mono text-xs font-semibold text-emerald-300">
          ...
          <div v-else-if="hotspot.type === 'info'" class="group relative flex flex-col items-center">
            <!-- Pulsing Info Radar Rings -->
            <span class="absolute -inset-2.5 rounded-full bg-sky-400 opacity-60 duration-1000" />
            <span class="absolute -inset-1 rounded-full bg-sky-500/40 opacity-80 blur-xs" />

            <!-- Interactive Info Button Pin -->
            <button
              type="button"
              class="relative flex size-9 items-center justify-center rounded-full border-2 border-sky-400 bg-sky-600/90 text-white shadow-xl backdrop-blur-md transition-transform hover:scale-115 focus-visible:ring-2 focus-visible:ring-sky-400 focus-visible:outline-none"
              :aria-label="hotspot.title"
            ...
            <div
              :class="[
                'pointer-events-none absolute top-11 z-30 flex w-52 flex-col items-center rounded-lg border border-sky-500/40 bg-zinc-950/95 p-2 text-center shadow-sm backdrop-blur-md transition-all duration-200',
                hoveredHotspotId === hotspot.id ? 'translate-y-0 opacity-100' : 'translate-y-1 opacity-0',
              ]"
            >
              ...
              <span class="mt-1 inline-flex items-center gap-1 font-mono text-xs font-semibold text-sky-300">
      ...
      >
        ...
        >
          ...
          <span class="font-mono text-xs text-zinc-400">FOV 85°</span>
          <Separator orientation="vertical" class="h-3.5 bg-zinc-700" />
        ...
        <div class="pointer-events-auto flex items-center gap-2">
          <!-- VR Mode Active Pill (If active) -->
          <div
            v-if="isVrMode"
            class="flex items-center gap-1.5 rounded-lg border border-indigo-500/50 bg-indigo-950/80 px-2.5 py-1 text-xs font-semibold text-indigo-300 backdrop-blur-md"
          >
            <Glasses class="size-3.5 animate-pulse" />
            <span>Gyroscope / VR Sensor Active</span>
          </div>

          <!-- Measurement Active Unit Switcher -->
          <div
            v-if="isMeasurementMode"
            class="flex items-center gap-1 rounded-lg border border-rose-500/50 bg-zinc-900/90 p-1 backdrop-blur-md"
          >
            <Button
              size="sm"
      ...
      <div
        v-if="isMinimapOpen"
        class="pointer-events-auto absolute top-14 left-3.5 z-30 w-56 overflow-hidden rounded-xl border border-zinc-700/80 bg-zinc-950/90 shadow-sm backdrop-blur-md transition-all sm:w-64"
      >
        ...
        <div class="relative p-2.5">
          ...
          <svg class="w-full" viewBox="0 0 240 160" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <g v-for="r in rooms" :key="'map-r-' + r.id" class="cursor-pointer" @click="switchRoom(r.id)">
              <rect
            ...
            >
              ...
              <line x1="0" y1="0" x2="-22" y2="-44" stroke="#10b981" stroke-width="1" stroke-dasharray="2 2" />
              <line x1="0" y1="0" x2="22" y2="-44" stroke="#10b981" stroke-width="1" stroke-dasharray="2 2" />
            </g>
      ...
      <div
        v-if="activeInfoHotspot && activeInfoHotspot.infoData"
        class="pointer-events-auto absolute inset-x-4 top-14 z-40 mx-auto max-w-md rounded-xl border border-sky-500/40 bg-zinc-950/95 p-4 shadow-sm backdrop-blur-xl sm:top-20"
      >
        <div class="flex items-start justify-between gap-3 border-b border-zinc-800 pb-3">
          <div class="space-y-1">
            <div class="flex items-center gap-2">
              <Badge variant="outline" class="border-sky-500/50 bg-sky-500/10 text-xs font-semibold text-sky-400">
                {{ activeInfoHotspot.infoData.category }}
              </Badge>
              <span class="font-mono text-xs text-zinc-400">{{ activeInfoHotspot.infoData.highlight }}</span>
            </div>
          ...
          <Button
            variant="ghost"
            size="icon"
            class="size-7 text-zinc-400 hover:text-white"
            aria-label="Close feature modal"
            @click="activeInfoHotspot = null"
          >
            <X class="size-4" />
          </Button>
        </div>
        ...
        <div class="space-y-3 pt-3 text-xs">
          <p class="leading-relaxed text-zinc-300">
            {{ activeInfoHotspot.infoData.description }}
          </p>
          ...
          <div class="space-y-1.5 rounded-lg border border-zinc-800 bg-zinc-900/60 p-2.5">
            ...
            <ul class="grid grid-cols-1 gap-1 text-zinc-200 sm:grid-cols-2">
              <li v-for="(spec, idx) in activeInfoHotspot.infoData.specs" :key="idx" class="flex items-center gap-1.5">
                <CheckCircle2 class="size-3 shrink-0 text-sky-400" />
                <span class="truncate">{{ spec }}</span>
              </li>
            </ul>
          </div>

          <div class="flex items-center justify-between border-t border-zinc-800 pt-2 text-xs text-zinc-400">
            <span
              >Finishes: <strong class="text-zinc-200">{{ activeInfoHotspot.infoData.material }}</strong></span
            >
            <Button size="sm" class="h-7 text-xs" @click="activeInfoHotspot = null"> Done </Button>
          </div>
        </div>
      </div>
        ...
        >
          ...
          <div class="flex items-center gap-1.5 overflow-x-auto py-0.5">
            <button
              ...
              type="button"
              :class="[
                'group flex items-center gap-2 rounded-xl px-2.5 py-1.5 text-xs font-medium transition-all focus-visible:ring-2 focus-visible:ring-emerald-400 focus-visible:outline-none',
                currentRoomId === r.id
        ...
        <CardContent class="space-y-4">
          ...
          <div class="space-y-2 border-t pt-3">
            ...
            <div class="grid grid-cols-1 gap-2 sm:grid-cols-2">
              ...
              >
                <ShieldCheck class="mt-0.5 size-3.5 shrink-0 text-emerald-500" />
                <div>
