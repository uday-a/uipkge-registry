...
<template>
  <div :class="cn('@container w-full max-w-4xl space-y-6', props.class)">
    ...
    <Card class="border-border shadow-xs">
      <CardHeader
        class="flex flex-col justify-between gap-4 pb-4 @md:flex-row @md:items-center"
      >
        <div class="min-w-0 space-y-1">
          <div class="flex flex-wrap items-center gap-2">
            ...
            <CardTitle class="text-base font-semibold tracking-tight sm:text-lg"
              >Cloud Backup & Recovery</CardTitle
            >
            <Badge
              variant="outline"
              class="border-success/20 bg-success/10 text-success shrink-0 gap-1"
            >
      ...
      <CardContent
        class="grid grid-cols-1 gap-4 p-4 pt-4 sm:gap-6 sm:p-6 sm:pt-6 @md:grid-cols-2"
      >
        <!-- Automation Schedule Tile -->
        <div
          class="border-border bg-card min-w-0 space-y-4 rounded-lg border p-3.5 sm:p-4"
        >
          ...
          <div class="text-muted-foreground space-y-2 text-xs">
            ...
            <div class="flex items-center justify-between gap-2">
              ...
              <span class="text-foreground truncate text-right font-medium"
                >{{ retentionDays }} days (rolling)</span
              >
            </div>
            <div class="flex items-center justify-between gap-2">
              <span class="shrink-0">Target:</span>
              <span class="text-foreground truncate text-right font-medium"
                >AWS S3 (us-east-1)</span
              >
            </div>
          </div>
        </div>
        ...
        >
          ...
          <div
            class="text-muted-foreground flex items-center justify-between gap-2 text-xs tabular-nums"
          >
      ...
      <CardContent class="space-y-4 p-4 pt-4 sm:p-6 sm:pt-6">
        <div class="flex flex-wrap items-center justify-between gap-2">
          ...
          <span class="text-muted-foreground text-xs"
            >Last verified: {{ lastBackupSuccess }}</span
          >
        </div>

        <div
          class="divide-border border-border bg-card divide-y rounded-lg border"
        >
          <div
          ...
          >
            ...
            <div
              class="flex shrink-0 items-center justify-between gap-3 pl-7 @md:justify-end @md:pl-0"
            >
