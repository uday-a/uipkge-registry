...
const props = withDefaults(defineProps<LastMileDispatchProps>(), {
...
const routePolylineGeoJson = computed(() => ({
  type: 'Feature',
  geometry: {
    type: 'LineString',
    coordinates: stops.value.map((s) => s.coords).filter(Boolean),
  },
}))
...
<template>
  <div data-slot="last-mile-courier-dispatch" class="w-full space-y-6">
    ...
    <Card class="overflow-hidden">
      ...
      <div class="relative h-72 w-full sm:h-80">
        <Map variant="dark" :center="[-73.982, 40.75]" :zoom="13.8" class="size-full">
          <!-- Route Polyline -->
          ...
          <MapMarker v-for="stop in stops" :key="stop.id" :lng-lat="stop.coords!" anchor="center">
          ...
          <MapMarker :lng-lat="[-73.984, 40.749]" anchor="center" class="z-30">
