...
function applyMove(board: MiniColumn[], { cardId, fromColumnId, toColumnId, toIndex }: KanbanMoveEvent) {
...
<template>
  ...
  >
    <Kanban @card-move="handleCardMove">
      <KanbanBoard>
        <KanbanColumn v-for="column in columns" :key="column.id" :id="column.id" :label="column.title">
          <KanbanColumnHeader>
          ...
          <KanbanColumnBody>
            <KanbanCard v-for="task in column.tasks" :key="task.id" :id="task.id">
  ...
  >
    <Kanban @card-move="(e) => applyMove(keyboardBoard, e)">
      <KanbanBoard>
        <KanbanColumn v-for="column in keyboardBoard" :key="column.id" :id="column.id" :label="column.label">
          <KanbanColumnHeader>
            <div class="flex items-center gap-2">
              <KanbanColumnDot :color="column.dotColor" />
          ...
          <KanbanColumnBody>
            <KanbanCard v-for="card in column.cards" :key="card.id" :id="card.id">
              <KanbanCardTitle>{{ card.title }}</KanbanCardTitle>
            </KanbanCard>
            <KanbanColumnEmpty v-if="!column.cards.length">Drop a card here</KanbanColumnEmpty>
          </KanbanColumnBody>
        </KanbanColumn>
      </KanbanBoard>
    </Kanban>
  ...
  >
    <Kanban @card-move="(e) => applyMove(reorderBoard, e)">
      <KanbanBoard>
        <KanbanColumn v-for="column in reorderBoard" :key="column.id" :id="column.id" :label="column.label">
          <KanbanColumnHeader>
            <div class="flex items-center gap-2">
              <KanbanColumnDot :color="column.dotColor" />
              <KanbanColumnTitle>{{ column.label }}</KanbanColumnTitle>
          ...
          <KanbanColumnBody>
            <KanbanCard v-for="(card, index) in column.cards" :key="card.id" :id="card.id">
              <KanbanCardTitle>{{ index + 1 }}. {{ card.title }}</KanbanCardTitle>
            </KanbanCard>
          </KanbanColumnBody>
        </KanbanColumn>
      </KanbanBoard>
  ...
  >
    <Kanban @card-move="(e) => applyMove(lockedBoard, e)">
      <KanbanBoard>
        <KanbanColumn v-for="column in lockedBoard" :key="column.id" :id="column.id" :label="column.label">
          <KanbanColumnHeader>
            <div class="flex items-center gap-2">
              <KanbanColumnDot :color="column.dotColor" />
              <KanbanColumnTitle>{{ column.label }}</KanbanColumnTitle>
          ...
          <KanbanColumnBody>
            <KanbanCard v-for="card in column.cards" :key="card.id" :id="card.id" :disabled="card.id === 'm-2'">
              <KanbanCardTitle>{{ card.title }}</KanbanCardTitle>
              <KanbanCardDescription v-if="card.id === 'm-2'">Locked by a workflow rule</KanbanCardDescription>
            </KanbanCard>
            <KanbanColumnEmpty v-if="!column.cards.length">Empty</KanbanColumnEmpty>
          </KanbanColumnBody>
        </KanbanColumn>
      </KanbanBoard>
    </Kanban>
  ...
  >
    <Kanban @card-move="(e) => applyMove(pointerOnlyBoard, e)">
      <KanbanBoard>
        <KanbanColumn v-for="column in pointerOnlyBoard" :key="column.id" :id="column.id" :label="column.label">
          <KanbanColumnHeader>
            <div class="flex items-center gap-2">
              <KanbanColumnDot :color="column.dotColor" />
              <KanbanColumnTitle>{{ column.label }}</KanbanColumnTitle>
          ...
          <KanbanColumnBody>
            <KanbanCard v-for="card in column.cards" :key="card.id" :id="card.id" :keyboard-draggable="false">
              <KanbanCardTitle>{{ card.title }}</KanbanCardTitle>
            </KanbanCard>
            <KanbanColumnEmpty v-if="!column.cards.length">Empty</KanbanColumnEmpty>
          </KanbanColumnBody>
  ...
  >
    <Kanban @card-move="(e) => applyMove(wipBoard, e)">
      <KanbanBoard>
        <KanbanColumn v-for="column in wipBoard" :key="column.id" :id="column.id" :label="column.label">
          <KanbanColumnHeader>
            <div class="flex items-center gap-2">
              <KanbanColumnDot :color="column.dotColor" />
              <KanbanColumnTitle>{{ column.label }}</KanbanColumnTitle>
              <KanbanColumnCount
                :class="column.cards.length > WIP_LIMIT ? 'bg-destructive/10 text-destructive' : undefined"
              >
  ...
  >
    <Kanban @card-move="(e) => applyMove(compactBoard, e)">
      <KanbanBoard>
        ...
        >
          <KanbanColumnHeader>
            <KanbanColumnTitle class="text-xs">{{ column.label }}</KanbanColumnTitle>
            <KanbanColumnCount :count="column.cards.length" />
          </KanbanColumnHeader>
          <KanbanColumnBody>
            <KanbanCard v-for="card in column.cards" :key="card.id" :id="card.id" class="gap-0 p-2">
              <KanbanCardTitle class="text-xs">{{ card.title }}</KanbanCardTitle>
            </KanbanCard>
          </KanbanColumnBody>
        </KanbanColumn>
      </KanbanBoard>
  ...
  >
    <Kanban @card-move="(e) => applyMove(scrollBoard, e)">
      <KanbanBoard class="max-w-full">
        <KanbanColumn v-for="column in scrollBoard" :key="column.id" :id="column.id" :label="column.label">
          <KanbanColumnHeader>
            <div class="flex items-center gap-2">
              <KanbanColumnDot :color="column.dotColor" />
              <KanbanColumnTitle>{{ column.label }}</KanbanColumnTitle>
