...
<template>
  ...
  >
    ...
    >
      ...
      >
        <p class="text-foreground text-xs font-semibold">Internet penetration</p>
        <div class="mt-1.5 flex flex-col gap-1">
          <div v-for="s in SCALE" :key="s.label" class="flex items-center gap-1.5">
            <span class="size-2.5 rounded-sm" :class="s.swatch" />
            <span class="text-muted-foreground font-mono text-[10px]">{{ s.label }}</span>
          </div>
        </div>
      </div>
    </LeafletMap>
