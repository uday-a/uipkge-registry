...
<template>
  ...
  >
    <Questionnaire
      ...
      required-message="Pick a direction before continuing."
    />
  </Story>
  ...
  <Story
    ...
    description="Listen for submit to read the answers map."
  >
    <Questionnaire
      :items="agentItems.slice(0, 1)"
      @submit="lastSubmit = $event"
    />
    <p v-if="lastSubmit" class="text-muted-foreground mt-3 font-mono text-xs">
