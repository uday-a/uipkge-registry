...
function seed(): Record<string, Task[]> {
  return {
    todo: [
      ...
      {
        id: "t2",
        title: "Schema for analytics events",
        who: "Marcus Rivera",
        initials: "MR",
      },
      {
    ...
    doing: [
      {
        id: "t4",
        title: "Rework pricing page hero",
        who: "Karan Nair",
        initials: "KN",
      },
      {
        id: "t5",
        title: "Wire OAuth refresh path",
        who: "Sundar Krishnan",
        initials: "SK",
      },
    ],
    review: [
      ...
      {
        id: "t7",
        title: "Q1 OKR alignment doc",
        who: "Sarah Connor",
        initials: "SC",
      },
      {
        id: "t8",
        title: "Migrate static images to CDN",
        who: "Pooja Iyer",
        initials: "PI",
      },
    ],
...
const b1 = useBoard<Task>({
  lanes: lanes1 as unknown as Ref<Record<string, Task[]>>,
});
...
const lanes2 = ref<Record<string, Task[]>>({
  list: [
    {
      id: "s1",
      title: "Tighten dashboard KPI tiles",
      who: "Priya",
      initials: "P",
    },
    {
...
const b2 = useBoard<Task>({
  lanes: lanes2 as unknown as Ref<Record<string, Task[]>>,
});
...
const lanes3 = ref<Record<string, Task[]>>({
  backlog: [
    {
      id: "d1",
      title: "Sketch new dashboard hero",
      who: "Karan",
      initials: "K",
    },
    {
      id: "d2",
      title: "Audit time-off mock data",
      who: "Marcus",
      initials: "M",
    },
  ...
  shipped: [
    { id: "d4", title: "Onboarding tour v2", who: "Priya", initials: "P" },
  ],
});
const b3 = useBoard<Task>({
  lanes: lanes3 as unknown as Ref<Record<string, Task[]>>,
});

// ─── Story 4: disabled lane (can't drop here) ────────────────────────────
const lanes4 = ref(seed());
const b4 = useBoard<Task>({
  lanes: lanes4 as unknown as Ref<Record<string, Task[]>>,
});

// ─── Story 5: disabled card (can't drag this one) ────────────────────────
const lanes5 = ref(seed());
const b5 = useBoard<Task>({
  lanes: lanes5 as unknown as Ref<Record<string, Task[]>>,
});

// ─── Story 6: per-card allowedLanes (Story Six only lets t1 go to review) ─
const lanes6 = ref(seed());
const b6 = useBoard<Task>({
  lanes: lanes6 as unknown as Ref<Record<string, Task[]>>,
});
...
const b8 = useBoard<Task>({
  lanes: lanes8 as unknown as Ref<Record<string, Task[]>>,
});
...
const lanes9 = ref<Record<string, Task[]>>({
  inbox: [
    {
      id: "p1",
      title: "New ticket — exporter timeouts",
      who: "System",
      initials: "SY",
    },
    {
      id: "p2",
      title: "New ticket — sso for Acme",
      who: "System",
      initials: "SY",
    },
  ],
...
const b9 = useBoard<Task>({
  lanes: lanes9 as unknown as Ref<Record<string, Task[]>>,
});
...
<template>
    ...
    >
      ...
      >
        <BoardLaneHeader>
          ...
          <Button variant="ghost" size="icon" class="size-7" aria-label="Add"
            ><Plus class="size-3.5"
          /></Button>
        </BoardLaneHeader>
        <BoardLaneBody>
        ...
        <BoardLaneEmpty :when="(lanes1[s]?.length ?? 0) === 0"
          >Drag a card here.</BoardLaneEmpty
        >
      </BoardLane>
    </Board>
  </Story>
      ...
      >
        ...
        <BoardLaneBody>
          <BoardCard
            ...
            :id="t.id"
            @dragstart="
              (e: DragEvent) => b2.handlers.onDragStart(e, t.id, 'list')
            "
      ...
      >
        ...
        <BoardLaneBody>
          <BoardCard
            ...
            :id="t.id"
            @dragstart="
              (e: DragEvent) => b3.handlers.onDragStart(e, t.id, k as string)
            "
      ...
      >
        <BoardLaneHeader>
          <div class="flex items-center gap-2">
            <span :class="['size-2 rounded-full', dot[s]]" />
            <span class="text-sm font-semibold">{{ labels[s] }}</span>
            <Badge v-if="s === 'done'" variant="secondary" class="text-xs"
              >locked</Badge
            >
            <span
              class="bg-muted text-muted-foreground rounded-full px-2 py-0.5 text-xs font-semibold tabular-nums"
    ...
    >
      ...
      >
        ...
        <BoardLaneBody>
          ...
          >
            <p class="truncate text-sm font-medium">{{ t.title }}</p>
            <p v-if="t.id === 't1'" class="text-info mt-0.5 text-xs">
              → only Review accepts this
            </p>
          </BoardCard>
        </BoardLaneBody>
      </BoardLane>
  ...
  >
    ...
    <p class="text-muted-foreground mt-2 text-xs">
      Tip: cmd/ctrl/shift+click to add a card to the selection. Selected:
      {{ b8.state.selectedIds.size }}
    </p>
  </Story>
  ...
  >
    ...
    >
      ...
      >
        ...
        <BoardLaneEmpty :when="(lanes9[k]?.length ?? 0) === 0"
          >Empty.</BoardLaneEmpty
        >
      </BoardLane>
    </Board>
  </Story>
        ...
        >
          ...
          <BoardLaneBody>
            <BoardCard
              ...
              :id="t.id"
              @dragstart="
                (e: DragEvent) => b10.handlers.onDragStart(e, t.id, s)
              "
      ...
      <div class="rounded-md border p-3">
        ...
        <ul v-if="log.length" class="space-y-1">
          <li
            v-for="(line, i) in log"
            :key="i"
            class="text-foreground/80 font-mono text-xs tabular-nums"
          >
            {{ line }}
          </li>
        </ul>
        <p v-else class="text-muted-foreground text-xs">
          Drag a card to log an event.
        </p>
      </div>
    </div>
  </Story>
  ...
  >
    <div
      class="text-muted-foreground bg-muted/30 rounded-md border p-3 text-xs"
    >
      Focus a card in any board above and use
      <kbd
        class="border-border bg-background rounded border px-1 py-0.5 font-mono text-xs"
        >Space</kbd
      >
      +
      <kbd
