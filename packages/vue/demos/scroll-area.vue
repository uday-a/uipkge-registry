...
<template>
  ...
  >
    <ScrollArea
      class="border-border max-w-2xl rounded-md border whitespace-nowrap"
    >
      <div class="flex w-max gap-4 p-4">
        <figure v-for="f in figures" :key="f.id" class="shrink-0">
          <div
            class="bg-muted text-muted-foreground grid size-32 place-items-center rounded-md text-xs"
          >
            {{ f.title }}
          </div>
    ...
    <ScrollArea class="border-border h-64 max-w-md rounded-md border">
      <div class="grid w-[640px] grid-cols-8 gap-2 p-4 font-mono text-xs">
        <div
          v-for="n in grid"
          :key="n"
          class="bg-muted grid aspect-square place-items-center rounded"
        >
          {{ n }}
        </div>
      </div>
