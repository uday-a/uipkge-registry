...
import { MoreHorizontal, Plus, RefreshCw, Settings } from 'lucide-vue-next'
</script>
...
<template>
    ...
    <SectionCard title="Team members" description="People with access to this workspace.">
      <template #header-action>
        <div class="flex items-center gap-2">
          <Button variant="outline" size="sm"><RefreshCw class="size-3.5" /> Sync</Button>
          <Button size="sm"><Plus class="size-3.5" /> Invite</Button>
          <Button variant="ghost" size="icon-sm"><MoreHorizontal class="size-4" /></Button>
        </div>
      </template>
      <ul class="space-y-2 text-sm">
        <li class="flex justify-between">
          <span>Alice Anderson</span><span class="text-muted-foreground">Owner</span>
        </li>
        <li class="flex justify-between"><span>Bob Bailey</span><span class="text-muted-foreground">Admin</span></li>
        <li class="flex justify-between"><span>Carol Chen</span><span class="text-muted-foreground">Member</span></li>
      </ul>
    </SectionCard>
  </Story>
  ...
  >
    <div class="space-y-4">
      ...
      <SectionCard title="Danger zone" description="Irreversible actions for this account.">
        <template #header-action>
  ...
  >
    <SectionCard title="Personal info" description="Used on invoices and team emails.">
      <template #header-action>
