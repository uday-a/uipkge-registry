  ...
  >
    <div class="bg-card h-[260px] w-full max-w-lg rounded-lg border">
      <OverlayScroll :thumb-width="3" :thumb-offset="1" class="h-full">
        <ul class="divide-y">
          <li v-for="(f, i) in files" :key="i" class="flex items-center gap-3 px-3 py-2 text-sm">
            <span
              class="inline-flex size-5 items-center justify-center rounded font-mono text-xs font-semibold"
              :class="{
                'bg-blue-500/15 text-blue-600 dark:text-blue-400': f.status === 'M',
                'bg-emerald-500/15 text-emerald-600 dark:text-emerald-400': f.status === 'A',
                'bg-red-500/15 text-red-600 dark:text-red-400': f.status === 'D',
              }"
              >{{ f.status }}</span
            >
  ...
  >
    <div class="space-y-3">
      <div class="flex gap-2">
        <Button size="sm" variant="outline" @click="scrollToTop">Scroll to top</Button>
      ...
      <div class="bg-card h-[240px] w-full max-w-md rounded-lg border">
        <OverlayScroll ref="externalRef" class="h-full p-4">
          <p v-for="i in 30" :key="i" class="text-foreground/90 mb-3 text-sm">
            Paragraph {{ i }}. The parent controls scroll position via the exposed `scrollerEl`.
          </p>
        </OverlayScroll>
      </div>
    </div>
  ...
  >
    <div class="space-y-3">
      <div class="flex items-center gap-2">
        <Button size="sm" variant="outline" @click="appendRows(20)">+20 rows</Button>
        <Button size="sm" :variant="autoGrow ? 'default' : 'outline'" @click="toggleAutoGrow">
      ...
      <div class="bg-card h-[280px] w-full max-w-lg rounded-lg border">
        <OverlayScroll class="h-full">
          <ul class="divide-y">
            <li v-for="row in dynamicRows" :key="row.id" class="flex items-center justify-between px-3 py-2 text-sm">
              <span>{{ row.title }}</span>
              <span
                class="rounded px-2 py-0.5 font-mono text-xs font-semibold"
                :class="{
                  'bg-amber-500/15 text-amber-600 dark:text-amber-400': row.detail === 'queued',
                  'bg-blue-500/15 text-blue-600 dark:text-blue-400': row.detail === 'in-flight',
                  'bg-emerald-500/15 text-emerald-600 dark:text-emerald-400': row.detail === 'done',
                  'bg-red-500/15 text-red-600 dark:text-red-400': row.detail === 'retrying',
                }"
                >{{ row.detail }}</span
              >
            </li>
