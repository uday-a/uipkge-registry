...
import { ref } from 'vue'
import { Gantt, GanttHeader, GanttTree, GanttTimeline, type GanttTask } from '@/components/ui/gantt'
...
<template>
  ...
  >
    <Gantt :tasks="sampleTasks.slice(0, 4)" scale="day" :row-height="32" :tree-width="220" class="h-64">
  ...
  >
    <div class="space-y-4">
      <Gantt :tasks="sampleTasks" scale="day" class="h-80" @task-click="selectedTask = $event">
      ...
      <div v-if="selectedTask" class="border-border bg-card flex items-center justify-between rounded-lg border p-4">
        <div>
