...
<template>
  ...
  <Story title="With pagination" description="pagination=true uses page size 10. Pass {pageSize} to override.">
    <Transfer v-model:target-keys="target3" :data-source="big" :pagination="{ pageSize: 8 }" show-search />
  </Story>
  ...
  >
    <Transfer v-model:target-keys="target6" :data-source="data" draggable show-search />
  </Story>
  ...
  >
    <Transfer v-model:target-keys="target7" :data-source="data" :selectable="false" draggable />
  </Story>

  <Story title="Custom titles + footer" description="titles prop labels each side; footer slots add per-side actions.">
    <Transfer v-model:target-keys="target5" :data-source="data" :titles="['Available', 'Selected']">
      <template #footer-left>
        <span class="text-muted-foreground text-xs">Tip: enable `draggable` for DnD.</span>
      </template>
    </Transfer>
  </Story>
</template>
