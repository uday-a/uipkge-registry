...
<template>
  ...
  >
    <div class="space-y-3">
      <Alert>
        ...
        <AlertDescription
          >Read this carefully — it explains a non-obvious
          behavior.</AlertDescription
        >
      </Alert>
      <Alert>
  ...
  <Story
    title="Dismissible"
    description="Pair the alert with v-if and a close button to make it dismissible. The Alert primitive itself is stateless; the host owns the visibility."
  >
    <div class="space-y-2">
      <Alert v-if="!dismissed" class="relative pr-12">
        <Info class="text-info size-4" />
        <AlertTitle>New release available</AlertTitle>
        <AlertDescription
          >v2.1.0 ships with the new theming engine. See the
          changelog.</AlertDescription
        >
        <Button
          variant="ghost"
          size="icon-sm"
