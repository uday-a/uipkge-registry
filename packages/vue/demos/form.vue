...
import { Loader2, Mail, Smartphone, KeyRound, CheckCircle2, AlertCircle } from 'lucide-vue-next'

// Helpers showcase
const validationDemoStatus = ref<'error' | 'warning' | 'success' | undefined>(undefined)
...
const signup2Form = useForm({
  defaultValues: { firstName: '', lastName: '', suEmail: '', suPhone: '', suPass: '', suConfirm: '' },
  onSubmit: () => {},
})
...
const profile2Form = useForm({
  defaultValues: { pfn2: '', pln2: '', pem2: '', pph2: '', pjob2: '', pdept2: '', pbio2: '' },
  onSubmit: () => {},
})
...
<template>
  ...
  >
    <Form :form="helpersForm" class="max-w-lg space-y-6">
      <FormSection title="Account Details" description="Enter your account information below." bordered>
        ...
        <div class="flex flex-wrap gap-2">
          <Button type="button" size="sm" variant="outline" @click="setValidationDemo('error')">
            <AlertCircle class="mr-1 size-3.5" /> Error
          </Button>
          <Button type="button" size="sm" variant="outline" @click="setValidationDemo('warning')">
            <AlertCircle class="mr-1 size-3.5 text-amber-500" /> Warning
          </Button>
          <Button type="button" size="sm" variant="outline" @click="setValidationDemo('success')">
  ...
  <!-- Horizontal Layout -->
  <Story title="Horizontal Layout" description="FormItem supports horizontal layout with configurable label width.">
    <Form :form="horizontalForm" class="max-w-lg space-y-4">
      <FormField name="hName" v-slot="{ componentField }">
        <FormItem label="Full Name" layout="horizontal" required>
          <FormControl><Input placeholder="Jane Doe" v-bind="componentField" /></FormControl>
        </FormItem>
      </FormField>
      <FormField name="hEmail" v-slot="{ componentField }">
        <FormItem label="Email" layout="horizontal" required>
          <FormControl><Input type="email" placeholder="jane@example.com" v-bind="componentField" /></FormControl>
        </FormItem>
      </FormField>
  ...
  >
    <Form :form="loginForm" class="max-w-sm space-y-4">
      <FormField name="email" :validators="{ onSubmit: loginEmailSchema }" v-slot="{ componentField }">
      ...
      <FormField name="password" :validators="{ onSubmit: loginPasswordSchema }" v-slot="{ componentField }">
        <FormItem label="Password">
          <FormControl>
            <Input type="password" placeholder="••••••••" show-password-toggle v-bind="componentField" />
          </FormControl>
          <FormMessage />
        </FormItem>
      </FormField>
      ...
      <FormStatus v-if="loginSuccess" status="success" message="Login successful! Redirecting..." />
  ...
  <Story title="Login — 2 Column" description="Compact side-by-side layout for wider containers.">
    <Form :form="login2Form" class="max-w-xl">
      ...
      <div class="mt-4 flex items-center justify-between">
        <label class="flex items-center gap-2 text-sm"> <Checkbox :model-value="false" /> Remember me </label>
  ...
  <!-- SIGN UP -->
  <Story title="Sign Up — 1 Column" description="Full registration form with validation.">
    <Form v-if="!signupSuccess" :form="signupForm" class="max-w-sm space-y-4">
      ...
      <FormField name="signupPassword" v-slot="{ componentField }">
        ...
        >
          <FormControl>
            <Input type="password" placeholder="Min 8 characters" show-password-toggle v-bind="componentField" />
          </FormControl>
        </FormItem>
      </FormField>
      <FormField name="confirmPassword" v-slot="{ componentField }">
        <FormItem label="Confirm Password" required>
          <FormControl>
            <Input type="password" placeholder="Repeat password" show-password-toggle v-bind="componentField" />
          </FormControl>
        </FormItem>
      </FormField>
    ...
    <div v-else class="flex max-w-sm flex-col items-center gap-3 rounded-lg border p-6 text-center">
      ...
      <Button variant="outline" size="sm" @click="signupSuccess = false">Back to form</Button>
    </div>
  </Story>

  <Story title="Sign Up — 2 Column" description="Two-column grid for denser layouts.">
      ...
      <div class="grid grid-cols-2 gap-4">
        <FormField name="firstName" v-slot="{ componentField }">
          <FormItem label="First Name" required>
            <FormControl><Input placeholder="Jane" v-bind="componentField" /></FormControl>
          </FormItem>
        </FormField>
        <FormField name="lastName" v-slot="{ componentField }">
          <FormItem label="Last Name" required>
            <FormControl><Input placeholder="Doe" v-bind="componentField" /></FormControl>
          </FormItem>
        </FormField>
        ...
        <FormField name="suPhone" v-slot="{ componentField }">
          <FormItem label="Phone">
            <FormControl><Input type="tel" placeholder="+1 (555) 000-0000" v-bind="componentField" /></FormControl>
          </FormItem>
        </FormField>
        <FormField name="suPass" v-slot="{ componentField }">
          <FormItem label="Password" required>
            <FormControl><Input type="password" show-password-toggle v-bind="componentField" /></FormControl>
          </FormItem>
        </FormField>
  ...
  <Story title="Sign Up — 3 Column" description="Three-column grid for maximum density.">
    <Form :form="signup3Form" class="max-w-3xl space-y-4">
      <div class="grid grid-cols-3 gap-4">
        ...
        <FormField name="ph3" v-slot="{ componentField }">
          <FormItem label="Phone"
            ><FormControl><Input type="tel" v-bind="componentField" /></FormControl
          ></FormItem>
        </FormField>
  ...
  <!-- OTP -->
  <Story title="OTP Verification" description="6-digit code entry with resend and error states.">
    <div class="max-w-sm space-y-4">
      ...
      <FormStatus v-if="otpError" status="error" message="Invalid code. Try again." />
      <FormStatus v-if="otpSuccess" status="success" message="Verified successfully!" />
  ...
  <!-- MFA -->
  <Story title="MFA Setup" description="Multi-factor authentication method selection and code entry.">
    <div class="max-w-sm space-y-4">
      ...
      <div v-if="!mfaSuccess" class="flex items-center gap-3">
        ...
        <Button size="sm" :disabled="mfaSubmitting || mfaCode.length < 6" @click="onMfaSubmit">
  ...
  <!-- PASSWORD RESET -->
  <Story title="Password Reset" description="Two-step flow: request link, then set new password.">
    <div class="max-w-sm space-y-4">
      <!-- Step 1: Request -->
      <div v-if="resetStep === 'request'">
        ...
        <Form :form="resetRequestForm" class="space-y-3">
          <FormField name="resetEmail" :validators="{ onSubmit: resetEmailSchema }" v-slot="{ componentField }">
            <FormItem label="Email" required>
              <FormControl>
                <Input type="email" placeholder="you@example.com" v-bind="componentField" />
              </FormControl>
      ...
      <div v-else-if="!resetSuccess">
        ...
        <Form :form="resetConfirmForm" class="space-y-3">
          <FormField name="newPassword" :validators="{ onSubmit: newPasswordSchema }" v-slot="{ componentField }">
            <FormItem label="New Password" required>
              <FormControl>
                <Input type="password" show-password-toggle v-bind="componentField" />
              </FormControl>
              <FormMessage />
            </FormItem>
          </FormField>
          <FormField
            ...
            :validators="{
              ...
              onSubmit: ({ value, fieldApi }) =>
                value === fieldApi.form.getFieldValue('newPassword') ? undefined : 'Passwords do not match',
            }"
      ...
      <!-- Success -->
      <div v-else class="flex flex-col items-center gap-3 rounded-lg border p-6 text-center">
        <CheckCircle2 class="size-10 text-emerald-500" />
  ...
  <!-- PROFILE EDIT -->
  <Story title="Profile Edit — 1 Column" description="Single-column profile form.">
    <Form v-if="!profileSuccess" :form="profileForm" class="max-w-md space-y-4">
      ...
      <FormField name="profileBio" v-slot="{ componentField }">
        <FormItem label="Bio">
          <FormControl><Textarea :rows="3" placeholder="Tell us about yourself" v-bind="componentField" /></FormControl>
        </FormItem>
      </FormField>
    ...
    <div v-else class="flex max-w-md flex-col items-center gap-3 rounded-lg border p-6 text-center">
      <CheckCircle2 class="size-10 text-emerald-500" />
      <h3 class="text-lg font-semibold">Profile updated</h3>
      <Button variant="outline" size="sm" @click="profileSuccess = false">Edit again</Button>
    </div>
  </Story>

  <Story title="Profile Edit — 2 Column" description="Two-column layout for profile forms.">
  ...
  <Story title="Profile Edit — 3 Column" description="Three-column dense profile layout.">
      ...
      <div class="grid grid-cols-3 gap-4">
        ...
        <FormField name="pweb3" v-slot="{ componentField }">
          <FormItem label="Website"
            ><FormControl><Input type="url" v-bind="componentField" /></FormControl
          ></FormItem>
        </FormField>
