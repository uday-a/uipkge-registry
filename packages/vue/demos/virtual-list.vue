...
const listRef = ref<{ scrollToIndex: (i: number, opts?: { align?: 'start' | 'center' | 'end' }) => void } | null>(null)
...
<template>
  ...
  >
    <VirtualList :items="fixed" :item-size="40" :height="400" class="rounded-md border">
  ...
  <Story title="Dynamic size" description="itemSize as a function returns per-item heights from the data.">
    <VirtualList :items="dynamic" :item-size="(item) => item.size" :height="400" class="rounded-md border">
      <template #default="{ item }">
  ...
  <Story title="Imperative scrollToIndex" description="Use a template ref to jump to any index, with align options.">
    <div class="space-y-2">
      ...
      <VirtualList ref="listRef" :items="fixed" :item-size="32" :height="320" class="rounded-md border">
  ...
  <Story title="Horizontal" description="direction='horizontal' switches to a horizontal viewport.">
    <VirtualList :items="horizontal" :item-size="80" :height="120" direction="horizontal" class="rounded-md border">
      <template #default="{ item }">
