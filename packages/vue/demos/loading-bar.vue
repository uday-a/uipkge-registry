  ...
  <div class="space-y-8">
    ...
    >
      <div class="flex flex-wrap gap-2">
        <Button @click="simulatePageLoad">
          <Loader2 class="mr-2 size-4 animate-spin" :class="pageBar.loading.value ? 'opacity-100' : 'opacity-0'" />
          Load dashboard
        </Button>
    ...
    >
      <Card class="max-w-md">
        ...
        <CardContent class="space-y-4">
          ...
          <Button class="w-full" :disabled="formStatus === 'saving'" @click="submitForm">
            <Save class="mr-2 size-4" />
            {{ formStatus === 'saving' ? 'Saving…' : formStatus === 'done' ? 'Saved!' : 'Save changes' }}
          </Button>
          <p v-if="formStatus === 'done'" class="text-xs text-emerald-600">Your billing details were updated.</p>
        </CardContent>
      </Card>
    </Story>
    ...
    >
      <div class="max-w-md space-y-3">
        ...
        <input v-model.number="manualValue" type="range" min="0" max="100" class="w-full" />
    ...
    >
      <Button variant="outline" @click="runBottomBar">
        <Loader2 class="mr-2 size-4 animate-spin" :class="bottomBar.loading.value ? 'opacity-100' : 'opacity-0'" />
        Sync in background
      </Button>
