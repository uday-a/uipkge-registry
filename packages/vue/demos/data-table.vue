...
import { ChevronDown, ChevronRight, MoreHorizontal, Pencil, Copy, Trash2 } from 'lucide-vue-next'
...
function loadMoreInfinite() {
  ...
  window.setTimeout(() => {
    infiniteRows.value = data.slice(0, Math.min(infiniteRows.value.length + 4, data.length))
...
const plainColumns: ColumnDef<Employee>[] = [
  ...
  { accessorKey: 'department', header: 'Department' },
  {
    ...
    cell: ({ row }) => {
      ...
      const variant = s === 'active' ? 'default' : s === 'on_leave' ? 'secondary' : 'outline'
      return h(Badge, { variant, class: 'capitalize' }, () => s.replace('_', ' '))
    },
  },
]
    ...
    h(DropdownMenu, null, {
      default: () => [
        h(DropdownMenuTrigger, { asChild: true }, () =>
          h(
            Button,
            { variant: 'ghost', size: 'icon-sm', class: 'size-8 -my-1', 'aria-label': 'Open row actions' },
...
const sortableColumns: ColumnDef<Employee>[] = [
  selectColumn,
  { accessorKey: 'name', header: ({ column }) => h(DataTableColumnHeader, { column, label: 'Name' }) },
  { accessorKey: 'email', header: ({ column }) => h(DataTableColumnHeader, { column, label: 'Email' }) },
  { accessorKey: 'role', header: ({ column }) => h(DataTableColumnHeader, { column, label: 'Role' }) },
  { accessorKey: 'department', header: ({ column }) => h(DataTableColumnHeader, { column, label: 'Department' }) },
  {
    accessorKey: 'status',
    header: ({ column }) => h(DataTableColumnHeader, { column, label: 'Status' }),
    cell: ({ row }) => {
      const s = row.original.status
      const variant = s === 'active' ? 'default' : s === 'on_leave' ? 'secondary' : 'outline'
      return h(Badge, { variant, class: 'capitalize' }, () => s.replace('_', ' '))
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return h('div', { class: 'flex items-center gap-2' }, [
        ...
        h('span', { class: 'text-muted-foreground text-xs tabular-nums' }, `${yrs}y`),
      ])
    },
  },
  { accessorKey: 'department', header: 'Department' },
...
const filters = [
  ...
  { column: 'status', label: 'Status', type: 'multiselect' as const, options: ['active', 'on_leave', 'terminated'] },
]

const dateFilters = [...filters, { column: 'hired', label: 'Hired', type: 'date' as const }]

const dateColumns: ColumnDef<Employee>[] = [
  ...sortableColumns.slice(0, -1),
  { accessorKey: 'hired', header: ({ column }) => h(DataTableColumnHeader, { column, label: 'Hired' }) },
  actionsColumn,
]
...
<template>
  <div class="space-y-4">
    ...
    >
      <DataTable :columns="sortableColumns" :data="data" :filters="filters" :enable-search="false" />
    </Story>
    ...
    >
      <DataTable :columns="sortableColumns" :data="data" filter-column="email" :enable-column-visibility="false" />
    </Story>
    ...
    >
      <DataTable :columns="sortableColumns" :data="data" filter-column="email" :enable-pagination="false" />
    </Story>
    ...
    >
      <DataTable :columns="sortableColumns" :data="data" hide-toolbar :enable-pagination="false" />
    </Story>
    ...
    >
      <DataTable :columns="sortableColumns" :data="data" filter-column="email" sticky-header max-height="280px" />
    </Story>

    <Story title="Density: compact" description="Tighter row padding for log-style or analytics views.">
      <DataTable :columns="sortableColumns" :data="data" filter-column="email" density="compact" />
    </Story>
    ...
    >
      <DataTable :columns="sortableColumns" :data="data" filter-column="email" density="comfortable" />
    </Story>
    ...
    >
      <DataTable
        ...
        :data="data"
        filter-column="email"
        :default-column-pinning="{ left: ['select', 'name'], right: ['actions'] }"
      />
    </Story>
    ...
    >
      <DataTable :columns="sortableColumns" :data="data" filter-column="email" enable-resize />
    </Story>
    ...
    >
      <DataTable :columns="sortableColumns" :data="data" :filters="filters" filter-column="email" enable-export />
    </Story>
    ...
    >
      <DataTable :columns="richColumns" :data="data" :enable-pagination="false" />
    </Story>
      ...
      >
        <template #custom-filters>
          <div class="border-border ml-2 flex items-center gap-2 border-l px-2">
            <Switch :model-value="onlyActive" @update:model-value="onlyActive = $event" id="only-active" />
    ...
    >
      <DataTable :columns="sortableColumns" :data="data" filter-column="email" enable-reorder />
    </Story>
          ...
          <tr class="font-medium">
            <td colspan="7" class="px-3 py-3 text-sm">
              ...
              <span class="text-muted-foreground ml-2"
                >· {{ rows.filter((r: any) => r.original.status === 'active').length }} active</span
              >
            </td>
          </tr>
        </template>
    ...
    >
      <DataTable :columns="sortableColumns" :data="[]" filter-column="email" loading />
    </Story>
    ...
    >
      <DataTable :columns="dateColumns" :data="data" :filters="dateFilters" filter-column="email" />
    </Story>
    ...
    >
      <DataTable :columns="sortableColumns" :data="data" filter-column="email" enable-keyboard-navigation />
    </Story>
    ...
    >
      <div class="bg-muted/30 rounded-lg p-3">
        <DataTable :columns="sortableColumns" :data="data" filter-column="email" borderless="full" />
      </div>
    </Story>
    ...
    >
      <DataTable :columns="sortableColumns" :data="data" filter-column="email" bulk-action-position="inline">
        <template #bulk-actions="{ rows, clear }">
          <Button size="sm" variant="outline" @click="announceAction(`Exporting ${rows.length} rows`)">Export</Button>
          <Button
            size="sm"
            variant="destructive"
            @click="
