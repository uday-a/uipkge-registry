...
const customPresets = [
  ...
  {
    label: 'Last 7 Days',
    value: { start: todayD.subtract({ days: 6 }).toString(), end: todayD.toString() },
  },
  {
    label: 'Last 30 Days',
    value: { start: todayD.subtract({ days: 29 }).toString(), end: todayD.toString() },
  },
  {
    label: 'This Month',
...
const categorizedPresets = [
  ...
  { label: 'Tomorrow', value: todayD.add({ days: 1 }).toString(), category: 'Quick' },
  { label: 'Next Week', value: todayD.add({ weeks: 1 }).toString(), category: 'Future' },
  { label: 'Next Month', value: todayD.add({ months: 1 }).toString(), category: 'Future' },
]
...
<template>
  ...
  <Story title="Two Months" description="Display two months side by side for easier range selection.">
    <DatePicker v-model="range" type="range" :number-of-months="2" class="max-w-sm" />
  </Story>

  <Story title="Week Range" description="Select ranges of full weeks.">
    <DatePicker v-model="weekRange" type="range" picker="week" class="max-w-sm" />
  </Story>

  <Story title="Month Range" description="Select ranges of months.">
    <DatePicker v-model="monthRange" type="range" picker="month" class="max-w-sm" />
  </Story>

  <Story title="Quarter Range" description="Select ranges of quarters.">
    <DatePicker v-model="quarterRange" type="range" picker="quarter" class="max-w-sm" />
  </Story>

  <Story title="Year Range" description="Select ranges of years.">
    <DatePicker v-model="yearRange" type="range" picker="year" class="max-w-sm" />
  </Story>
  ...
  <Story title="Placement" description="Control where the popover appears relative to the trigger.">
    <div class="flex max-w-md flex-wrap gap-3">
      <DatePicker v-model="placementDate" placement="topLeft" placeholder="topLeft" />
      <DatePicker v-model="placementDate" placement="top" placeholder="top" />
      <DatePicker v-model="placementDate" placement="topRight" placeholder="topRight" />
      <DatePicker v-model="placementDate" placement="bottomLeft" placeholder="bottomLeft" />
      <DatePicker v-model="placementDate" placement="bottom" placeholder="bottom" />
      <DatePicker v-model="placementDate" placement="bottomRight" placeholder="bottomRight" />
      <DatePicker v-model="placementDate" placement="left" placeholder="left" />
      <DatePicker v-model="placementDate" placement="right" placeholder="right" />
    </div>
  </Story>

  <!-- PRESETS -->
  <Story title="Range with Presets" description="Quick-select common date ranges with preset shortcuts.">
    <DatePicker v-model="presetRange" type="range" :presets="customPresets" class="max-w-sm" />
  </Story>

  <Story title="Single with Presets" description="Preset shortcuts work for single-date mode too.">
    <DatePicker v-model="presetSingle" :presets="categorizedPresets" class="max-w-xs" />
  </Story>
  ...
  <Story title="Format Options" description="Different date display formats.">
    <div class="flex max-w-xs flex-col gap-3">
      <DatePicker v-model="formatDate" format="short" placeholder="Short format" />
      <DatePicker v-model="formatDate" format="medium" placeholder="Medium format" />
      <DatePicker v-model="formatDate" format="long" placeholder="Long format" />
      <DatePicker v-model="formatDate" format="full" placeholder="Full format" />
    </div>
  </Story>
  ...
  <Story title="Min / Max Dates" description="Restrict selection to a date range.">
    <DatePicker v-model="rangeDate" :min-value="minDate" :max-value="maxDate" class="max-w-xs" />
  </Story>

  <Story title="Disabled Date" description="Disable specific dates via a function (weekends disabled here).">
    <DatePicker v-model="disabledDate" :disabled-date="disabledDateFn" class="max-w-xs" />
  </Story>

  <Story title="Disabled Time" description="Disable specific hours, minutes, or seconds.">
    <DatePicker v-model="disabledTimeDate" show-time :disabled-time="disabledTimeFn" class="max-w-xs" />
  </Story>
  ...
  <Story title="Range with Confirm" description="Range selections require confirmation.">
    <DatePicker v-model="confirmRange" type="range" need-confirm class="max-w-sm" />
  </Story>

  <Story title="Status States" description="Error and warning validation states.">
    <div class="flex max-w-xs flex-col gap-3">
      <DatePicker v-model="statusDate" status="error" placeholder="Error state" />
      <DatePicker v-model="statusDate" status="warning" placeholder="Warning state" />
    </div>
  </Story>

  <Story title="Range Status" description="Validation states for range picker.">
    <div class="flex max-w-sm flex-col gap-3">
      <DatePicker v-model="statusRange" type="range" status="error" placeholder="Error state" />
      <DatePicker v-model="statusRange" type="range" status="warning" placeholder="Warning state" />
    </div>
  </Story>
  ...
  >
    <Sheet v-model:open="sheetOpen">
      <SheetTrigger as-child>
        <Button variant="outline" class="w-full max-w-xs justify-start font-normal">
          {{
            sheetDate
      ...
      <SheetContent side="bottom" class="rounded-t-xl">
        ...
        <div class="flex justify-center py-2">
          <Calendar v-model="sheetDate" class="rounded-md border" @update:model-value="sheetOpen = false" />
        </div>
      </SheetContent>
    </Sheet>
  </Story>
