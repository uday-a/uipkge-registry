...
import { ChevronRight, ExternalLink, Bell, Lock, CreditCard, User, Sparkles } from 'lucide-vue-next'
...
<template>
  ...
  >
    <List class="max-w-md">
      <ListItem v-for="i in settings" :key="i.id" class="flex items-center gap-3">
        <component :is="i.icon" class="text-muted-foreground size-4" />
        <div class="flex-1">
  ...
  <Story title="With subheaders" description="ListSubheader groups items into labeled sections inside a single list.">
    <List class="max-w-md">
      <ListSubheader>Account</ListSubheader>
      <ListItem class="flex items-center gap-2"><User class="text-muted-foreground size-4" />Profile</ListItem>
      <ListItem class="flex items-center gap-2"><Lock class="text-muted-foreground size-4" />Security</ListItem>
      <ListSubheader>Workspace</ListSubheader>
      <ListItem class="flex items-center gap-2"><Bell class="text-muted-foreground size-4" />Notifications</ListItem>
      <ListItem class="flex items-center gap-2"><CreditCard class="text-muted-foreground size-4" />Billing</ListItem>
    </List>
  </Story>
  ...
  <Story title="Active state" description="Set active on a ListItem to highlight the current selection.">
    <List class="max-w-md">
      ...
      <ListItem :active="true">Drafts</ListItem>
  ...
  <Story title="Disabled state" description="Disabled items are dimmed and ignore pointer events.">
    <List class="max-w-md">
      ...
      <ListItem :disabled="true">Unavailable (disabled)</ListItem>
      <ListItem>Available</ListItem>
      <ListItem :disabled="true">Coming soon</ListItem>
    </List>
  </Story>

  <Story title="Anchor links" description="Render items as anchors by setting the as prop on List and ListItem.">
    <List as="ul" class="max-w-md">
      <ListItem v-for="d in docs" :key="d.label" as="a" :href="d.href" class="flex items-center justify-between">
  ...
  >
    <List class="max-w-md space-y-1">
      <ListItem class="border-border hover:bg-muted/40 rounded-lg border p-2.5">
        <ListItemMedia>
      ...
      <ListItem class="border-border hover:bg-muted/40 rounded-lg border p-2.5">
        ...
        <ListItemActions>
          <Badge variant="outline" class="text-success border-success/30">Active</Badge>
        </ListItemActions>
      </ListItem>
    </List>
  </Story>
