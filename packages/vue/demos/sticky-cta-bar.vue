...
import StickyCtaBar from '@/components/blocks/sticky-cta-bar/StickyCtaBar.vue'
// StickyCtaBar is the block file the user installs. Open
// `components/blocks/StickyCtaBar.vue` after install to change the reveal
...
<template>
  ...
  >
    <div class="mx-auto max-w-2xl space-y-6 pb-40">
      <p v-for="(paragraph, index) in paragraphs" :key="index" class="text-muted-foreground leading-relaxed">
        {{ paragraph }}
      </p>
      <p
        v-for="(paragraph, index) in [...paragraphs, ...paragraphs, ...paragraphs]"
        :key="`repeat-${index}`"
