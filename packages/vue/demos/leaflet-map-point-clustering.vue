...
function drillInto(c: { count: number; lngLat: [number, number] }) {
  if (c.count < 2) return
  mapRef.value?.flyTo({ center: c.lngLat, zoom: 5.5, duration: 900 })
}
function resetView() {
  mapRef.value?.flyTo({ center: [-98.5795, 39.8283], zoom: 3.5, duration: 900 })
}
</script>
...
<template>
  ...
  >
    <div class="space-y-3">
      ...
      >
        <LeafletMarker v-for="c in clusters" :key="c.key" :lng-lat="c.lngLat" anchor="center" @click="drillInto(c)">
          <div
            v-if="c.count > 1"
            class="bg-primary text-primary-foreground ring-primary/25 flex cursor-pointer items-center justify-center rounded-full font-mono text-xs font-bold shadow-md ring-4 transition-transform hover:scale-110"
            :style="{ width: `${c.size}px`, height: `${c.size}px` }"
          >
            {{ c.count }}
          </div>
          <span v-else class="block size-2 rounded-full bg-sky-400 ring-4 ring-sky-400/25" />
        </LeafletMarker>
      </LeafletMap>
    </div>
  </Story>
