...
<template>
  ...
  >
    <div class="flex max-w-2xl flex-col gap-3">
      <ImageCompare v-model="pos" :before-src="photoBefore" :after-src="photoAfter" class="h-72 w-full" />
      <div class="flex items-center gap-3 text-sm">
        <span class="text-muted-foreground w-16 tabular-nums">{{ pos.toFixed(0) }}%</span>
        <input v-model.number="pos" type="range" min="0" max="100" class="flex-1" />
      </div>
    </div>
  </Story>
  ...
  <Story title="Orientation & labels" description="Vertical divider and hidden captions for minimalist layouts.">
    <div class="grid max-w-2xl gap-4 sm:grid-cols-2">
      <ImageCompare :before-src="photoBefore" :after-src="photoAfter" orientation="vertical" class="h-80 w-full" />
      <ImageCompare :before-src="uiBefore" :after-src="uiAfter" :show-labels="false" class="h-80 w-full" />
    </div>
  </Story>
  ...
  >
    <div class="grid max-w-2xl gap-4 sm:grid-cols-2">
      <ImageCompare :before-src="photoBefore" :after-src="photoAfter" class="h-72 w-full">
        <template #handle>
          <GripHorizontal class="text-primary size-5" />
        </template>
      </ImageCompare>
      <ImageCompare :before-src="uiBefore" :after-src="uiAfter" :show-handle="false" class="h-72 w-full" />
    </div>
  </Story>
  ...
  >
    <div class="grid max-w-2xl gap-4 sm:grid-cols-2">
      <ImageCompare :before-src="photoBefore" :after-src="photoAfter" :model-value="30" disabled class="h-72 w-full" />
      <ImageCompare :before-src="uiBefore" :after-src="uiAfter" :model-value="25" class="h-72 w-full" />
    </div>
  </Story>
