...
function onVisible(v: boolean) {
  visibleLog.value.unshift(`${v ? 'shown' : 'hidden'} at ${new Date().toLocaleTimeString()}`)
...
<template>
    ...
    <Card class="max-w-md">
      ...
      <CardContent>
        <div class="back-top-feed border-border relative max-h-64 space-y-2 overflow-y-auto rounded-md border p-3">
          <div v-for="item in feed" :key="item.id" class="bg-muted/40 rounded-md p-3">
  ...
  >
    ...
    <BackTop :threshold="200" :offset="24" position="bottom-right" @visible="onVisible" />
  </Story>
  ...
  >
    <div class="max-w-md space-y-1 text-xs">
      <p v-for="log in visibleLog" :key="log" class="text-muted-foreground tabular-nums">{{ log }}</p>
      <p v-if="!visibleLog.length" class="text-muted-foreground">Scroll the page to fire @visible events.</p>
    </div>
  </Story>
