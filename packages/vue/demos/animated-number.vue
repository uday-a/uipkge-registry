...
const liveTimer = setInterval(() => {
  live.value = Math.max(0, live.value + Math.round((Math.random() - 0.4) * 300))
}, 2000)
...
<template>
  ...
  <Story title="Percentage" description="One decimal place via Intl percent style.">
    <p class="text-4xl font-bold"><AnimatedNumber :value="0.842" :format="(v) => percent.format(v)" /></p>
  </Story>

  <Story title="Compact notation" description="Large numbers collapse to 1.2K / 3.4M style.">
    <p class="text-4xl font-bold"><AnimatedNumber :value="3420000" :format="(v) => compact.format(v)" /></p>
  </Story>
  ...
  <Story title="Tabular column" description="tabular-nums keeps a column of changing figures aligned.">
    <div class="w-64 space-y-2">
      <div v-for="n in [3, 17, 128]" :key="n" class="flex justify-between border-b pb-2 text-sm">
        <span class="text-muted-foreground">Region {{ n }}</span>
        <span class="font-medium"><AnimatedNumber :value="n * 137" :duration="1400" /></span>
      </div>
    </div>
  </Story>
  ...
  <Story title="Staggered trio" description="Delay 0 / 150 / 300ms for a cascading count-up.">
    <div class="grid grid-cols-3 gap-6 text-center">
      <div>
        <p class="text-3xl font-bold"><AnimatedNumber :value="99" :delay="0" /></p>
        <p class="text-muted-foreground text-xs tracking-widest uppercase">uptime %</p>
      </div>
      <div>
        <p class="text-3xl font-bold"><AnimatedNumber :value="54" :delay="150" /></p>
        <p class="text-muted-foreground text-xs tracking-widest uppercase">components</p>
      </div>
      <div>
        <p class="text-3xl font-bold"><AnimatedNumber :value="12" :delay="300" /></p>
  ...
  <Story title="Retargeting" description="Buttons jump the target mid-animation; the tween eases from wherever it is.">
    <div class="space-y-3">
  ...
  <Story title="Locale" description="de-DE grouping — formatters are just functions.">
    <p class="text-4xl font-bold"><AnimatedNumber :value="1234567" :format="(v) => deDe.format(v)" /></p>
  </Story>
</template>
