...
function detect(source: string, det: Detected) {
  for (const m of source.matchAll(IMPORT_RE)) {
    ...
    if (VUE_BUILTINS.has(spec) || spec.startsWith('#')) {
      if (spec === 'nuxt' || spec.startsWith('#') || spec === 'nuxt/app') det.nuxtFlag = true
      continue
    }
...
async function main() {
  ...
  console.log('\nname'.padEnd(28) + 'files'.padStart(6) + '  deps'.padEnd(8) + 'reg-deps'.padEnd(10) + 'nuxt')
  ...
  console.log(`[port] nuxt-flagged: ${results.filter((r) => r.nuxt).length} (review manually)`)
}
