...
function findUnimportedHelpers(script: string, imported: Set<string>): string[] {
...
const REACT_LEAKS = ['react', 'react-dom', '@radix-ui', 'lucide-react', '@tanstack/react-']
...
function insideStringOrComment(source: string, idx: number): boolean {
  ...
  const count = (s: string, ch: string) => s.split('').filter((c, i) => c === ch && s[i - 1] !== '\\').length
  if (count(lineBefore, '"') % 2 === 1 || count(lineBefore, "'") % 2 === 1) return true
  const backticks = before.split('').filter((c, i) => c === '`' && before[i - 1] !== '\\').length
...
function findReactLeaks(source: string): string[] {
  ...
  while ((m = re.exec(source)) !== null) {
    ...
    if (spec.endsWith('.tsx') || spec.endsWith('.jsx')) leaks.push(`'${spec}' (React file)`)
    else if (REACT_LEAKS.some((r) => spec === r || spec.startsWith(r))) leaks.push(`'${spec}'`)
  }
  return leaks
}

async function main() {
  const files = await fg(['components/**/*.vue'], { cwd: ROOT, absolute: true })
  const tsFiles = await fg(['components/**/*.ts', 'blocks/**/*.{ts,vue}', 'bootstrap/**/*.ts'], {
    cwd: ROOT,
    absolute: true,
  ...
  if (failures.length) {
    ...
    console.error(`  Fix: add the helper to the existing \`import { ... } from 'vue'\` line.\n`)
    process.exit(1)
  }
