...
function getScrollElement(): HTMLElement | Window | null {
  ...
  if (typeof props.scrollTarget === "string") {
    return (
      (document.querySelector(props.scrollTarget) as HTMLElement | null) ??
      window
    );
  }
...
function check() {
  if (props.disabled || props.loading || !props.hasMore || !sentinel.value)
    return;
  ...
  if (props.reverse) {
    ...
    if (
      sentinelRect.bottom >= edgeTop - props.distance &&
    ...
    if (
      sentinelRect.top <= edgeBottom + props.distance &&
      sentinelRect.bottom >= edgeTop - props.distance
    ) {
      emit("load");
    }
