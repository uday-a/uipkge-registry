...
describe("InfiniteScroll", () => {
  ...
  it("has data-uipkge on container", () => {
    const w = mount(InfiniteScroll, { attachTo: document.body });
    expect(w.find('[data-slot="infinite-scroll"][data-uipkge]').exists()).toBe(
      true,
    );
    w.unmount();
  });
  ...
  it("renders a sentinel element", () => {
    const w = mount(InfiniteScroll, { attachTo: document.body });
    expect(w.find('[data-slot="infinite-scroll-sentinel"]').exists()).toBe(
      true,
    );
    w.unmount();
  });
  ...
  it("does not render loader when loading is false", () => {
    ...
    expect(w.find('[data-slot="infinite-scroll-loading"]').exists()).toBe(
      false,
    );
    w.unmount();
  });
