...
<template>
  ...
  >
    ...
    <div
      v-if="parseError"
      class="text-destructive flex items-start gap-2 p-4 text-sm"
    >
      <AlertCircle class="mt-0.5 size-4 shrink-0" />
      <div class="min-w-0">
        <p class="font-sans font-medium">Invalid XML</p>
        <p class="text-muted-foreground mt-1 font-mono text-xs break-words">
          {{ parseError }}
        </p>
      </div>
    </div>
