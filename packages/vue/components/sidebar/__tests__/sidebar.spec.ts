...
import { Sidebar, SidebarProvider, SidebarContent, SidebarMenu, SidebarMenuItem, SidebarTrigger } from '../index'
...
function mountSidebar(template: string, props: Record<string, unknown> = {}) {
  return mount(
    {
      components: { SidebarProvider, Sidebar, SidebarContent, SidebarMenu, SidebarMenuItem, SidebarTrigger },
      data() {
...
describe('Sidebar', () => {
  it('renders root with data-slot="sidebar"', () => {
    const w = mountSidebar('<SidebarProvider><Sidebar collapsible="none" /></SidebarProvider>')
  ...
  it('applies data-variant and data-side attributes', () => {
    const w = mountSidebar('<SidebarProvider><Sidebar side="right" variant="floating" /></SidebarProvider>')
    const sidebar = w.find('[data-slot="sidebar"]')
...
describe('SidebarProvider', () => {
  ...
  it('has data-uipkge on wrapper', () => {
    const w = mountSidebar('<SidebarProvider><Sidebar collapsible="none" /></SidebarProvider>')
    expect(w.find('[data-slot="sidebar-wrapper"][data-uipkge]').exists()).toBe(true)
    w.unmount()
  })
})
