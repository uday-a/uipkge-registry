...
const period = computed(() =>
  parts.value && parts.value.hour24 >= 12 ? "PM" : "AM",
);
...
const hours12List = computed(() => {
  ...
  const list = Array.from({ length: 12 }, (_, i) => i + 1).filter(
    (h) => (h - 1) % step === 0,
  );
...
const hours24List = computed(() => {
  const step = Math.max(1, props.hourStep);
  const list = Array.from({ length: 24 }, (_, i) => i).filter(
    (h) => h % step === 0,
  );
  if (!props.hideDisabledOptions) return list;
  return list.filter((h) => !isHourDisabledItem(h));
...
function resolveEl(
  node: { $el?: HTMLElement } | HTMLElement | null | undefined,
): HTMLElement | null {
  if (!node) return null;
