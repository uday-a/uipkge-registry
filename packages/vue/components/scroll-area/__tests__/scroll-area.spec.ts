...
describe('ScrollArea', () => {
  ...
  it('has data-uipkge', () => {
    const w = mountScrollArea()
    expect(w.find('[data-slot="scroll-area"]').attributes('data-uipkge')).toBeDefined()
    w.unmount()
  })
  ...
  it('renders slot content', () => {
    const w = mountScrollArea({ default: '<div class="my-content">Hello Scroll</div>' })
    expect(w.text()).toContain('Hello Scroll')
    w.unmount()
  })

  it('renders without crashing with content', () => {
    const w = mountScrollArea({ default: '<div><p>Line 1</p><p>Line 2</p></div>' })
  ...
  it('has data-orientation on scrollbar when rendered', () => {
    ...
    if (scrollbar.exists()) {
      ...
      expect(w.find('[data-slot="scroll-area"]').attributes('dir')).toBeTruthy()
    }
    w.unmount()
  })
})
