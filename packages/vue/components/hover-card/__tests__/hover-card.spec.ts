...
describe("HoverCard", () => {
  ...
  it("HoverCardContent renders slot content", async () => {
    ...
    ).toContain("My Hover Content");
    w.unmount();
  });

  it("HoverCardTrigger renders slot content", () => {
    const w = mountHoverCard(
      "<HoverCard><HoverCardTrigger>Trigger Label</HoverCardTrigger></HoverCard>",
    );
    expect(w.find('[data-slot="hover-card-trigger"]').text()).toContain(
      "Trigger Label",
    );
    w.unmount();
  });
