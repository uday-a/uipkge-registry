...
describe("Alert", () => {
  ...
  it("renders text prop content", () => {
    const w = mount(Alert, {
      props: { text: "Something happened" },
      attachTo: document.body,
    });
    expect(w.find('[data-slot="alert-description"]').text()).toBe(
      "Something happened",
    );
    w.unmount();
  });
  ...
  it("applies destructive variant classes", () => {
    ...
    expect(w.find('[data-slot="alert"]').classes()).toContain(
...
describe("AlertDescription", () => {
  it('renders with data-slot="alert-description"', () => {
    const w = mount(AlertDescription, {
      slots: { default: "Desc" },
      attachTo: document.body,
    });
