...
describe('Sheet', () => {
  ...
  it('SheetTrigger renders as a button', () => {
    const w = mountSheet('<Sheet><SheetTrigger>Open</SheetTrigger></Sheet>')
    expect(w.find('[data-slot="sheet-trigger"]').element.tagName.toLowerCase()).toBe('button')
    w.unmount()
  })
  ...
  it('SheetContent has data-uipkge', async () => {
    ...
    await flushPromises()
    expect(document.querySelector('[data-slot="sheet-content"]')?.getAttribute('data-uipkge')).toBeDefined()
    w.unmount()
  })
  ...
  it('SheetContent has data-state="open" when open', async () => {
    ...
    await flushPromises()
    expect(document.querySelector('[data-slot="sheet-content"]')?.getAttribute('data-state')).toBe('open')
    w.unmount()
  })
  ...
  it('SheetClose renders with data-slot="sheet-close"', async () => {
    ...
    await flushPromises()
    expect(document.querySelectorAll('[data-slot="sheet-close"]').length).toBeGreaterThanOrEqual(1)
    w.unmount()
  })
  ...
  it('SheetContent renders title text when open', async () => {
    ...
    await flushPromises()
    expect(document.querySelector('[data-slot="sheet-title"]')?.textContent).toContain('My Sheet Title')
    w.unmount()
  })
})
