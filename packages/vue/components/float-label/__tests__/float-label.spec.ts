...
function mountFloatLabel(props: Record<string, unknown> = {}, slots: Record<string, unknown> = {}) {
...
describe('FloatLabel', () => {
  ...
  it('data-floating is false initially when input is empty', async () => {
    const w = mountFloatLabel({ label: 'Email' }, { default: '<input />' })
    await flushPromises()
    expect(w.find('[data-slot="float-label"]').attributes('data-floating')).toBe('false')
    w.unmount()
  })
  ...
  it('label floats on focus (data-floating becomes true)', async () => {
    ...
    expect(w.find('[data-slot="float-label"]').attributes('data-floating')).toBe('true')
    w.unmount()
  })

  it('shows required indicator when required prop is set', () => {
    const w = mountFloatLabel({ label: 'Email', required: true }, { default: '<input />' })
