...
describe("ScrollSpy (Vue)", () => {
  ...
  it("renders bottom floating stepper mode", () => {
    ...
    expect(w.find('[data-slot="scroll-spy-stepper-bottom"]').exists()).toBe(
      true,
    );
    w.unmount();
  });
  ...
  it("supports compound component composition", () => {
    ...
    expect(w.find('[data-slot="scroll-spy-title"]').text()).toBe(
      "On this page",
    );
  ...
  it("supports keepScrolled prop and sets data-keep-scrolled", () => {
    ...
    expect(
      w.find('[data-slot="scroll-spy"]').attributes("data-keep-scrolled"),
    ).toBe("true");
    w.unmount();
  });
