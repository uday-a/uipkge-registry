...
describe('Carousel', () => {
  ...
  it('has data-uipkge', () => {
    const w = mountCarousel()
    expect(w.find('[data-slot="carousel"]').attributes('data-uipkge')).toBeDefined()
    w.unmount()
  })
  ...
  it('CarouselItem has role="group"', () => {
    const w = mountCarousel()
    expect(w.find('[data-slot="carousel-item"]').attributes('role')).toBe('group')
    w.unmount()
  })
  ...
  it('CarouselContent applies aria-orientation', () => {
    ...
    expect(w.find('[data-slot="carousel-content"]').attributes('aria-orientation')).toBe('horizontal')
    w.unmount()
  })
})
