...
<template>
  <HoverCard v-if="popover" :open-delay="openDelay" :close-delay="closeDelay">
    ...
    <HoverCardContent
      align="start"
      :side-offset="6"
      class="border-border/80 w-80 rounded-xl p-4 shadow-lg"
    >
      <slot name="popup">
        ...
        <div class="space-y-3">
          ...
          <div>
            ...
            <p v-if="email" class="text-muted-foreground mt-0.5 text-xs">
              {{ email }}
            </p>
          </div>

          <p v-if="bio" class="text-foreground/90 text-xs leading-relaxed">
            {{ bio }}
          </p>

          <div
            v-if="joined || location"
