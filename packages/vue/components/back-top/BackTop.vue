...
const props = withDefaults(
  ...
  {
    threshold: 200,
    target: () => (typeof window !== 'undefined' ? window : (undefined as unknown as Window)),
    behavior: 'smooth',
    size: 'default',
...
const positionStyle = computed(() => {
  ...
  switch (props.position) {
    case 'bottom-left':
      return { left: offsetVar, bottom: offsetVar, '--back-top-offset': `${props.offset}px` }
    case 'top-right':
      return { right: offsetVar, top: offsetVar, '--back-top-offset': `${props.offset}px` }
    case 'top-left':
      return { left: offsetVar, top: offsetVar, '--back-top-offset': `${props.offset}px` }
    default:
      return { right: offsetVar, bottom: offsetVar, '--back-top-offset': `${props.offset}px` }
  }
})
