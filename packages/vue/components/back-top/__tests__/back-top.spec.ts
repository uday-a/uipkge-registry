...
describe("BackTop", () => {
  ...
  it('applies data-state="open" when visible', async () => {
    const w = mountBackTop({ threshold: 0 });
    await flushPromises();
    expect(w.find('[data-slot="back-top"]').attributes("data-state")).toBe(
      "open",
    );
    w.unmount();
  });
  ...
  it("applies data-position attribute", () => {
    ...
    expect(w.find('[data-slot="back-top"]').attributes("data-position")).toBe(
      "top-left",
    );
    w.unmount();
  });
  ...
  it("has accessible aria-label", () => {
    ...
    expect(w.find('[data-slot="back-top"]').attributes("aria-label")).toBe(
      "Back to top",
    );
    w.unmount();
  });
