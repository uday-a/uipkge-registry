...
describe('Attachment', () => {
  it('renders with data-slot="attachment"', () => {
    const w = mount(Attachment, { props: { title: 'notes.pdf' }, attachTo: document.body })
  ...
  it('sets state from the state prop', () => {
    const w = mount(Attachment, { props: { title: 'a', state: 'uploading' }, attachTo: document.body })
    expect(w.find('[data-slot="attachment"]').attributes('data-state')).toBe('uploading')
    w.unmount()
  })
  ...
  it('emits remove when removable', async () => {
    const w = mount(Attachment, { props: { title: 'notes.pdf', removable: true }, attachTo: document.body })
