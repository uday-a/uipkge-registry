...
describe("Link", () => {
  ...
  it("renders as an anchor element", () => {
    const w = mountLink({ href: "/about" });
    expect(w.find('[data-slot="link"]').element.tagName.toLowerCase()).toBe(
      "a",
    );
    w.unmount();
  });
  ...
  it("applies data-underline attribute", () => {
    ...
    expect(w.find('[data-slot="link"]').attributes("data-underline")).toBe(
      "always",
    );
    w.unmount();
  });
  ...
  it("opens external http href in a new tab", () => {
    ...
    expect(w.find('[data-slot="link"]').attributes("rel")).toBe(
      "noopener noreferrer",
    );
    w.unmount();
  });
