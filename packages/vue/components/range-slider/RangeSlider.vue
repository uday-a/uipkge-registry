...
const displayValue = ref<[number, number]>(
  props.modelValue ?? props.defaultValue ?? [props.min, props.max],
);
watch(
  () => props.modelValue,
  (v) => {
...
const hasError = computed(() => {
  ...
  if (
    props.errorMessages &&
    (typeof props.errorMessages === "string"
      ? props.errorMessages
...
<template>
  <div class="flex flex-col gap-2">
    ...
    <div class="flex items-center gap-4">
      <!-- Min value display -->
      <div
        class="text-muted-foreground min-w-[3rem] text-sm tabular-nums"
        aria-hidden="true"
      >
        {{ displayValue[0] }}
      </div>
      ...
      >
        ...
        >
          <SliderRange
            ...
            :class="
              cn(
                'absolute h-full',
                colorClasses[color as string] || colorClasses.primary,
              )
            "
          />
        </SliderTrack>
        ...
        >
          <div
            v-for="tick in ticks"
            :key="tick"
            class="bg-muted-foreground/30 h-2 w-0.5 rounded-full"
          />
        </div>
        ...
        >
          ...
          >
            {{
              thumbLabelFormat
                ? thumbLabelFormat(modelValue?.[0] ?? 0)
                : (modelValue?.[0] ?? 0)
            }}
          </span>
        </SliderThumb>
        ...
        >
          ...
          >
            {{
              thumbLabelFormat
                ? thumbLabelFormat(modelValue?.[1] ?? 0)
                : (modelValue?.[1] ?? 0)
            }}
          </span>
        </SliderThumb>
    ...
    <div
      v-if="showTicks && ticks.length > 0"
      class="text-muted-foreground flex justify-between px-1 text-xs"
    >
      <span>{{ min }}</span>
      <span>{{ max }}</span>
    </div>
