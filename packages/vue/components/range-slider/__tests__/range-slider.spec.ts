...
describe('RangeSlider', () => {
  ...
  it('renders hint text', () => {
    const w = mountRangeSlider({ modelValue: [20, 80], hint: 'Drag to adjust' })
    expect(w.text()).toContain('Drag to adjust')
    w.unmount()
  })

  it('renders error messages', () => {
    const w = mountRangeSlider({ modelValue: [20, 80], errorMessages: 'Invalid range' })
  ...
  it('disables slider when disabled prop is true', () => {
    ...
    expect(w.find('[data-slot="range-slider"]').attributes('data-disabled')).toBeDefined()
    w.unmount()
  })

  it('renders ticks when showTicks is true', () => {
    const w = mountRangeSlider({ modelValue: [20, 80], showTicks: true, min: 0, max: 100, step: 25 })
    expect(w.find('[data-slot="range-slider"]').exists()).toBe(true)
    w.unmount()
  })
