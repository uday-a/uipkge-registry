...
function mountVerticalTabs(overrides: Record<string, unknown> = {}) {
  return mount(
    {
      components: { VerticalTabs, VerticalTabsList, VerticalTabsTrigger, VerticalTabsContent },
      template:
...
describe('VerticalTabs', () => {
  ...
  it('has data-uipkge on root', () => {
    const w = mountVerticalTabs()
    expect(w.find('[data-slot="vertical-tabs"]').attributes('data-uipkge')).toBeDefined()
    w.unmount()
  })

  it('applies data-orientation="vertical" on root', () => {
    const w = mountVerticalTabs()
    expect(w.find('[data-slot="vertical-tabs"]').attributes('data-orientation')).toBe('vertical')
    w.unmount()
  })
  ...
  it('VerticalTabsList has data-uipkge', () => {
    const w = mountVerticalTabs()
    expect(w.find('[data-slot="vertical-tabs-list"]').attributes('data-uipkge')).toBeDefined()
    w.unmount()
  })
  ...
  it('VerticalTabsTrigger has data-uipkge', () => {
    const w = mountVerticalTabs()
    expect(w.find('[data-slot="vertical-tabs-trigger"]').attributes('data-uipkge')).toBeDefined()
    w.unmount()
  })
