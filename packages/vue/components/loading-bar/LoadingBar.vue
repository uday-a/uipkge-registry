...
const barColor = computed(() => props.color || (isError.value ? 'var(--destructive)' : 'var(--primary)'))
const visible = computed(() => !props.hidden && !fading.value && (props.indeterminate || internal.value > 0))
...
function trickle(gen: number) {
  ...
  const step = () => {
    ...
    const next = Math.min(95, internal.value + (95 - internal.value) * 0.04 + 0.15)
    set(next)
