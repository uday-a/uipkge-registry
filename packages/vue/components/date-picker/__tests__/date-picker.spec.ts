...
describe('DatePicker', () => {
  ...
  it('disables trigger when disabled prop is true', () => {
    ...
    expect(w.find('button[data-slot="date-picker"]').attributes('disabled')).toBeDefined()
    w.unmount()
  })
  ...
  it('has data-uipkge on trigger', () => {
    const w = mountDatePicker()
    expect(w.find('[data-slot="date-picker"]').attributes('data-uipkge')).toBeDefined()
    w.unmount()
  })
  ...
  it('keeps each selected date when type is multiple', async () => {
    ...
    const days = Array.from(document.querySelectorAll<HTMLButtonElement>('[data-slot="calendar-cell-trigger"]')).filter(
      (day) => !day.disabled,
    )
    days[10]!.click()
    await nextTick()
