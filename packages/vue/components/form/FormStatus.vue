...
const statusConfig: Record<NonNullable<FormStatus>, { icon: Component; container: string }> = {
  error: {
    icon: CircleAlert,
...
<template>
  <div
    ...
    "
  >
    <component :is="statusConfig[status].icon" class="size-4 shrink-0" aria-hidden="true" />
    <span>{{ message }}</span>
  </div>
</template>
