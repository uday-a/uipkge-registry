...
describe('Board', () => {
  ...
  it('Board has data-uipkge', () => {
    const w = mountBoard()
    expect(w.find('[data-slot="board"]').attributes('data-uipkge')).toBeDefined()
    w.unmount()
  })

  it('Board applies data-orientation', () => {
    const w = mountBoard()
    expect(w.find('[data-slot="board"]').attributes('data-orientation')).toBe('horizontal')
    w.unmount()
  })
  ...
  it('BoardLane has data-uipkge', () => {
    const w = mountBoard()
    expect(w.find('[data-slot="board-lane"]').attributes('data-uipkge')).toBeDefined()
    w.unmount()
  })
  ...
  it('BoardCard has data-uipkge', () => {
    const w = mountBoard()
    expect(w.find('[data-slot="board-card"]').attributes('data-uipkge')).toBeDefined()
    w.unmount()
  })

  it('BoardCard has draggable attribute', () => {
    const w = mountBoard()
    expect(w.find('[data-slot="board-card"]').attributes('draggable')).toBe('true')
    w.unmount()
  })

  it('BoardCard has role="button"', () => {
    const w = mountBoard()
    expect(w.find('[data-slot="board-card"]').attributes('role')).toBe('button')
    w.unmount()
  })
  ...
  it('BoardLane applies data-disabled when disabled', () => {
    const w = mount(
      {
        components: { Board, BoardLane, BoardLaneHeader, BoardCard },
        template: '<Board><BoardLane id="todo" disabled><BoardLaneHeader>Todo</BoardLaneHeader></BoardLane></Board>',
      },
      { attachTo: document.body },
    )
    expect(w.find('[data-slot="board-lane"]').attributes('data-disabled')).toBeDefined()
    w.unmount()
  })
})
