...
const board = inject(BOARD_CONTEXT)!;
const lane = inject(BOARD_LANE_CONTEXT)!;
if (!board || !lane) {
  throw new Error(
    "<BoardCard> must be a descendant of <Board> and <BoardLane>.",
  );
}
...
function onKeydown(e: KeyboardEvent) {
  ...
  if (e.key === "ArrowLeft" || e.key === "ArrowRight") {
    ...
    const currentIdx = allLanes.findIndex(
      (el) => el.dataset.laneId === lane.laneId.value,
    );
  ...
  if (e.key === "ArrowUp" || e.key === "ArrowDown") {
    ...
    const currentIdx = cards.findIndex(
      (el) => el.dataset.boardCardId === props.id,
    );
    ...
    if (targetIdx !== currentIdx)
      board.moveItem(props.id, lane.laneId.value, targetIdx);
  }
}
