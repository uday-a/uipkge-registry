...
describe("Countdown (Vue)", () => {
  ...
  it("rolls days into hours for HH:MM:SS and into minutes for MM:SS", () => {
    ...
    const mmss = mount(Countdown, {
      props: { target: NOW + 2 * HOUR + MINUTE, format: "MM:SS" },
    });
  ...
  it("honours pad=false so single digits are not zero-padded", () => {
    const padded = mount(Countdown, {
      props: { target: NOW + 5 * SECOND, format: "SS" },
    });
    expect(padded.text()).toContain("05");
    padded.unmount();

    const bare = mount(Countdown, {
      props: { target: NOW + 5 * SECOND, format: "SS", pad: false },
    });
  ...
  it("ticks down once per second and emits the remaining milliseconds", async () => {
    const wrapper = mount(Countdown, {
      props: { target: NOW + 10 * SECOND, format: "SS" },
    });
  ...
  it("emits finish once when it reaches zero and then stops ticking", async () => {
    const wrapper = mount(Countdown, {
      props: { target: NOW + 2 * SECOND, format: "SS" },
    });
  ...
  it("emits finish immediately for a target already in the past", () => {
    const wrapper = mount(Countdown, {
      props: { target: NOW - MINUTE, format: "SS" },
    });
    expect(wrapper.emitted("finish")).toHaveLength(1);
    expect(wrapper.text()).toContain("00");
    wrapper.unmount();
  ...
  it("stops while paused and resumes from the current time", async () => {
    const wrapper = mount(Countdown, {
      props: { target: NOW + 60 * SECOND, format: "SS", paused: true },
    });
  ...
  it("accepts a Date and an ISO string as well as epoch milliseconds", () => {
    const fromDate = mount(Countdown, {
      props: { target: new Date(NOW + 30 * SECOND), format: "SS" },
    });
  ...
  it("renders only the units named by the format", () => {
    const wrapper = mount(Countdown, {
      props: { target: NOW + DAY + HOUR, format: "HH:MM" },
    });
    ...
    expect(wrapper.find('[data-slot="countdown-seconds"]').exists()).toBe(
      false,
    );
    wrapper.unmount();
  });
  ...
  it("hands the slot a display string with DD/HH/MM/SS tokens substituted", () => {
    const wrapper = mount(Countdown, {
      ...
      slots: {
        default: `<template #default="{ display }"><span class="custom">{{ display }}</span></template>`,
      },
    });
