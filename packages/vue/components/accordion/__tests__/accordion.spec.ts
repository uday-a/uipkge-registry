...
describe('Accordion', () => {
  ...
  it('has data-uipkge', () => {
    const w = mountAccordion()
    expect(w.find('[data-slot="accordion"]').attributes('data-uipkge')).toBeDefined()
    w.unmount()
  })
  ...
  it('AccordionTrigger has role="button" or is a button', () => {
    ...
    expect(trigger.attributes('role') === 'button' || trigger.element.tagName.toLowerCase() === 'button').toBe(true)
    w.unmount()
  })
  ...
  it('Accordion applies data-variant attribute', () => {
    ...
    expect(w.find('[data-slot="accordion"]').attributes('data-variant')).toBe('separated')
    w.unmount()
  })
