...
const effectivePageSize = computed(
  () => ctx.pageSize.value ?? Math.max(1, filtered.value.length),
);
const totalPages = computed(() =>
  Math.max(1, Math.ceil(filtered.value.length / effectivePageSize.value)),
);
...
const visibleEnabledKeys = computed(() =>
  visible.value.filter((i) => !i.disabled).map((i) => i.key),
);
const selectedSet = computed(() => new Set(props.selected));

const visibleSelectedCount = computed(
  () => visibleEnabledKeys.value.filter((k) => selectedSet.value.has(k)).length,
);

const masterChecked = computed(
  () =>
    visibleEnabledKeys.value.length > 0 &&
    visibleSelectedCount.value === visibleEnabledKeys.value.length,
);
...
function onRowClick(e: MouseEvent, item: TransferItem) {
  ...
  const enabledKeys = visible.value
    .filter((i) => !i.disabled)
    .map((i) => i.key);
  if (
    e.shiftKey &&
    lastAnchor.value &&
    enabledKeys.includes(lastAnchor.value)
  ) {
  ...
  if (e.metaKey || e.ctrlKey) {
    let next = [...props.selected];
    if (selectedSet.value.has(item.key))
      next = next.filter((k) => k !== item.key);
...
function onRowKeydown(e: KeyboardEvent, item: TransferItem) {
  ...
  emits(
    "update:selected",
    selectedSet.value.has(item.key) && props.selected.length === 1
      ? []
      : [item.key],
  );
  lastAnchor.value = item.key;
...
const heightStyle = computed(() => ({
  height:
    typeof ctx.height.value === "number"
      ? ctx.height.value + "px"
      : ctx.height.value,
}));
...
const dropIndicator = ref<{ key: string; position: "before" | "after" } | null>(
  null,
);
...
const isDropTarget = computed(() => {
  ...
  if (props.side === "left" && ctx.oneWay.value && p.fromSide === "right")
    return false;
...
<template>
  ...
  >
    <div
      class="bg-muted/40 flex items-center justify-between gap-2 border-b px-3 py-2"
    >
    ...
    >
      ...
      >
        <li
          ...
          :draggable="
            ctx.draggable.value && !item.disabled && !ctx.disabled.value
          "
          :class="[
