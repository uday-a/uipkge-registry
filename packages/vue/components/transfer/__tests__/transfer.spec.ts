...
describe("Transfer", () => {
  ...
  it("has data-uipkge", () => {
    const w = mountTransfer();
    expect(
      w.find('[data-slot="transfer"]').attributes("data-uipkge"),
    ).toBeDefined();
    w.unmount();
  });
  ...
  it("renders operation buttons", () => {
    const w = mountTransfer();
    expect(w.find('button[aria-label="Move selected to right"]').exists()).toBe(
      true,
    );
    expect(w.find('button[aria-label="Move selected to left"]').exists()).toBe(
      true,
    );
    w.unmount();
  });
