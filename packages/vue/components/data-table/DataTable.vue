...
function clearDateFilter(filter: FilterDefinition) {
  clearFilter(filter);
  dateRangeModels.value[filter.column] = {
    start: undefined as any,
    end: undefined as any,
  };
}
...
watch(
  [() => props.infinite, sentinelEl],
  () => attachInfiniteObserver(sentinelEl.value),
  {
    flush: "post",
  },
);
...
function onTableKeyDown(e: KeyboardEvent) {
  ...
  if (isDown) {
    ...
    if (selected.length > 0) {
      ...
      focusedRowIndex.value < rows.length
    ) {
      e.preventDefault();
...
<template>
  ...
  >
    ...
    <DataTableToolbar
      ...
      @text-filter-update="
        (col, val) => table.getColumn(col)?.setFilterValue(val)
      "
      ...
      @update:density="
        (d: 'compact' | 'cozy' | 'comfortable') => (currentDensity = d)
      "
    >
    ...
    >
      ...
      >
        <span
          class="bg-primary/10 text-primary rounded-md px-2 py-0.5 text-xs font-semibold tabular-nums"
        >
      ...
      <div
        :class="[
          densityClass,
          'relative overflow-auto [&_[data-slot=table-container]]:overflow-visible',
        ]"
        ...
        <Table>
          ...
          >
            ...
            >
              <TableHead
                ...
                @dragstart="
                  enableReorder
                    ? onColDragStart(header.column.id, $event)
                    : undefined
                "
                @dragover="
                  enableReorder
                    ? onColDragOver(header.column.id, $event)
                    : undefined
                "
          ...
          >
            ...
            <template v-if="loading && !table.getRowModel().rows?.length">
              ...
              >
                <TableCell
                  v-for="j in Math.max(columns.length, 1)"
                  :key="`sk-${i}-${j}`"
                >
                  <div
            ...
            <template v-else>
              <TableRow class="hover:bg-transparent">
                <TableCell :colspan="columns.length" class="h-36 text-center">
                  <slot name="empty">
                    <div
                      class="text-muted-foreground flex flex-col items-center justify-center gap-2 py-8"
                    >
                      <div
                        class="bg-muted/80 text-muted-foreground grid size-10 place-items-center rounded-full"
                      >
                      ...
                      <div class="space-y-1">
                        ...
                        <p
                          class="text-muted-foreground max-w-[28ch] text-xs leading-relaxed"
                        >
