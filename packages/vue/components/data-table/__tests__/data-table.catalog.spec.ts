 ...
 * Mirrors docs/data-table-test-cases-vue.md
 */
...
describe("DataTable Vue catalog (200)", () => {
    ...
    const cols = [
      {
        id: "title",
        header: "Title",
        accessorFn: (r: Employee) => r.nested?.title ?? "—",
      },
    ...
    const cols = [
      {
        ...
        header: "Status",
        cell: ({ row }: any) =>
          h("span", { "data-testid": "sc" }, row.original.status),
      },
    ];
    const w = await mountTable({ columns: cols, enablePagination: false });
  ...
  it("DT-VUE-018 column size no crash", async () => {
    const w = await mountTable({
      columns: [{ accessorKey: "name", header: "Name", size: 200 }],
    });
    expect(root(w).exists()).toBe(true);
    w.unmount();
  });
  ...
  it("DT-VUE-019 View button when enableColumnVisibility", async () => {
    ...
    expect(w.findAll("button").some((b) => b.text().includes("View"))).toBe(
      true,
    );
    w.unmount();
  });
  ...
  it("DT-VUE-020 meta no crash", async () => {
    const w = await mountTable({
      columns: [
        { accessorKey: "name", header: "Name", meta: { className: "x" } },
      ],
    });
    expect(root(w).exists()).toBe(true);
    w.unmount();
  ...
  it("DT-VUE-022 actions column", async () => {
    ...
    expect(w.findAll("button").some((b) => b.text().includes("···"))).toBe(
      true,
    );
  ...
  it("DT-VUE-029 sort button present", async () => {
    const w = await mountSortable();
    expect(w.findAll("button").some((b) => b.text().includes("Name"))).toBe(
      true,
    );
    w.unmount();
  });
    ...
    const cols = [
      {
        accessorKey: "age",
        header: ({ column }: any) =>
          h(DataTableColumnHeader, { column, label: "Age" }),
      },
      { accessorKey: "name", header: "Name" },
    ];
    const w = await mountTable({ columns: cols, enablePagination: false });
  ...
  it("DT-VUE-033 empty name sort safe", async () => {
    const data = [
      ...employees,
      { ...employees[0], id: "z", name: "", email: "z@z.com" },
    ];
  ...
  it("DT-VUE-039 filter via table API + sort", async () => {
    ...
    expect(
      texts.length === 0 || texts.every((t) => t.includes("Engineering")),
    ).toBe(true);
    w.unmount();
  });
  ...
  it("DT-VUE-048 bulk slot when selected", async () => {
    ...
    expect(
      root(w).exists() ||
        w.find('[data-slot="data-table"]').exists() ||
        w.html().length > 0,
    ).toBe(true);
    w.unmount();
  });
  ...
  it("DT-VUE-051 checkbox is input", async () => {
    const w = await mountSortable();
    expect(w.find('input[aria-label="Select row"]').element.tagName).toBe(
      "INPUT",
    );
    w.unmount();
  });
  ...
  it("DT-VUE-053 search visible with filterColumn", async () => {
    ...
    expect(
      w
        .find(
          'input[placeholder*="email"], input[placeholder*="Search"], input[placeholder*="Filter"]',
        )
        .exists(),
    ).toBe(true);
    w.unmount();
  ...
  it("DT-VUE-065 default filterMode inline chips", async () => {
    const w = await mountSortable();
    expect(
      w.findAll("button").some((b) => b.text().includes("Department")),
    ).toBe(true);
    expect(w.findAll("button").some((b) => b.text().includes("Status"))).toBe(
      true,
    );
  ...
  it("DT-VUE-068 multi option OR", async () => {
    ...
    exposedTable(w)
      ...
      ?.setFilterValue(["Engineering", "Design"]);
    await flushPromises();
    const texts = bodyTexts(w);
    expect(
      texts.every((t) => t.includes("Engineering") || t.includes("Design")),
    ).toBe(true);
    w.unmount();
  });
  ...
  it("DT-VUE-070 Reset button when filter active", async () => {
    ...
    await flushPromises();
    await nextTick();
    expect(w.findAll("button").some((b) => b.text().includes("Reset"))).toBe(
      true,
    );
    w.unmount();
  });
  ...
  it("DT-VUE-075 date filter type mounts", async () => {
    const w = await mountSortable({
      filters: [{ column: "email", label: "When", type: "date" }],
    });
    expect(root(w).exists()).toBe(true);
    w.unmount();
  });
  ...
  it("DT-VUE-081 custom-filters slot", async () => {
    ...
    await nextTick();
    expect(
      w.find('[data-testid="cf"]').exists() || w.html().includes("Custom"),
    ).toBe(true);
    w.unmount();
  });
  ...
  it("DT-VUE-082 AND two filters", async () => {
    ...
    const texts = bodyTexts(w);
    expect(
      texts.every((t) => t.includes("Engineering") && t.includes("active")),
    ).toBe(true);
    w.unmount();
  });
  ...
  it("DT-VUE-085 popover mode Filters button", async () => {
    ...
    expect(w.findAll("button").some((b) => /Filter/i.test(b.text()))).toBe(
      true,
    );
    w.unmount();
  });
  ...
  it("DT-VUE-095 modal mode Filters button", async () => {
    ...
    expect(w.findAll("button").some((b) => b.text().includes("Filters"))).toBe(
      true,
    );
    w.unmount();
  });
  ...
  it("DT-VUE-106 modal still available opt-in", async () => {
    ...
    const deptChips = w
      ...
      .filter(
        (b) =>
          /^\s*Department/.test(b.text()) || b.text().startsWith("Department"),
      );
  ...
  it("DT-VUE-118 next page", async () => {
    ...
    const next = w.find(
      'button[aria-label="Next page"], button:has(span.sr-only)',
    );
    const nextBtn = w
      .findAll("button")
      .find(
        (b) =>
          b.html().includes("Next page") ||
          b.attributes("aria-label") === "Next page",
      );
  ...
  it("DT-VUE-132 View hidden by default", async () => {
    ...
    expect(
      w
        .findAll("button")
        .some((b) => b.text() === "View" || b.text().includes("View")),
    ).toBe(false);
    w.unmount();
  });
  ...
  it("DT-VUE-134 hide column via API", async () => {
    ...
    await flushPromises();
    expect(w.findAll("th").some((th) => th.text().includes("Email"))).toBe(
      false,
    );
    w.unmount();
  });
  ...
  it("DT-VUE-139 export respects visibility smoke", async () => {
    ...
    expect(w.findAll("button").some((b) => b.text().includes("Export"))).toBe(
      true,
    );
    w.unmount();
  });
  ...
  it("DT-VUE-150 density toggle hidden default", async () => {
    ...
    expect(
      w
        .findAll("button")
        .some((b) => /compact|cozy|comfortable/i.test(b.text())),
    ).toBe(false);
    w.unmount();
  });
  ...
  it("DT-VUE-151 density toggle shown", async () => {
    ...
    expect(
      w
        ...
        .some(
          (b) =>
            /compact|cozy|comfortable|Density|Rows/i.test(b.text()) ||
            b.html().includes("Rows3"),
        ),
    ).toBe(true);
    w.unmount();
  });
  ...
  it("DT-VUE-165 column pinning left", async () => {
    const w = await mountSortable({
      defaultColumnPinning: { left: ["select", "name"] },
    });
    expect(root(w).exists()).toBe(true);
    w.unmount();
  });
    ...
    const cols = [
      {
        accessorKey: "name",
        header: ({ column }: any) =>
          h(DataTableColumnHeader, { column, label: "Name" }),
      },
    ];
    const w = await mountTable({
      columns: cols,
  ...
  it("DT-VUE-191 filter popover trigger is button", async () => {
    ...
    const dept = w
      .findAll("button")
      .find((b) => b.text().includes("Department"));
