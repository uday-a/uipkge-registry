...
describe('QRCode', () => {
  it('renders with data-slot="qr-code"', () => {
    const w = mount(QRCode, { props: { value: 'https://example.com' }, attachTo: document.body })
  ...
  it('renders an img element for canvas type', async () => {
    const w = mount(QRCode, { props: { value: 'https://example.com', type: 'canvas' }, attachTo: document.body })
  ...
  it('renders bordered container by default', () => {
    const w = mount(QRCode, { props: { value: 'test' }, attachTo: document.body })
    expect(w.find('[data-slot="qr-code"]').classes()).toContain('border')
    w.unmount()
  })

  it('does not apply border when bordered=false', () => {
    const w = mount(QRCode, { props: { value: 'test', bordered: false }, attachTo: document.body })
