...
describe('Checkbox', () => {
  ...
  it('shows checked state when modelValue is true', () => {
    const w = mountCheckbox({ modelValue: true })
    expect(w.find('[role="checkbox"]').attributes('data-state')).toBe('checked')
    w.unmount()
  })

  it('shows unchecked state when modelValue is false', () => {
    const w = mountCheckbox({ modelValue: false })
    expect(w.find('[role="checkbox"]').attributes('data-state')).toBe('unchecked')
    w.unmount()
  })
  ...
  it('shows indeterminate state when indeterminate prop is true', () => {
    ...
    expect(w.find('[role="checkbox"]').attributes('data-state')).toBe('indeterminate')
    w.unmount()
  })
  ...
  it('renders label before checkbox when labelPosition is before', () => {
    const w = mountCheckbox({ modelValue: false, label: 'Before', labelPosition: 'before' })
  ...
  it('renders label after checkbox when labelPosition is after', () => {
    const w = mountCheckbox({ modelValue: false, label: 'After', labelPosition: 'after' })
    const labels = w.findAll('label')
    expect(labels.length).toBeGreaterThanOrEqual(1)
