...
export type CheckboxProps = Omit<
  CheckboxRootProps<boolean | string>,
...
const groupContext = inject<{ name?: ComputedRef<string | undefined> } | null>(
...
const resolvedId = computed(
  () => props.id ?? (props.label ? autoId : undefined),
);
...
const isControlled = computed(() =>
  Boolean(instance?.vnode?.props?.["onUpdate:modelValue"]),
);
...
const checkboxStateBindings = computed(() => {
  ...
  if (userPassedModelValue.value)
    return { "default-value": Boolean(props.modelValue) };
  ...
  >
    ...
    <div class="flex items-center">
      <CheckboxRoot
        ...
        @update:model-value="
          (val: boolean | 'indeterminate') => emits('update:modelValue', val)
        "
      >
        <CheckboxIndicator
          data-uipkge
        ...
        >
          <slot v-bind="slotProps">
            ...
            <Minus
              ...
              :class="[iconSizes[size], 'checkbox-indicator-icon']"
            />
            <Check
              v-else-if="!hideIcon"
              :class="[iconSizes[size], 'checkbox-indicator-icon']"
