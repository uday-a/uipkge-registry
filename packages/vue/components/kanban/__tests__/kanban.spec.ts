      ...
      default: () =>
        h(KanbanBoard, {}, () =>
          Object.entries(cards).map(([columnId, ids]) =>
            h(
              KanbanColumn,
              { id: columnId, label: columnId === "todo" ? "To do" : "Done" },
              () => [
                ...
                h(KanbanColumnBody, {}, () =>
                  ids.map((id) =>
                    h(KanbanCard, { id, ...cardProps }, () => [
                      h(KanbanCardTitle, {}, () => id),
                    ]),
                  ),
                ),
              ],
...
describe("Kanban Primitive (Vue)", () => {
  ...
  it("exposes each draggable card to assistive tech as a focusable grab target", () => {
    ...
    expect(
      wrapper.find('[data-slot="kanban-live-region"]').attributes("aria-live"),
    ).toBe("polite");
    wrapper.unmount();
  });
  ...
  it("picks a card up on Space and announces how to move it", async () => {
    ...
    expect(wrapper.find('[data-slot="kanban-live-region"]').text()).toContain(
      "Picked up card",
    );
    wrapper.unmount();
  });
  ...
  it("moves a grabbed card to the next column on ArrowRight", async () => {
    ...
    expect(wrapper.emitted("card-move")?.[0]).toEqual([
      { cardId: "card-1", fromColumnId: "todo", toColumnId: "done" },
    ]);
    expect(wrapper.find('[data-slot="kanban-live-region"]').text()).toContain(
      "Moved to Done",
    );
