...
const isDragging = computed(
  () => kanban?.draggingCardId.value === props.id || isGrabbed.value,
);

const canKeyboardDrag = computed(
  () => props.keyboardDraggable && !props.disabled && !!kanban && !!column,
);
...
function focusSelfAfterMove() {
  ...
  requestAnimationFrame(() => {
    const moved = document.querySelector<HTMLElement>(
      `[data-slot="kanban-card"][data-card-id="${props.id}"]`,
    );
    moved?.focus();
  });
}
...
function moveWithinColumn(delta: -1 | 1) {
  const columnEl = cardEl.value?.closest<HTMLElement>(
