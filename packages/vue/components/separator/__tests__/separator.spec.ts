...
describe('Separator', () => {
  ...
  it('has data-uipkge', () => {
    const w = mountSeparator()
    expect(w.find('[data-slot="separator"]').attributes('data-uipkge')).toBeDefined()
    w.unmount()
  })
  ...
  it('has role="separator" or data-orientation', () => {
    ...
    expect(el.attributes('role') === 'separator' || el.attributes('data-orientation')).toBeTruthy()
    w.unmount()
  })

  it('has data-orientation="horizontal" by default', () => {
    const w = mountSeparator()
    expect(w.find('[data-slot="separator"]').attributes('data-orientation')).toBe('horizontal')
    w.unmount()
  })

  it('has data-orientation="vertical" when orientation prop is vertical', () => {
    const w = mountSeparator({ orientation: 'vertical' })
    expect(w.find('[data-slot="separator"]').attributes('data-orientation')).toBe('vertical')
    w.unmount()
  })
