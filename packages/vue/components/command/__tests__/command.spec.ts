...
import { Command, CommandInput, CommandList, CommandItem, CommandGroup, CommandEmpty } from '../index'
  ...
  return mount(
    {
      ...
      components: { Command, CommandInput, CommandList, CommandItem, CommandGroup, CommandEmpty },
    },
...
describe('Command', () => {
  ...
  it('has data-uipkge', () => {
    const w = mountCommand()
    expect(w.find('[data-slot="command"]').attributes('data-uipkge')).toBeDefined()
    w.unmount()
  })
  ...
  it('CommandItem has data-uipkge', () => {
    const w = mountCommand()
    expect(w.find('[data-slot="command-item"]').attributes('data-uipkge')).toBeDefined()
    w.unmount()
  })
  ...
  it('CommandGroup renders heading text', () => {
    const w = mountCommand()
    expect(w.find('[data-slot="command-group-heading"]').text()).toContain('Suggestions')
    w.unmount()
  })
  ...
  it('CommandEmpty renders when no results match', async () => {
    ...
    expect(w.find('[data-slot="command-empty"]').text()).toContain('No results')
    w.unmount()
  })
