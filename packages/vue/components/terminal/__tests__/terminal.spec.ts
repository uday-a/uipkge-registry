...
describe('Terminal', () => {
  ...
  it('has data-uipkge', () => {
    const w = mount(Terminal, { props: { lines }, attachTo: document.body })
    expect(w.find('[data-slot="terminal"]').attributes('data-uipkge')).toBeDefined()
    w.unmount()
  })
  ...
  it('applies data-theme="dark" by default', () => {
    const w = mount(Terminal, { props: { lines }, attachTo: document.body })
    expect(w.find('[data-slot="terminal"]').attributes('data-theme')).toBe('dark')
    w.unmount()
  })

  it('applies data-theme="light" when theme prop is set', () => {
    const w = mount(Terminal, { props: { lines, theme: 'light' }, attachTo: document.body })
    expect(w.find('[data-slot="terminal"]').attributes('data-theme')).toBe('light')
    w.unmount()
  })

  it('renders the title in the title bar', () => {
    const w = mount(Terminal, { props: { lines, title: 'zsh' }, attachTo: document.body })
