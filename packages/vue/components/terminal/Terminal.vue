    ...
    <div
      :class="
        cn(
          ...
          theme === 'dark' ? 'border-border bg-muted' : 'border-border bg-muted',
        )
      "
    >
    ...
    <div ref="bodyRef" class="overflow-auto p-4 leading-relaxed" :style="{ maxHeight: props.maxHeight }">
      <div v-for="(line, i) in shownLines" :key="i" data-slot="terminal-line" class="break-words whitespace-pre-wrap">
        <slot name="line" :line="line" :index="i">
          ...
          >
            <span :class="cn('text-success shrink-0 font-semibold')">{{ line.prompt }}</span>
            <span>{{ line.command }}</span>
          </div>
          <div v-if="line.output" data-slot="terminal-output" class="text-muted-foreground">{{ line.output }}</div>
        </slot>
      </div>
    </div>
  </div>
