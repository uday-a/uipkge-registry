...
describe('EmptyState', () => {
  ...
  it('renders icon when icon prop is provided', () => {
    const w = mount(EmptyState, { props: { icon: TestIcon }, attachTo: document.body })
    expect(w.find('[data-testid="test-icon"]').exists()).toBe(true)
    w.unmount()
  })

  it('renders title text when title prop is provided', () => {
    const w = mount(EmptyState, { props: { title: 'No results' }, attachTo: document.body })
  ...
  it('renders description text when description prop is provided', () => {
    const w = mount(EmptyState, { props: { description: 'Try adjusting filters' }, attachTo: document.body })
  ...
  it('uses custom heading tag when headingTag prop is set', () => {
    const w = mount(EmptyState, { props: { title: 'Title', headingTag: 'h2' }, attachTo: document.body })
