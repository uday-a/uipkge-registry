...
describe('ContextMenu', () => {
  it('ContextMenuTrigger renders with data-slot="context-menu-trigger"', () => {
    const w = mountContextMenu('<ContextMenu><ContextMenuTrigger>Right click</ContextMenuTrigger></ContextMenu>')
  ...
  it('ContextMenuItem has data-uipkge', async () => {
    ...
    await openContextMenu(w)
    expect(document.querySelector('[data-slot="context-menu-item"]')?.getAttribute('data-uipkge')).toBeDefined()
    w.unmount()
  })
  ...
  it('ContextMenuItem applies data-inset when inset is true', async () => {
    ...
    await openContextMenu(w)
    expect(document.querySelector('[data-slot="context-menu-item"]')?.getAttribute('data-inset')).toBeDefined()
    w.unmount()
  })
  ...
  it('ContextMenuItem applies data-variant when variant is destructive', async () => {
    ...
    await openContextMenu(w)
    expect(document.querySelector('[data-slot="context-menu-item"]')?.getAttribute('data-variant')).toBe('destructive')
    w.unmount()
  })
  ...
  it('ContextMenuItem renders slot content', async () => {
    ...
    await openContextMenu(w)
    expect(document.querySelector('[data-slot="context-menu-item"]')?.textContent).toContain('My Item Text')
    w.unmount()
  })
})
