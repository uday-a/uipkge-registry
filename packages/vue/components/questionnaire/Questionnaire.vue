...
const answers = defineModel<QuestionnaireAnswers>("answers", {
  default: () => ({}),
});
...
const activeIndex = computed(() =>
  Math.max(0, names.value.indexOf(currentName.value ?? "")),
);
const current = computed(() => props.items[activeIndex.value]);
const isFirst = computed(() => activeIndex.value <= 0);
const isLast = computed(
  () => names.value.length === 0 || activeIndex.value >= names.value.length - 1,
);
...
function toggleChoice(name: string, value: string, multiple?: boolean) {
  if (multiple) {
    const currentVal = Array.isArray(answers.value[name])
      ? [...(answers.value[name] as string[])]
      : [];
    const next = currentVal.includes(value)
...
function shortcutFor(index: number) {
  ...
  if (props.shortcuts === "letters" && index < 26)
    ...
    >
      ...
      >
        <label
          ...
          :data-checked="
            choiceChecked(current.name, choice.value) ? '' : undefined
          "
          :class="
            cn(
              questionnaireChoiceVariants(),
      ...
      <input
        ...
        :value="
          typeof answers[current.name] === 'string' ? answers[current.name] : ''
        "
