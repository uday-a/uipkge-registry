...
describe("CodeBlock", () => {
  ...
  it("exposes disclosure state and the controlled code region", async () => {
    ...
    const toggle = w
      .findAll("button")
      .find((button) => button.text().includes("Hide code"))!;
  ...
  it("syntax-highlights recognized languages", async () => {
    ...
    await vi.waitFor(() =>
      expect(w.findAll("[data-syntax-token]").length).toBeGreaterThan(1),
    );

    expect(w.find("[data-syntax-token]").attributes("style")).toContain(
      "--shiki-light",
    );
    w.unmount();
  });
  ...
  it("keeps block elements outside the pre element", () => {
    ...
    const childTags = Array.from(w.find("pre").element.children).map(
      (child) => child.tagName,
    );
