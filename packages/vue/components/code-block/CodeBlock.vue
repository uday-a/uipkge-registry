  ...
  async ([code, language]) => {
    ...
    try {
      ...
      if (request === highlightRequest)
        highlightedSegments.value = toHighlightSegments(tokens, code);
    } catch {
...
<template>
  ...
  >
    ...
    <div
      v-if="showHeader"
      class="border-border bg-muted/40 flex items-center justify-between gap-2 border-b px-3 py-2"
    >
      <span
    ...
    >
      <div class="flex w-max min-w-full">
        ...
        >
          <span v-for="(_, i) in lines" :key="i" class="block">{{
            i + 1
          }}</span>
        </div>
        <pre
          class="m-0 min-w-max flex-1"
        ><code class="block cursor-text whitespace-pre px-4 py-3"><template
