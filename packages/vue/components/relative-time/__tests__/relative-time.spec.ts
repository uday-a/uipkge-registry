...
describe('RelativeTime', () => {
  it('renders with data-slot="relative-time"', () => {
    const w = mount(RelativeTime, { props: { date: now, now, locale: 'en' }, attachTo: document.body })
  ...
  it('shows absolute UTC when display is absolute', () => {
    const w = mount(RelativeTime, {
      props: { date: now, now, locale: 'en-GB', display: 'absolute', timeZone: 'UTC' },
      attachTo: document.body,
    })
    expect(w.find('[data-slot="relative-time"]').text()).toContain('12:00')
    expect(w.find('[data-slot="relative-time"]').attributes('data-timezone')).toBe('UTC')
    w.unmount()
  })
  ...
  it('parses naive ISO as UTC', () => {
    ...
    expect(w.find('time').attributes('datetime')).toBe('2026-08-14T12:00:00.000Z')
    w.unmount()
  })
  ...
  it('accepts custom class', () => {
    ...
    expect(w.find('[data-slot="relative-time"]').classes()).toContain('custom-class')
    w.unmount()
  })
})
