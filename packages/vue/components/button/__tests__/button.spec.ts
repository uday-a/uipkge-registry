...
describe('ButtonGroup', () => {
  it('renders with data-slot="button-group" and role="group"', async () => {
    ...
    const w = mount(ButtonGroup, {
      slots: {
        default: '<button data-slot="button">First</button><button data-slot="button">Second</button>',
      },
      attachTo: document.body,
    })
    expect(w.find('[data-slot="button-group"]').exists()).toBe(true)
    expect(w.find('[data-slot="button-group"]').attributes('role')).toBe('group')
    expect(w.find('[data-slot="button-group"]').attributes('data-orientation')).toBe('horizontal')
    w.unmount()
  })
  ...
  it('renders vertical orientation attribute', async () => {
    ...
    const w = mount(ButtonGroup, {
      ...
      slots: {
        default: '<button data-slot="button">Top</button><button data-slot="button">Bottom</button>',
      },
      attachTo: document.body,
    })
    expect(w.find('[data-slot="button-group"]').attributes('data-orientation')).toBe('vertical')
    w.unmount()
  })
  ...
  it('renders attached attribute by default', async () => {
    ...
    const w = mount(ButtonGroup, {
      slots: {
        default: '<button data-slot="button">A</button><button data-slot="button">B</button>',
      },
      attachTo: document.body,
    })
    expect(w.find('[data-slot="button-group"]').attributes('data-attached')).toBeDefined()
    w.unmount()
  })
})
