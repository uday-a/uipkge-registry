...
function mountPinInput(props: Record<string, unknown> = {}, slots = "") {
  const slotContent =
    slots ||
    Array.from(
      { length: props.maxLength ?? 4 },
      (_, i) => `<PinInputSlot :index="${i}" />`,
    ).join("");
  return mount(
    {
...
describe("PinInput", () => {
  ...
  it("sets data-status attribute when status is set", () => {
    ...
    expect(w.find('[data-slot="pin-input"]').attributes("data-status")).toBe(
      "error",
    );
    w.unmount();
  });
    ...
    const w = mount(
      {
        components: { PinInput, PinInputSlot },
        template:
          '<PinInput :max-length="4"><PinInputSlot :index="0" /><PinInputSlot :index="1" /></PinInput>',
      },
      { attachTo: document.body },
    );
    expect(w.find('[data-slot="pin-input"]').exists()).toBe(true);
