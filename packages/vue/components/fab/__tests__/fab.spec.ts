...
function mountFab(props: Record<string, unknown> = {}, slots: Record<string, unknown> = {}) {
...
describe('Fab', () => {
  ...
  it('applies data-position attribute', () => {
    ...
    expect(w.find('[data-slot="fab"]').attributes('data-position')).toBe('top-left')
    w.unmount()
  })

  it('applies data-variant attribute', () => {
    const w = mountFab({ variant: 'destructive' })
    expect(w.find('[data-slot="fab"]').attributes('data-variant')).toBe('destructive')
    w.unmount()
  })
