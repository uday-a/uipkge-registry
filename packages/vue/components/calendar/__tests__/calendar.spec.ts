...
describe("Calendar", () => {
  ...
  it("has data-uipkge", () => {
    const w = mountCalendar();
    expect(
      w.find('[data-slot="calendar"]').attributes("data-uipkge"),
    ).toBeDefined();
    w.unmount();
  });
  ...
  it("renders a grid with dates", () => {
    ...
    expect(
      w.findAll('[data-slot="calendar-cell-trigger"]').length,
    ).toBeGreaterThan(0);
    w.unmount();
  });
  ...
  it("renders weekday headers", () => {
    ...
    expect(
      w.findAll('[data-slot="calendar-head-cell"]').length,
    ).toBeGreaterThan(0);
    w.unmount();
  });
  ...
  it("renders current month/year heading", () => {
    ...
    expect(
      w.find('[data-slot="calendar-heading"]').text().trim().length,
    ).toBeGreaterThan(0);
    w.unmount();
  });
  ...
  it("renders calendar grid body with rows", () => {
    ...
    expect(w.findAll('[data-slot="calendar-grid-row"]').length).toBeGreaterThan(
      0,
    );
    w.unmount();
  });
