<script
  setup
  lang="ts"
  generic="T extends Record<string, unknown> | string | number"
>
...
import { readKey } from "@/components/ui/select";
...
const filteredOptions = computed(() => {
  ...
  if (q && props.filterSort) {
    result = [...result].sort((a, b) =>
      props.filterSort!(a, b, { searchValue: q }),
    );
  }
...
const atMax = computed(() => {
  ...
  const count = Array.isArray(props.modelValue)
...
const showClear = computed(() => {
  return (
    props.allowClear && !isEmpty.value && !props.disabled && !props.loading
  );
});
            ...
            >
              ...
              >
                ...
                <span
                  ...
                  @keydown.enter.prevent="removeTag(getValue(opt), $event)"
                  @keydown.space.prevent="removeTag(getValue(opt), $event)"
                >
                  <X class="size-3" />
                </span>
              </Badge>
        ...
        <CommandList class="flex-1 overflow-y-auto">
          ...
          <template v-for="(group, gi) in grouped" :key="group.heading || gi">
            ...
            <CommandGroup :heading="group.heading || undefined">
              <CommandItem
                ...
                :disabled="
                  isDisabled(opt) || (atMax && !selectedSet.has(getValue(opt)))
                "
                :data-active="
                ...
                "
                :style="
                  virtual && props.options.length > 100
          ...
          <CommandItem
            v-if="
              ...
              !props.options.some((o) => getLabel(o) === query.trim())
            "
            :value="`create:${query}`"
            @select="createTag"
          >
