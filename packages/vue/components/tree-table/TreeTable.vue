...
const expanded = ref<Set<string>>(props.defaultExpanded ? collectExpandedIds(props.data) : new Set())
const internalSelected = ref<Set<string>>(new Set())

const selectedSet = computed(() => (props.selected ? new Set(props.selected) : internalSelected.value))
...
function onRowKeydown(e: KeyboardEvent, fr: FlatRow) {
  ...
  if (e.key === 'ArrowLeft') {
    ...
    if (fr.hasChildren && isExpanded(fr.row.id)) {
      ...
      const parent = root?.querySelector<HTMLElement>(`[data-tree-row][data-tree-id="${CSS.escape(fr.parentId)}"]`)
      focusRow(parent)
    }
    return
  }
        ...
        >
          ...
          <TableCell v-for="(col, ci) in columns" :key="col.key" :class="cn(ci === 0 && 'font-medium', col.cellClass)">
            <div
              ...
              :style="ci === 0 ? { paddingLeft: `calc(${fr.depth} * var(--tree-indent))` } : {}"
            >
