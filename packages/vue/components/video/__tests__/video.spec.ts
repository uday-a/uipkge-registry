...
describe("Video", () => {
  ...
  it("has data-uipkge on container", () => {
    ...
    expect(
      w.find('[data-slot="video"]').attributes("data-uipkge"),
    ).toBeDefined();
    w.unmount();
  });
  ...
  it("does not render custom controls when nativeControls is true", () => {
    const w = mount(Video, {
      props: { src: "test.mp4", nativeControls: true },
      attachTo: document.body,
    });
...
it("shows a recoverable error when the browser rejects playback", async () => {
  ...
  vi.spyOn(w.get("video").element, "play").mockRejectedValue(
