...
describe('CircularProgress', () => {
  ...
  it('renders a progress circle (second circle)', () => {
    const w = mount(CircularProgress, { props: { value: 50 }, attachTo: document.body })
    const circles = w.findAll('circle')
    expect(circles.length).toBeGreaterThanOrEqual(2)
  ...
  it('sets data-indeterminate="true" when indeterminate', () => {
    const w = mount(CircularProgress, { props: { indeterminate: true }, attachTo: document.body })
    expect(w.find('[data-slot="circular-progress"]').attributes('data-indeterminate')).toBe('true')
    w.unmount()
  })

  it('sets data-complete="true" when value reaches 100', () => {
    const w = mount(CircularProgress, { props: { value: 100 }, attachTo: document.body })
    expect(w.find('[data-slot="circular-progress"]').attributes('data-complete')).toBe('true')
    w.unmount()
  })

  it('shows numeric value when showValue is true', () => {
    const w = mount(CircularProgress, { props: { value: 42, showValue: true }, attachTo: document.body })
