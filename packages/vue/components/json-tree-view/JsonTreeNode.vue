...
<template>
  ...
  >
    ...
    <div v-if="isContainer && open" role="group">
      ...
      />
      <div class="text-muted-foreground py-0.5 select-none" :style="{ paddingLeft: `${indent}px` }">
        {{ type === 'array' ? ']' : '}' }}
      </div>
    </div>
    ...
    >
      ...
      <span class="text-muted-foreground">:</span>
      <span :class="typeColor[type] ?? 'text-foreground'" class="rounded text-left font-mono">
        {{ formatValue(data) }}
      </span>
      <button
        type="button"
