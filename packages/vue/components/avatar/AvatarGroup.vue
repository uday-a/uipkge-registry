...
const allChildren = computed(() =>
  flattenVNodes(slots.default?.() as VNode[] | undefined),
);
...
const isOverflowing = computed(
  () => props.max != null && totalCount.value > props.max,
);
const visibleLimit = computed(() =>
  isOverflowing.value
    ? Math.max((props.max ?? 0) - 1, 0)
    : allChildren.value.length,
);
const overflowCount = computed(() =>
  isOverflowing.value ? totalCount.value - visibleLimit.value : 0,
);
const visibleChildren = computed(() =>
  allChildren.value.slice(0, visibleLimit.value),
);
...
<template>
  <div
    :class="
      cn('flex items-center', overlap ? '-space-x-2' : 'gap-1', props.class)
    "
    data-uipkge
  ...
  >
    ...
    >
      <slot name="overflow" :count="overflowCount">
        <span class="flex size-full items-center justify-center font-medium"
          >+{{ overflowCount }}</span
        >
      </slot>
    </div>
