...
describe('OrganizationChart', () => {
  ...
  it('has data-uipkge on container', async () => {
    const w = await mountChart()
    expect(w.find('[data-slot="organization-chart"]').attributes('data-uipkge')).toBeDefined()
    w.unmount()
  })

  it('applies data-direction="top-down" by default', async () => {
    const w = await mountChart()
    expect(w.find('[data-slot="organization-chart"]').attributes('data-direction')).toBe('top-down')
    w.unmount()
  })
