...
describe("GradientText", () => {
  ...
  it("renders as a span by default", () => {
    const w = mount(GradientText, { attachTo: document.body });
    expect(
      w.find('[data-slot="gradient-text"]').element.tagName.toLowerCase(),
    ).toBe("span");
    w.unmount();
  });

  it("renders slot text content", () => {
    const w = mount(GradientText, {
      slots: { default: "Hello gradient" },
  ...
  it("applies data-preset when preset prop is set", () => {
    ...
    expect(
      w.find('[data-slot="gradient-text"]').attributes("data-preset"),
    ).toBe("sunset");
    w.unmount();
  });
  ...
  it('applies data-animated="true" when animated prop is set', () => {
    ...
    expect(
      w.find('[data-slot="gradient-text"]').attributes("data-animated"),
    ).toBe("true");
    w.unmount();
  });
