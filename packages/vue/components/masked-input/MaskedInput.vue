...
const emit = defineEmits<{
  ...
  validate: [payload: { isValid: boolean; isComplete: boolean; rawValue: string; maskedValue: string }]
}>()
...
const editableSlots = computed(() => {
  ...
  for (let i = 0; i < props.mask.length; i++) {
    ...
    if (char === props.replacement || activeTokens.value[char]) {
      slots.push({ index: i, tokenChar: char === props.replacement ? props.replacement : char })
    }
  }
  return slots
})

const editablePositions = computed(() => editableSlots.value.map((s) => s.index))
...
function getSlotPattern(slotIndex: number): RegExp {
  ...
  if (!slot) return /.*/
  return activeTokens.value[slot.tokenChar] ?? activeTokens.value[props.replacement] ?? /.*/
}
...
function isSeparatorChar(char: string, index?: number): boolean {
  ...
  return props.mask.includes(char) && !editableSlots.value.some((s) => s.tokenChar === char)
}
  ...
  for (let i = 0; i < props.mask.length; i++) {
    ...
    if (isEditable) {
      ...
      if (rawIndex < rawValue.length && isValidCharForSlot(rawValue[rawIndex]!, slotIdx)) {
        ...
        rawIndex++
      } else if (props.showMask && (isFocused.value || !props.placeholder || props.modelValue)) {
...
function handlePaste(event: ClipboardEvent) {
  ...
  const newRaw = (raw.slice(0, rawIndex) + filteredPasted + raw.slice(rawIndex)).slice(0, maxLength.value)
  const masked = applyMask(newRaw)
  emit('update:modelValue', masked)

  const newPos = findCursorPosFromRaw(Math.min(rawIndex + filteredPasted.length, maxLength.value))
  nextTick(() => {
    if (inputRef.value) {
      inputRef.value.setSelectionRange(newPos, newPos)
    }
