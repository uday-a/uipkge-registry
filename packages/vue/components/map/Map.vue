  ...
  }>(),
  {
    accessToken: () =>
      ...
      (typeof import.meta !== "undefined" &&
        (import.meta as any).env?.PUBLIC_MAPBOX_TOKEN) ||
...
defineExpose({
  ...
  fitBounds: (...args: Parameters<mapboxgl.Map["fitBounds"]>) =>
    map?.fitBounds(...args),
  resize: () => map?.resize(),
});
...
<template>
  <div
    ...
    :data-muted="isMuted"
    :class="
      cn(mapVariants({ variant, ...(size ? { size } : {}) }), props.class)
    "
  >
    <MapboxMap
