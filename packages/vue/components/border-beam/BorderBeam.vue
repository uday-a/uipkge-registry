...
const beamStyles = computed<CSSProperties[]>(() => [
  {
    ...
    animation: reducedMotion.value
      ? "none"
      : `uipkge-border-beam ${props.duration}s linear infinite ${props.delay}s`,
    animationPlayState:
      !reducedMotion.value && props.paused ? "paused" : undefined,
  },
  {
...
<template>
  <span
    ...
    aria-hidden="true"
    :class="
      cn('pointer-events-none absolute inset-0 rounded-[inherit]', props.class)
    "
    :style="beamStyles"
  />
</template>
