...
const mergedOption = computed(() => {
  const totals = props.data.map((d) => props.yFields.reduce((s, f) => s + Math.abs(d[f] ?? 0), 0) || 1)
  const series = props.yFields.map((f, i) => ({
    ...
    label: props.percent
      ...
      : undefined,
    data: props.data.map((d, r) => (props.percent ? (Math.abs(d[f] ?? 0) / totals[r]!) * 100 : d[f])),
  }))
  ...
  const count = Math.max(props.yFields.length, Array.isArray(userSeries) ? userSeries.length : 0)
