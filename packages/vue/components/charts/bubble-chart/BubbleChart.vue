...
const mergedOption = computed(() => {
  ...
  const cats = categoryField
    ? [...new Set(props.data.map((d) => String(d[categoryField])))]
    : ["default"];
  const sizes = props.data.map((d) => +d[props.sizeField] || 0);
  const maxSize = Math.max(...sizes, 1);
  const scale = (v: number) =>
    props.minSize + Math.sqrt(v / maxSize) * (props.maxSize - props.minSize);
  const series = cats.map((cat, i) => ({
    ...
    data: (categoryField
      ? props.data.filter((d) => String(d[categoryField]) === cat)
      : props.data
    ).map((d) => [
      d[props.xField],
      d[props.yField],
