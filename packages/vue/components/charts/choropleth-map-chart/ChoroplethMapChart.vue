...
const values = computed(() =>
  props.data ? props.data.map((d) => d.value) : [],
);
...
const enrichedGeoJson = computed(() => {
  ...
  const features = props.geoJson.features.map((f: any) => {
    ...
    const nameVal = f.properties?.name
      ...
      : "";
    const matchedVal =
      dataMap.value.get(idVal) ?? dataMap.value.get(nameVal) ?? null;
...
<template>
  ...
  >
    ...
    >
      ...
      <div
        class="h-2 w-24 rounded-full bg-gradient-to-r from-sky-400/30 to-sky-400"
      />
