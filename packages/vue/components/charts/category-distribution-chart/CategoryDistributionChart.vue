...
const total = computed(
  () => props.categories.reduce((s, c) => s + c.percentage, 0) || 1,
);
  ...
  >
    <div class="flex items-baseline gap-2">
      ...
      <span
        ...
        :style="{
          color:
            trend.direction === 'up' ? 'var(--chart-2)' : 'var(--destructive)',
        }"
      >
        {{ trend.direction === "up" ? "+" : "−" }}{{ trend.value }}
      </span>
      <span v-if="primaryLabel" class="text-muted-foreground text-xs">{{
        primaryLabel
      }}</span>
    </div>
    <div
      class="mt-3 flex h-3 w-full overflow-hidden rounded-full"
      role="presentation"
    >
      <div
        v-for="s in slices"
