...
export async function verifyRegistry(cfg: RegistryVerifyConfig) {
  ...
  const fail = (item: string, check: string, detail: string) => failures.push({ item, check, detail })
  ...
  async function checkItem(jsonPath: string) {
    ...
    if (item.$schema !== cfg.itemSchema) fail(name, '$schema', `expected '${cfg.itemSchema}', got '${item.$schema}'`)
    ...
    if (!ALLOWED_TYPES.has(item.type)) fail(name, 'type', `'${item.type}' not in spec`)
    for (const field of FORBIDDEN_FIELDS) {
      if (field in item) fail(name, `forbidden:${field}`, 'should not be in output')
    }
    ...
    if (Array.isArray(item.registryDependencies)) {
      for (const url of item.registryDependencies) {
        ...
        if (url.startsWith('http') && !url.startsWith(CANONICAL_DEP_BASE)) {
          fail(name, 'registryDependencies host', `'${url}' should start with '${CANONICAL_DEP_BASE}'`)
        }
      }
    }
  }
  ...
  async function checkIndex() {
    ...
    if (idx.$schema !== cfg.indexSchema)
      fail('<index>', '$schema', `expected '${cfg.indexSchema}', got '${idx.$schema}'`)
    ...
    if (Array.isArray(idx.items)) {
      for (const it of idx.items) {
        ...
        if (!it.title) fail('<index>', 'items[].title', `'${it.name}' missing title`)
      }
    }
  }

  const files = (await readdir(OUT_DIR)).filter((f) => f.endsWith('.json') && f !== 'registry.json')
  for (const f of files) await checkItem(join(OUT_DIR, f))
  await checkIndex()

  if (failures.length === 0) {
    console.log(`[verify] PASS — ${files.length} items + index conform to spec`)
    return
  }
  ...
  console.error(`[verify] FAIL — ${failures.length} issue(s) across ${byCheck.size} category(ies):`)
  for (const [check, fs] of byCheck.entries()) {
    console.error(`\n  ${check} (${fs.length}):`)
    for (const f of fs.slice(0, 5)) console.error(`    - ${f.item}: ${f.detail}`)
